...
# `vitest --shard X/N` against the server's vitest.coverage-shard.config.ts
# (which uses the `blob` reporter so the merge step can combine shard outputs
# into one canonical coverage report).
#
# Usage:
#   bash scripts/ci/backend-coverage-shard.sh <shard-index> <shard-total>
#
# Run from the server/ directory (vitest binary path assumes server cwd).
...
coverage_reports_dir_for_shard() {
  ...
  printf '%s' "${SANCTUARY_BACKEND_COVERAGE_REPORTS_DIR:-coverage-shards/shard-${shard_index}-${shard_total}}"
}
...
run_vitest_shard_with_native_retry() {
  ...
  local shard_total="$3"
  local expected_blob="$4"
  local attempts="${SANCTUARY_BACKEND_COVERAGE_SEGFAULT_ATTEMPTS:-3}"

  is_positive_integer "$attempts" || fail 'SANCTUARY_BACKEND_COVERAGE_SEGFAULT_ATTEMPTS must be a positive integer'

  local log_dir="${SANCTUARY_BACKEND_COVERAGE_LOG_DIR:-.tmp/backend-coverage}"
  mkdir -p "$log_dir"
  ...
  for attempt in $(seq 1 "$attempts"); do
    ...
    echo "backend-coverage-shard: retrying shard ${shard_index}/${shard_total} after retryable Vitest infrastructure failure (attempt $((attempt + 1))/${attempts})" >&2
  done
}
