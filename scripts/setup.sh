...
COMPOSE_SSL_DIR="${SANCTUARY_COMPOSE_SSL_DIR:-$SSL_DIR}"
...
get_container_network_name() {
    ...
    docker inspect --format '{{range $k, $v := .NetworkSettings.Networks}}{{println $k}}{{end}}' "$container_id" 2>/dev/null \
        | head -n 1
}
...
validate_postgres_password() {
    ...
    network_name="$(get_container_network_name "$container_id")"
...
sync_postgres_password_to_runtime_env() {
    ...
    if run_local_postgres_command "$container_id" "" \
        "ALTER USER \"$db_user\" WITH PASSWORD '$escaped_password';"; then
        return 0
    fi

    if [ -n "$container_password" ] && [ "$container_password" != "$POSTGRES_PASSWORD" ] \
        && run_local_postgres_command "$container_id" "$container_password" \
            "ALTER USER \"$db_user\" WITH PASSWORD '$escaped_password';"; then
        return 0
    fi
    ...
    docker exec \
        -u postgres \
        "$container_id" \
        psql -d postgres -v ON_ERROR_STOP=1 -c "ALTER USER \"$db_user\" WITH PASSWORD '$escaped_password';" \
        >/dev/null 2>&1
}
...
load_or_generate_secrets() {
    ...
    if [ -n "$WORKER_DIAGNOSTICS_SECRET" ]; then
        echo "  - WORKER_DIAGNOSTICS_SECRET: using existing"
    else
        WORKER_DIAGNOSTICS_SECRET=$(openssl rand -hex 32 2>/dev/null || generate_secret)
        echo "  - WORKER_DIAGNOSTICS_SECRET: generated"
    fi
...
# CIDR allowlist.
LLM_EGRESS_PROXY_ALLOWED_HOSTS=${LLM_EGRESS_PROXY_ALLOWED_HOSTS:-}
LLM_EGRESS_PROXY_ALLOWED_CIDRS=${LLM_EGRESS_PROXY_ALLOWED_CIDRS:-}
LLM_EGRESS_PROXY_ALLOW_PUBLIC_HTTPS=${LLM_EGRESS_PROXY_ALLOW_PUBLIC_HTTPS:-false}
...
show_backup_reminder() {
    ...
    echo ""
    # Name the secrets, never print them: this output is captured in install
    # and upgrade logs that stay on disk and get shared when asking for help.
    echo -e "${YELLOW}Critical secrets to back up:${NC} ENCRYPTION_KEY and ENCRYPTION_SALT"
    echo "(their values are in the file above; they are not shown here)"
    echo ""
    echo "Back up options:"
    echo "  1. Copy the runtime env file to a secure location"
    echo "  2. Save both values from that file to a password manager"
    echo "  3. Print them from that file and store the copy in a safe place"
    echo ""
