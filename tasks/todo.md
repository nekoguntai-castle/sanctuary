...
- PR delivery follow-up: regenerated the frontend architecture graph after CI detected stale generated output, and added root test-harness dev dependencies for the AI proxy modules imported by `tests/ai-proxy/*` so `npm ci && npm run typecheck:tests` passes without relying on a nested `ai-proxy/node_modules`.
