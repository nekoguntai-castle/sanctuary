...
- [x] Add UI labels for "Agent Funding Wallet", "Agent Operational Wallet", and "Linked agent".
...
## Resolved Decisions And Residual Follow-Up

- [x] Operational receive address behavior: Sanctuary returns an existing verified receive address or derives one from linked watch-only descriptor metadata when available. Agent-provided destinations are accepted only when Sanctuary verifies they belong to the linked operational wallet.
- [x] Approval semantics: the human multisig signature is the approval for normal in-policy funding. Explicit approval/voting is reserved for override/governance-style workflows.
