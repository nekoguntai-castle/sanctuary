# Middleware Bug Audit — Claude

Audit of `/home/nekoguntai/sanctuary/server/src/middleware/` (18 TS files).
Date: 2026-05-12
...
**Repro / trigger:** `curl -H 'X-Forwarded-For: 1.1.1.1' …` repeated with rotating values from the same source IP — each request gets a fresh rate-limit bucket.
**Fix shape:** Replace the manual XFF parse with `req.ip` (already proxy-aware) and only fall back to `socket.remoteAddress`. Drop the leftmost-IP logic entirely.
**Confidence:** high
...
### [LOW] middleware/csrf.ts:217-222 — `req.cookies` reassignment is technically a mutation footgun
**Category:** Logic/invariant violations
...
### [LOW] middleware/featureGate.ts:73-90 — Service-error fallback to config silently masks feature-flag-service outages
**Category:** Logic/invariant violations
**What:** When `featureFlagService.isEnabled` rejects, all three middlewares fall back to `getConfig().features` without distinguishing a transient outage from a routine "service unavailable" state. A persistent flag service outage means runtime feature toggles (audit-traceable) silently revert to file-config values, potentially re-enabling a feature that was deliberately disabled at runtime.
**Why it matters:** Security-sensitive: a feature disabled in the DB for a compliance reason silently re-enables itself if the service breaks. The `log.warn` is the only signal.
...
**Fix shape:** Fail closed on service errors for security-sensitive flags, OR add a per-flag policy (e.g. `failOpen: false`) controlling fallback behavior. At minimum, escalate the log to `error` and add a metric.
**Confidence:** medium
...
**Severity counts:** 0 critical, 2 high, 7 medium, 7 low

**Top 3 by impact:**
1. `rateLimit.ts:86-95` — XFF leftmost-trust enables auth-rate-limit bypass (HIGH)
2. `auth.ts:97-109` — `authenticate` masks DB outages as 401 (HIGH)
3. `apiVersion.ts:140-151` — Duplicate `api_version` query param crashes versioning middleware (MEDIUM, trivially exploitable DoS)

**Files reviewed (18):** agentAuth.ts, apiVersion.ts, auth.ts, authCookieNames.ts, bodyParsing.ts, corsOrigin.ts, csrf.ts, deviceAccess.ts, featureGate.ts, gatewayAuth.ts, i18n.ts, metrics.ts, rateLimit.ts, requestLogger.ts, requestTimeout.ts, resourceAccess.ts, validate.ts, walletAccess.ts
