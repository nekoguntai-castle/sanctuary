...
| largest_file_lines | 1258 | `server/tests/unit/api/admin-routes.test.ts` after the blockchain service split; next largest validated files include `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` at 1228 LOC and `server/tests/unit/repositories/policyRepository.test.ts` at 1210 LOC when generated verified-vector files are excluded. `server/tests/unit/services/blockchainService.test.ts` is now a 23-line registrar, with blockchain service contract modules capped at 352 LOC. | 3.3 → 0 |
...
2. **Remaining oversized API/service tests** — the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, and transfer service god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/api/admin-routes.test.ts` at 1258 LOC, followed by `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` at 1228 LOC and `server/tests/unit/repositories/policyRepository.test.ts` at 1210 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, and transfer service god-files are gone, but `server/tests/unit/api/admin-routes.test.ts` is now the largest scoped TS/TSX file at 1258 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/api/admin-routes.test.ts` (1258 LOC), `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` (1228 LOC), and `server/tests/unit/repositories/policyRepository.test.ts` (1210 LOC), excluding generated verified-vector files.
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `server/tests/unit/api/admin-routes.test.ts` at 1258 LOC after the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, transaction HTTP route, admin API, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/unit/api/admin-routes.test.ts` at 1258 LOC, followed by `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` at 1228 LOC and `server/tests/unit/repositories/policyRepository.test.ts` at 1210 LOC when generated verified-vector files are excluded.
