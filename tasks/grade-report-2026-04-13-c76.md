...
| Maintainability         | 8/15      | lizard baseline measured at 83 warnings; jscpd measured at 2.33%; the validated largest-file split backlog through the AISettings test split is complete, but the largest non-generated TS/TSX file is now `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC; clean architecture |
...
| largest_file_lines | 1056 | `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` after the AISettings test split; next largest validated files are `server/tests/unit/api/push.test.ts` at 1052 LOC and `server/tests/unit/services/approvalService.test.ts` at 1032 LOC when generated verified-vector files are excluded. `tests/components/AISettings.test.tsx` is now a 31-line registrar, with AISettings contract modules capped at 157 LOC. | 3.3 → 0 |
...
| Largest file | Corrected after implementation: prior oversized split passes are recorded in the implementation log below. After the AISettings split, `tests/components/AISettings.test.tsx` is a 31-line registrar with AISettings contract modules capped at 157 LOC; the scoped largest-file scan now reports `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC, followed by `server/tests/unit/api/push.test.ts` at 1052 LOC and `server/tests/unit/services/approvalService.test.ts` at 1032 LOC when generated verified-vector files are excluded. | Next largest-file criterion work is splitting `server/tests/unit/services/bitcoin/sync/confirmations.test.ts`, then `server/tests/unit/api/push.test.ts`, or reframing the scoring threshold. |
...
2. **Remaining oversized API/service modules** — the validated split backlog through the AISettings test file is complete, but the current largest non-generated TS/TSX file is now `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC, followed by `server/tests/unit/api/push.test.ts` at 1052 LOC and `server/tests/unit/services/approvalService.test.ts` at 1032 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The validated largest-file split backlog through the AISettings test split is complete, but `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` is still 1056 LOC and keeps the repo-wide largest-file criterion at 0.
...
- Next validated targets: `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` (1056 LOC), followed by `server/tests/unit/api/push.test.ts` (1052 LOC) and `server/tests/unit/services/approvalService.test.ts` (1032 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after validated largest-file split backlog through AISettings test) | 0 (`server/tests/unit/services/bitcoin/sync/confirmations.test.ts` 1056 LOC; next `server/tests/unit/api/push.test.ts` 1052 LOC) | 0 | +2 | +2 |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals do not move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized modules**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest scoped TS/TSX file is now `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC after the validated largest-file split backlog through the AISettings test split.
