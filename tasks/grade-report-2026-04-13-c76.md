...
| Maintainability         | 8/15      | lizard baseline measured at 83 warnings; jscpd measured at 2.33%; OpenAPI, transaction API, admin API, policy engine, blockchain service, and backup service test splits are complete, but largest non-generated TS/TSX file is still a 2275 LOC service test; clean architecture |
...
| largest_file_lines | 2275 | `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` after the backup service contract split; next largest validated files include `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded. `server/tests/unit/services/backupService.test.ts` is now a 29-line registrar, with backup service contract modules capped at 559 LOC. | 3.3 → 0 |
...
2. **Remaining oversized service/integration tests** — the OpenAPI, transaction API, admin API, policy engine, blockchain service, and backup service god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` at 2275 LOC, followed by `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, admin API, policy engine, blockchain service, and backup service god-files are gone, but `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` is now the largest scoped TS/TSX file at 2275 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` (2275 LOC), `server/tests/integration/flows/wallet.integration.test.ts` (2064 LOC), and `server/tests/unit/api/auth.routes.registration.test.ts` (2030 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + admin API + policy engine + blockchain service + backup service splits) | 0 (`phases.processTransactions.test.ts` 2275 LOC) | 0 | +2 | +2 |
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, admin API, policy engine, blockchain service, and backup service splits is `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` at 2275 LOC, followed by `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded.
...
- Current largest non-generated TS/TSX file after the later policy engine, blockchain service, and backup service splits is `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` at 2275 LOC, followed by `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded. The admin split closed the named oversized API-test P1 item, while service/integration test files still keep the repo-wide 3.3 score at 0.
...
- Current largest non-generated TS/TSX file after the later blockchain service and backup service splits is `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` at 2275 LOC, followed by `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded. This reduces the policy engine hotspot but does not move the repo-wide 3.3 score yet.
...
- Current largest non-generated TS/TSX file after the later backup service split is `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` at 2275 LOC, followed by `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded. This reduces the blockchain service hotspot but does not move the repo-wide 3.3 score yet.
...
- Current largest non-generated TS/TSX file after the split is `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` at 2275 LOC, followed by `server/tests/integration/flows/wallet.integration.test.ts` at 2064 LOC and `server/tests/unit/api/auth.routes.registration.test.ts` at 2030 LOC when generated verified-vector files are excluded. This reduces the backup service hotspot but does not move the repo-wide 3.3 score yet.
