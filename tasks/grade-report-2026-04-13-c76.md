...
- `lizard` — not installed globally, but a temporary `/tmp` install measured 31 warnings after the continued extraction passes through the maintenance jobs forwarding split. `.github/workflows/quality.yml` is now blocking with `-i 31`, so the warning count cannot grow without failing CI.
...
1. **CI quality signals are now blocking, but lizard is baseline-gated.** The new lint, gitleaks, lizard, and jscpd jobs are blocking in `.github/workflows/quality.yml`. `lizard` still has 31 existing warnings, so the immediate guardrail is "do not increase warning count"; further baseline reduction remains future maintainability work.
...
- Baseline the current lizard/jscpd output, document any intentional deltas, then remove `continue-on-error: true` from each job in `quality.yml` **per-job, not as a batch**. Implemented with jscpd at 2.33%, lizard now at a 31-warning no-increase baseline after the continued extraction passes through the maintenance jobs forwarding split, and gitleaks scoped to PR/latest-commit regression scanning because full-history/current-directory scans include legacy/test/ignored-file false positives.
...
| CI signals enforceable | Done for regression gating: `scripts/quality.sh` runs all three tools; `quality.yml`'s `gitleaks`, `lizard`, and `jscpd` jobs no longer use `continue-on-error`; gitleaks gates PR/latest commits, lizard gates no increase above 31 warnings, and jscpd gates the existing 5% threshold. |
...
|---|---|---|---|---|
| 3.1 Cyclomatic complexity (lizard warnings) | +0 (31 warnings) | +0 (`>15`) | +3 (`1–5`) | +5 (`0`) |
