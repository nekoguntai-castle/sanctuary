...
| largest_file_lines | 1058 | `server/tests/unit/worker/electrumManager.test.ts` after the Trezor sign PSBT branch test split; next largest validated files are `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC each, followed by `server/tests/unit/api/push.test.ts` at 1052 LOC when generated verified-vector files are excluded. `tests/services/hardwareWallet/trezor.signPsbt.branches.test.ts` is now a 15-line registrar, with Trezor sign PSBT branch contract modules capped at 338 LOC. | 3.3 → 0 |
...
2. **Remaining oversized API/service modules** — the OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, policy repository, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, API client, OpenAPI admin path, OpenAPI admin schema, AppNotificationContext test, Payjoin service test, worker job queue test, transaction service batch test, advanced transaction test, auth middleware test, and Trezor sign PSBT branch test files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/worker/electrumManager.test.ts` at 1058 LOC, followed by `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC each when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Next validated targets: `server/tests/unit/worker/electrumManager.test.ts` (1058 LOC) and `tests/components/AISettings.test.tsx` / `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` (1056 LOC each), excluding generated verified-vector files.
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals do not move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized modules**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest scoped TS/TSX file is now `server/tests/unit/worker/electrumManager.test.ts` at 1058 LOC after the OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, policy repository, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, clientServerLimits WebSocket, API client, OpenAPI admin path, OpenAPI admin schema, AppNotificationContext test, Payjoin service test, worker job queue test, transaction service batch test, advanced transaction test, auth middleware test, and Trezor sign PSBT branch test splits.
