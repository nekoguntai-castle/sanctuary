...
| largest_file_lines | 1154 | `tests/contexts/AppNotificationContext.test.tsx` after the OpenAPI admin path split; next largest validated files are `server/tests/unit/services/payjoinService.test.ts` at 1115 LOC and `server/tests/unit/worker/workerJobQueue.test.ts` at 1098 LOC when generated verified-vector files are excluded. `server/src/api/openapi/paths/admin.ts` is now an 18-line aggregate, with OpenAPI admin path modules capped at 369 LOC. | 3.3 → 0 |
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, policy repository, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, and transfer service god-files are gone, but `tests/contexts/AppNotificationContext.test.tsx` is now the largest scoped TSX file at 1154 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `tests/contexts/AppNotificationContext.test.tsx` (1154 LOC), `server/tests/unit/services/payjoinService.test.ts` (1115 LOC), and `server/tests/unit/worker/workerJobQueue.test.ts` (1098 LOC), excluding generated verified-vector files.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, policy repository, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, clientServerLimits WebSocket, API client, and OpenAPI admin path splits is `tests/contexts/AppNotificationContext.test.tsx` at 1154 LOC, followed by `server/tests/unit/services/payjoinService.test.ts` at 1115 LOC and `server/tests/unit/worker/workerJobQueue.test.ts` at 1098 LOC when generated verified-vector files are excluded.
