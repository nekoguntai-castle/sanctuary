...
| largest_file_lines | 1941 | `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` after the transaction creation service contract split; next largest validated files include `server/tests/unit/services/syncService.test.ts` at 1872 LOC and `server/tests/unit/services/wallet.test.ts` at 1816 LOC when generated verified-vector files are excluded. `server/tests/unit/services/bitcoin/transactionService.create.test.ts` is now a 21-line registrar, with transaction creation contract modules capped at 553 LOC. | 3.3 → 0 |
...
| Largest file | Corrected after implementation: `server/tests/unit/api/openapi.test.ts` is now a 17-line suite registrar, with OpenAPI contract modules capped at 819 LOC. `server/tests/unit/api/transactions.test.ts` is now a 25-line suite registrar, with transaction API contract modules capped at 821 LOC. `server/tests/unit/api/admin.test.ts` is now a 26-line suite registrar, with admin API contract modules capped at 937 LOC. `server/tests/unit/api/auth.routes.registration.test.ts` is now a 21-line suite registrar, with auth registration contract modules capped at 534 LOC. `server/tests/unit/services/bitcoin/transactionService.create.test.ts` is now a 21-line suite registrar, with transaction creation contract modules capped at 553 LOC. `server/tests/unit/services/policyEvaluationEngine.test.ts` is now a 30-line registrar, with policy engine contract modules capped at 601 LOC. `server/tests/unit/services/bitcoin/blockchain.test.ts` is now a 24-line registrar, with blockchain contract modules capped at 493 LOC. `server/tests/unit/services/backupService.test.ts` is now a 29-line registrar, with backup service contract modules capped at 559 LOC. `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` is now a 43-line registrar, with process-transactions contract modules capped at 473 LOC. `server/tests/integration/flows/wallet.integration.test.ts` is now a 65-line registrar, with wallet integration contract modules capped at 604 LOC. The current largest non-generated TS/TSX file found in the scoped search is `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` at 1941 LOC. | API, policy-engine, blockchain service, backup service, process-transactions phase, wallet integration, auth registration, and transaction creation service splits are complete; the next largest-file criterion work is service/integration test decomposition or reframing the scoring criterion. |
...
2. **Remaining oversized service/integration tests** — the OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, and transaction creation service god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` at 1941 LOC, followed by `server/tests/unit/services/syncService.test.ts` at 1872 LOC and `server/tests/unit/services/wallet.test.ts` at 1816 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, and transaction creation service god-files are gone, but `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` is now the largest scoped TS/TSX file at 1941 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` (1941 LOC), `server/tests/unit/services/syncService.test.ts` (1872 LOC), and `server/tests/unit/services/wallet.test.ts` (1816 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + admin API + auth registration + transaction creation + policy engine + blockchain service + backup service + process-transactions + wallet integration splits) | 0 (`electrumPool.connections.test.ts` 1941 LOC) | 0 | +2 | +2 |
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, admin API, auth registration API, transaction creation service, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` at 1941 LOC, followed by `server/tests/unit/services/syncService.test.ts` at 1872 LOC and `server/tests/unit/services/wallet.test.ts` at 1816 LOC when generated verified-vector files are excluded.
