...
| Maintainability         | 8/15      | lizard baseline measured at 83 warnings; jscpd measured at 2.33%; OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, policy repository, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, clientServerLimits WebSocket, API client test, OpenAPI admin path module, OpenAPI admin schema module, AppNotificationContext test, Payjoin service test, worker job queue test, transaction service batch test, advanced transaction test, auth middleware test, Trezor sign PSBT branch test, and Electrum manager test splits are complete, but the largest non-generated TS/TSX files are now `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC each; clean architecture |
...
| largest_file_lines | 1056 | `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` after the Electrum manager test split; next largest validated file is `server/tests/unit/api/push.test.ts` at 1052 LOC when generated verified-vector files are excluded. `server/tests/unit/worker/electrumManager.test.ts` is now a 21-line registrar, with Electrum manager contract modules capped at 570 LOC. | 3.3 → 0 |
...
2. **Remaining oversized API/service modules** — the OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, policy repository, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, API client, OpenAPI admin path, OpenAPI admin schema, AppNotificationContext test, Payjoin service test, worker job queue test, transaction service batch test, advanced transaction test, auth middleware test, Trezor sign PSBT branch test, and Electrum manager test files are split, but the current largest non-generated TS/TSX files are now `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC each, followed by `server/tests/unit/api/push.test.ts` at 1052 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Next validated targets: `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` (1056 LOC each), followed by `server/tests/unit/api/push.test.ts` (1052 LOC), excluding generated verified-vector files.
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals do not move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized modules**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest scoped TS/TSX files are now `tests/components/AISettings.test.tsx` and `server/tests/unit/services/bitcoin/sync/confirmations.test.ts` at 1056 LOC each after the OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, transaction broadcast-and-save submodule, policy repository, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, clientServerLimits WebSocket, API client, OpenAPI admin path, OpenAPI admin schema, AppNotificationContext test, Payjoin service test, worker job queue test, transaction service batch test, advanced transaction test, auth middleware test, Trezor sign PSBT branch test, and Electrum manager test splits.
