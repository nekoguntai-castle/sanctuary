...
| largest_file_lines | 1428 | `server/tests/unit/services/bitcoin/psbtValidation.test.ts` after the Wallets API split; next largest validated files include `server/tests/unit/services/walletImport.imports.test.ts` at 1387 LOC and `server/tests/unit/services/transferService.test.ts` at 1347 LOC when generated verified-vector files are excluded. `server/tests/unit/api/wallets.test.ts` is now a 20-line registrar, with Wallets API contract modules capped at 322 LOC. | 3.3 → 0 |
...
2. **Remaining oversized API/service tests** — the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, and Wallets API god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/services/bitcoin/psbtValidation.test.ts` at 1428 LOC, followed by `server/tests/unit/services/walletImport.imports.test.ts` at 1387 LOC and `server/tests/unit/services/transferService.test.ts` at 1347 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, and Wallets API god-files are gone, but `server/tests/unit/services/bitcoin/psbtValidation.test.ts` is now the largest scoped TS/TSX file at 1428 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/services/bitcoin/psbtValidation.test.ts` (1428 LOC), `server/tests/unit/services/walletImport.imports.test.ts` (1387 LOC), and `server/tests/unit/services/transferService.test.ts` (1347 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + transaction HTTP route + admin API + auth registration + Bitcoin API + Draft API + Wallets API + transaction creation + transaction broadcast + Electrum connection + sync service + wallet service + WebSocket hook + device API + render-regression e2e + Intelligence tabs + policy engine + blockchain service + backup service + process-transactions + wallet integration splits) | 0 (`psbtValidation.test.ts` 1428 LOC) | 0 | +2 | +2 |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `server/tests/unit/services/bitcoin/psbtValidation.test.ts` at 1428 LOC after the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, Bitcoin API, Draft API, Wallets API, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, transaction HTTP route, admin API, auth registration API, Bitcoin API, Draft API, Wallets API, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/unit/services/bitcoin/psbtValidation.test.ts` at 1428 LOC, followed by `server/tests/unit/services/walletImport.imports.test.ts` at 1387 LOC and `server/tests/unit/services/transferService.test.ts` at 1347 LOC when generated verified-vector files are excluded.
