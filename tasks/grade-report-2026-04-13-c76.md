...
| Maintainability         | 8/15      | lizard baseline measured at 83 warnings; jscpd measured at 2.33%; OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, and blockchain aggregate service test splits are complete, but the largest non-generated TS/TSX file is still a 1228 LOC transaction-broadcast contract module; clean architecture |
...
| largest_file_lines | 1228 | `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` after the admin-routes HTTP split; next largest validated files are `server/tests/unit/repositories/policyRepository.test.ts` at 1210 LOC and `server/tests/unit/websocket/clientServerLimits.test.ts` at 1189 LOC when generated verified-vector files are excluded. `server/tests/unit/api/admin-routes.test.ts` is now a 26-line registrar, with Admin Routes HTTP contract modules capped at 313 LOC. | 3.3 → 0 |
...
2. **Remaining oversized API/service tests** — the OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, and transfer service god-files are split, but the current largest non-generated TS/TSX file is now the transaction broadcast-and-save contract module `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` at 1228 LOC, followed by `server/tests/unit/repositories/policyRepository.test.ts` at 1210 LOC and `server/tests/unit/websocket/clientServerLimits.test.ts` at 1189 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, and transfer service god-files are gone, but `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` is now the largest scoped TS/TSX file at 1228 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` (1228 LOC), `server/tests/unit/repositories/policyRepository.test.ts` (1210 LOC), and `server/tests/unit/websocket/clientServerLimits.test.ts` (1189 LOC), excluding generated verified-vector files.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, transaction HTTP route, admin API, admin-routes HTTP, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, admin integration, Docker utility, analysis service, descriptor parser, blockchain aggregate service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/unit/services/bitcoin/transactionServiceBroadcast/transactionServiceBroadcast.broadcastAndSave.contracts.ts` at 1228 LOC, followed by `server/tests/unit/repositories/policyRepository.test.ts` at 1210 LOC and `server/tests/unit/websocket/clientServerLimits.test.ts` at 1189 LOC when generated verified-vector files are excluded.
