...
| largest_file_lines | 1494 | `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` after the Intelligence tabs split; next largest validated files include `server/tests/unit/api/bitcoin.test.ts` at 1480 LOC and `server/tests/unit/api/drafts.test.ts` at 1466 LOC when generated verified-vector files are excluded. `tests/components/Intelligence.tabs.test.tsx` is now a 12-line registrar, with Intelligence tabs contract modules capped at 445 LOC. | 3.3 → 0 |
...
2. **Remaining oversized service/API tests** — the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, and Intelligence tabs god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` at 1494 LOC, followed by `server/tests/unit/api/bitcoin.test.ts` at 1480 LOC and `server/tests/unit/api/drafts.test.ts` at 1466 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, and Intelligence tabs god-files are gone, but `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` is now the largest scoped TS/TSX file at 1494 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` (1494 LOC), `server/tests/unit/api/bitcoin.test.ts` (1480 LOC), and `server/tests/unit/api/drafts.test.ts` (1466 LOC), excluding generated verified-vector files.
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` at 1494 LOC after the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
