...
| Maintainability         | 8/15      | lizard baseline measured at 83 warnings; jscpd measured at 2.33%; OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, and WebSocket hook test splits are complete, but the largest non-generated TS/TSX file is still a 1702 LOC API test; clean architecture |
...
| largest_file_lines | 1702 | `server/tests/unit/api/devices.test.ts` after the WebSocket hook contract split; next largest validated files include `e2e/render-regression.spec.ts` at 1608 LOC and `server/tests/unit/api/transactions-http-routes.test.ts` at 1548 LOC when generated verified-vector files are excluded. `tests/hooks/useWebSocket.test.tsx` is now a 20-line registrar, with WebSocket hook contract modules capped at 437 LOC. | 3.3 → 0 |
...
2. **Remaining oversized API/e2e/frontend tests** — the OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, and WebSocket hook god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/api/devices.test.ts` at 1702 LOC, followed by `e2e/render-regression.spec.ts` at 1608 LOC and `server/tests/unit/api/transactions-http-routes.test.ts` at 1548 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, and WebSocket hook god-files are gone, but `server/tests/unit/api/devices.test.ts` is now the largest scoped TS/TSX file at 1702 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/api/devices.test.ts` (1702 LOC), `e2e/render-regression.spec.ts` (1608 LOC), and `server/tests/unit/api/transactions-http-routes.test.ts` (1548 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + admin API + auth registration + transaction creation + Electrum connection + sync service + wallet service + WebSocket hook + policy engine + blockchain service + backup service + process-transactions + wallet integration splits) | 0 (`devices.test.ts` 1702 LOC) | 0 | +2 | +2 |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `server/tests/unit/api/devices.test.ts` at 1702 LOC after the OpenAPI, transaction API, admin API, auth registration API, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, admin API, auth registration API, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/unit/api/devices.test.ts` at 1702 LOC, followed by `e2e/render-regression.spec.ts` at 1608 LOC and `server/tests/unit/api/transactions-http-routes.test.ts` at 1548 LOC when generated verified-vector files are excluded.
