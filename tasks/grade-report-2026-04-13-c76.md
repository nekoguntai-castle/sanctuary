...
- **[4.3] Input validation quality — Medium → +1**: Zod-backed `validate({body: …})` middleware used on more routes after the implementation passes (address generation, AI model pull/delete, AI suggest/query, device account creation, device sharing, wallet sharing, and wallet-device linking); other mutation routes still rely on inline body checks, e.g. `server/src/api/wallets/policies.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, and Bitcoin/label mutation routes. ISO Integrity.
...
| Input validation | Valid mixed state: `server/src/api/transactions/addresses.ts` uses Zod `validate({ body: GenerateAddressesBodySchema })`; `server/src/api/ai/models.ts`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, `server/src/api/devices/sharing.ts`, `server/src/api/wallets/sharing.ts`, and `server/src/api/wallets/devices.ts` have now been migrated during implementation. Remaining inline body-check examples include `server/src/api/wallets/policies.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, and Bitcoin/label mutation routes. | P1: continue normalizing mutation request bodies onto Zod. |
...
3. **Inconsistent input validation at mutation boundaries** — `server/src/api/ai/models.ts`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, `server/src/api/devices/sharing.ts`, `server/src/api/wallets/sharing.ts`, and `server/src/api/wallets/devices.ts` are now on the preferred Zod `validate({ body: ... })` pattern, but other handlers still read `req.body` and validate inline, e.g. `server/src/api/wallets/policies.ts`, `server/src/api/wallets/crud.ts`, and Bitcoin/label mutation routes. A handler missing validation is a latent CWE-20.
...
- `server/src/api/ai/models.ts` `POST /pull-model` and `DELETE /delete-model`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, `server/src/api/devices/sharing.ts`, `server/src/api/wallets/sharing.ts`, and `server/src/api/wallets/devices.ts` have been migrated during implementation.
- Continue with remaining inline body-check offenders such as `server/src/api/wallets/policies.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, `server/src/api/bitcoin/*.ts`, and `server/src/api/labels.ts`, then sweep any other `if (!foo) return 400` patterns in mutation handlers.
...
| P1 | Continue normalizing request-body validation onto Zod for mutation endpoints, starting with small inline checks. | `server/src/api/transactions/addresses.ts` uses `validate({ body: GenerateAddressesBodySchema })`; `server/src/api/schemas/transactions.ts` caps `count` at 1000. `server/src/api/ai/models.ts`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, `server/src/api/devices/sharing.ts`, `server/src/api/wallets/sharing.ts`, and `server/src/api/wallets/devices.ts` have now been migrated; remaining examples include `server/src/api/wallets/policies.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, and Bitcoin/label mutation routes. | Use the address-generation schema pattern as the template. |
...
- Remaining sweep candidates still include `server/src/api/wallets/policies.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, Bitcoin mutation routes, and label mutation routes.
