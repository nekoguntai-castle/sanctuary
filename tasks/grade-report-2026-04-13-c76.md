...
| Maintainability         | 10/15     | lizard baseline measured at 74 warnings after the syncAddress, Payjoin SSRF, device account normalization, policy create-data, draft create-data, intelligence settings, Telegram collector, and wallet autopilot settings extraction passes; jscpd measured at 2.33%; the scoped largest-file threshold is now cleared after the Electrum connection test split; clean architecture |
...
| lizard_warning_count | 74 | lizard 1.21.3 temporary `/tmp` install, CI command with current exclusions; `server/src/services/bitcoin/blockchain/syncAddress.ts`, `server/src/services/payjoin/ssrf.ts`, `server/src/services/deviceAccountConflicts.ts`, `server/src/repositories/policyRepository.ts`, `server/src/repositories/draftRepository.ts`, `server/src/services/intelligence/settings.ts`, `server/src/services/supportPackage/collectors/telegram.ts`, and `server/src/api/wallets/autopilot.ts` now have no lizard warnings after helper extraction | 3.1 → +0 measured; enforced as no-increase baseline |
...
- `lizard` — not installed globally, but a temporary `/tmp` install measured 74 warnings after the syncAddress, Payjoin SSRF, device account normalization, policy create-data, draft create-data, intelligence settings, Telegram collector, and wallet autopilot settings extraction passes. `.github/workflows/quality.yml` is now blocking with `-i 74`, so the warning count cannot grow without failing CI.
...
1. **CI quality signals are now blocking, but lizard is baseline-gated.** The new lint, gitleaks, lizard, and jscpd jobs are blocking in `.github/workflows/quality.yml`. `lizard` still has 74 existing warnings, so the immediate guardrail is "do not increase warning count"; further baseline reduction remains future maintainability work.
...
- Baseline the current lizard/jscpd output, document any intentional deltas, then remove `continue-on-error: true` from each job in `quality.yml` **per-job, not as a batch**. Implemented with jscpd at 2.33%, lizard now at a 74-warning no-increase baseline after the syncAddress, Payjoin SSRF, device account normalization, policy create-data, draft create-data, intelligence settings, Telegram collector, and wallet autopilot settings extraction passes, and gitleaks scoped to PR/latest-commit regression scanning because full-history/current-directory scans include legacy/test/ignored-file false positives.
...
| CI signals enforceable | Done for regression gating: `scripts/quality.sh` runs all three tools; `quality.yml`'s `gitleaks`, `lizard`, and `jscpd` jobs no longer use `continue-on-error`; gitleaks gates PR/latest commits, lizard gates no increase above 74 warnings, and jscpd gates the existing 5% threshold. |
...
|---|---|---|---|---|
| 3.1 Cyclomatic complexity (lizard warnings) | +0 (74 warnings) | +0 (`>15`) | +3 (`1–5`) | +5 (`0`) |
...
- Reduced the CI-style lizard count from 75 to 74 warnings under the current exclusions, so `.github/workflows/quality.yml` and `scripts/quality.sh` now gate at `-i 74`.
