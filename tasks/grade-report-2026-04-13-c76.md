...
| largest_file_lines | 1816 | `server/tests/unit/services/wallet.test.ts` after the sync service contract split; next largest validated files include `tests/hooks/useWebSocket.test.tsx` at 1752 LOC and `server/tests/unit/api/devices.test.ts` at 1702 LOC when generated verified-vector files are excluded. `server/tests/unit/services/syncService.test.ts` is now a 29-line registrar, with sync service contract modules capped at 471 LOC. | 3.3 → 0 |
...
2. **Remaining oversized service/integration/frontend tests** — the OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, and sync service god-files are split, but the current largest non-generated TS/TSX file is now `server/tests/unit/services/wallet.test.ts` at 1816 LOC, followed by `tests/hooks/useWebSocket.test.tsx` at 1752 LOC and `server/tests/unit/api/devices.test.ts` at 1702 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, and sync service god-files are gone, but `server/tests/unit/services/wallet.test.ts` is now the largest scoped TS/TSX file at 1816 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/unit/services/wallet.test.ts` (1816 LOC), `tests/hooks/useWebSocket.test.tsx` (1752 LOC), and `server/tests/unit/api/devices.test.ts` (1702 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + admin API + auth registration + transaction creation + Electrum connection + sync service + policy engine + blockchain service + backup service + process-transactions + wallet integration splits) | 0 (`wallet.test.ts` 1816 LOC) | 0 | +2 | +2 |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `server/tests/unit/services/wallet.test.ts` at 1816 LOC after the OpenAPI, transaction API, admin API, auth registration API, transaction creation service, Electrum connection, sync service, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, admin API, auth registration API, transaction creation service, Electrum connection, sync service, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/unit/services/wallet.test.ts` at 1816 LOC, followed by `tests/hooks/useWebSocket.test.tsx` at 1752 LOC and `server/tests/unit/api/devices.test.ts` at 1702 LOC when generated verified-vector files are excluded.
