...
`gitleaks` is not installed globally in this environment, but the pinned workflow version, gitleaks `v8.30.1`, was downloaded to `/tmp` for verification. Latest-commit, tracked-tree, and full-history scans now pass with `.gitleaks.toml` fixture allowlists and a `.gitleaksignore` limited to four historical ignored-artifact fingerprints.
...
- **Maintainability +3 after continued lizard reduction** (10→13): the paper airplanes extraction pass lowered the lizard baseline from 6 to 4 warnings, moving criterion 3.1 from `+0` to `+3`.
- **Maintainability baseline-only improvement after lavender extraction**: the lavender fields pass lowered the lizard baseline from 4 to 3 warnings; no additional numeric score movement occurs until the baseline reaches zero.
- **Maintainability baseline-only improvement after butterfly garden extraction**: the butterfly garden pass lowered the lizard baseline from 3 to 2 warnings; no additional numeric score movement occurs until the baseline reaches zero.
...
- `lizard` — not installed globally, but a temporary `/tmp` install measured 0 warnings after the continued extraction passes through the WalletDetail filter hook split. `.github/workflows/quality.yml` is now blocking with `-i 0`, so any new warning fails CI.
...
- **No longer missing: full-history/tracked-tree gitleaks signal** — `gitleaks` is not installed globally, but the pinned workflow binary was used to verify latest-commit, tracked-tree, and full-history scans. Raw `gitleaks detect --no-git` over every local file still finds ignored local `.env` and generated SSL artifacts; that all-files scan is intentionally not the quality signal because those paths are excluded from the tracked-tree scan and remain ignored by Git.
...
1. **CI quality signals are now blocking and the local static-analysis backlog is clean.** The lint, gitleaks, lizard, and jscpd jobs are blocking in `.github/workflows/quality.yml`. `lizard` is gated at zero warnings, gitleaks gates PR/latest commits plus the tracked tree, and full-history gitleaks now passes with narrow fixture and historical-fingerprint allowlists.
...
- Baseline the current lizard/jscpd output, document any intentional deltas, then remove `continue-on-error: true` from each job in `quality.yml` **per-job, not as a batch**. Implemented with jscpd at 2.33%, lizard now at a zero-warning baseline after the continued extraction passes through the WalletDetail filter hook split, and gitleaks now clean for latest-commit, tracked-tree, and full-history scans.
- Score impact: net **+7 points** in this continued pass. jscpd remains at 3.2 `+3`, the measured lizard baseline moved 3.1 from `+0 → +5` after reaching zero warnings, and Security 4.2 now claims the full clean secret-scan signal.
...
- Evidence produced 2026-04-15 UTC (proof artifacts removed from repo; available in git history).
...
| CI signals enforceable | Done for regression gating: `scripts/quality.sh` runs all three tools; `quality.yml`'s `gitleaks`, `lizard`, and `jscpd` jobs no longer use `continue-on-error`; gitleaks gates PR/latest commits and the tracked tree, lizard gates zero warnings, and jscpd gates the existing 5% threshold. |
...
|---|---|---|---|---|
| 3.1 Cyclomatic complexity (lizard warnings) | +5 (0 warnings) | +5 (`0`) | +5 (`0`) | +5 (`0`) |
...
- Replaced the advisory gitleaks action step with a blocking gitleaks `v8.30.1` CLI job that scans the PR commit range; scheduled/manual runs scan the latest commit. At this checkpoint, the latest commit was clean while full-history/current-directory scans still contained legacy/test/ignored-file false positives; those were resolved in the later gitleaks cleanup pass.
...
- `/tmp/sanctuary-quality/gitleaks detect --source . --config .gitleaks.toml --redact --no-banner` — failed with 36 findings at this checkpoint; the later gitleaks cleanup pass replaces this raw all-file current-directory signal with a tracked-tree scan and clears full-history findings.
...
### Lizard accessibility API harness extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `e2e/accessibility.spec.ts` by moving the mocked accessibility API response surface into a static response table plus small route parsing, response lookup, and route handler helpers.
- Preserved the authenticated accessibility behavior for dashboard, settings, wallet detail, create-wallet, device, price, Bitcoin status, AI/intelligence status, labels, and unhandled-route tracking.
- Removed the prior lizard warning for `apiRouteHandler` without introducing a new warning in the spec.
- Reduced the CI-style lizard count from 19 to 18 warnings under the current exclusions.
- Maintainability 3.1 still scores `+0` because 18 warnings remains above the `>15` threshold; this is another verified baseline reduction but not a score-band movement.

Verification after lizard accessibility API harness extraction:

- `npx playwright test e2e/accessibility.spec.ts --project=chromium` — passed: 12 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 e2e/accessibility.spec.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 18 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `18`.

### Lizard dashboard price/block API harness extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `e2e/dashboard-price-blocks.spec.ts` by moving the mocked dashboard API response surface into static route tables plus small scenario, price-response, mempool-response, response lookup, and route handler helpers.
- Preserved dynamic price/change and mempool block behavior for positive, negative, zero, and null 24h change cases plus block visualizer tooltip coverage.
- Removed the prior lizard warning for `apiRouteHandler` without introducing a new warning in the spec.
- Reduced the CI-style lizard count from 18 to 17 warnings under the current exclusions.
- Maintainability 3.1 still scores `+0` because 17 warnings remains above the `>15` threshold; this is another verified baseline reduction but not a score-band movement.

Verification after lizard dashboard price/block API harness extraction:

- `npx playwright test e2e/dashboard-price-blocks.spec.ts --project=chromium` — passed: 8 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 e2e/dashboard-price-blocks.spec.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 17 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `17`.

### Lizard admin operations API harness extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `e2e/admin-operations.spec.ts` by moving the large mutable mocked API surface into static route responses plus small domain responders for registration status, feature flags, users, groups, settings, node config, backup, and injected failures.
- Preserved the mutable admin behavior for feature flag updates, user/group create-update-delete flows, variable saves, node-config saves, backup/encryption-key endpoints, and unhandled-route tracking.
- Removed the prior lizard warning for `apiRouteHandler` without introducing a new warning in the spec.
- Reduced the CI-style lizard count from 17 to 16 warnings under the current exclusions.
- Maintainability 3.1 still scores `+0` because 16 warnings remains above the `>15` threshold; this is another verified baseline reduction but not a score-band movement.

Verification after lizard admin operations API harness extraction:

- `npx playwright test e2e/admin-operations.spec.ts --project=chromium` — passed: 19 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 e2e/admin-operations.spec.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 16 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `16`.

### Lizard import wallet API harness extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `e2e/import-wallet-flow.spec.ts` by moving the mocked import-wallet API response surface into static route responses plus small failure, validation, import mutation, wallet-detail, labels, and route handler helpers.
- Preserved dynamic validation-response overrides, import error injection, imported wallet detail routes, AI/intelligence status routes, and unhandled-route tracking.
- Removed the prior lizard warning for `apiRouteHandler` without introducing a new warning in the spec.
- Reduced the CI-style lizard count from 16 to 15 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 15 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard import wallet API harness extraction:

- `npx playwright test e2e/import-wallet-flow.spec.ts --project=chromium` — passed: 13 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 e2e/import-wallet-flow.spec.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 15 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `15`.

### Lizard settings persistence API harness extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `e2e/settings-persistence.spec.ts` by moving fixed mocked API responses into a route table and isolating the mutable preference state in a small preference-response helper.
- Preserved preference-update recording, preference state merging, auth/me preference hydration, and unhandled-route tracking.
- Removed the prior lizard warning for `apiRouteHandler` without introducing a new warning in the spec.
- Reduced the CI-style lizard count from 15 to 14 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 14 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard settings persistence API harness extraction:

- `npx playwright test e2e/settings-persistence.spec.ts --project=chromium` — passed: 9 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 e2e/settings-persistence.spec.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 14 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `14`.

### Lizard create wallet API harness extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `e2e/create-wallet-flow.spec.ts` by moving fixed mocked API responses into a route table and isolating dynamic device-list, wallet-creation, wallet-detail, labels, and failure injection behavior in focused helpers.
- Preserved configurable device lists, wallet creation errors, generated wallet response payloads, created-wallet detail routes, and unhandled-route tracking.
- Removed the prior lizard warning for `apiRouteHandler` without introducing a new warning in the spec.
- Reduced the CI-style lizard count from 14 to 13 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 13 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard create wallet API harness extraction:

- `npx playwright test e2e/create-wallet-flow.spec.ts --project=chromium` — passed: 14 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 e2e/create-wallet-flow.spec.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 13 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `13`.

### Lizard paper boats animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/paperBoats.ts` by splitting the large `animate` frame loop into focused helpers for sky/water gradients, bank tracing, bank edges, water base, caustics, water-flow lines, shimmer, ripples, boat movement, grass tufts, and frame orchestration.
- Preserved the existing boat movement, ripple generation, bank rendering, caustic drift, shimmer, and grass behavior while keeping the animation loop as the scheduler.
- Removed the prior lizard warning for `animate` without introducing a new warning in the file.
- Reduced the CI-style lizard count from 13 to 12 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 12 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard paper boats animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/paperBoats.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 12 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `12`.

### Lizard bunny meadow animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/bunnyMeadow.ts` by splitting `drawBunny` into focused palette, shadow, tail, body, spot, paw, head, ear, snout, mouth, and eye helpers.
- Refactored `updateBunny` into focused blink, hop-start, sitting, hopping, eating, alert, and state-dispatch helpers while preserving the existing timers and random state transitions.
- Removed the prior lizard warnings for `drawBunny` and `updateBunny` without introducing a new warning in the file.
- Reduced the CI-style lizard count from 12 to 10 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 10 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard bunny meadow animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/bunnyMeadow.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 10 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `10`.

### Lizard train station animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/trainStation.ts` by splitting the `animate` frame loop into train-state, platform-spawn, passenger-state, screen-wrapping, and draw orchestration helpers.
- Preserved the existing train arrival/stop/departure transitions, passenger boarding/exiting/walking probabilities, platform train spawning behavior, and frame scheduling.
- Removed the prior lizard warning for `animate` without introducing a new warning in the file.
- Reduced the CI-style lizard count from 10 to 9 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 9 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard train station animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/trainStation.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 9 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `9`.

### Lizard fireworks animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/fireworks.ts` by splitting `explodeRocket` into particle-count, pattern-builder, center-flash, crossette mini-burst, and particle-push helpers.
- Preserved the existing burst pattern range, particle counts, color choices, flash counts, crossette delay, and rocket-to-particle conversion behavior.
- Removed the prior lizard warning for `explodeRocket` without introducing a new warning in the file.
- Reduced the CI-style lizard count from 9 to 8 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 8 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard fireworks animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/fireworks.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 8 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `8`.

### Lizard duckling parade animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/ducklingParade.ts` by splitting `drawDuck` into palette, shadow, tail, body, wing, neck, head, cheek, eye, beak, and feet helpers.
- Refactored `updateDucks` into focused timer, leader reset, duckling follow, wing-flap, and ripple helpers while preserving family movement and timer behavior.
- Removed the prior lizard warnings for `drawDuck` and `updateDucks` without introducing a new warning in the file.
- Reduced the CI-style lizard count from 8 to 6 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+0` in this report because 6 warnings still has not reached the scored `1-5` warning band; this is another verified baseline reduction but not a score-band movement.

Verification after lizard duckling parade animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/ducklingParade.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 6 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `6`.

### Lizard paper airplanes animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/paperAirplanes.ts` by splitting `drawPaperAirplane` into focused dart-path, shadow, gradient, body, fold, crease, close-detail, and shape helpers.
- Refactored the large airplane frame callback into focused turn-decision, energy, gravity, stall, depth, motion, boundary, trail, depth-draw, and reset helpers while preserving the existing flight behavior.
- Removed the prior lizard warnings for `drawPaperAirplane` and the airplane frame callback without introducing a new warning in the file.
- Reduced the CI-style lizard count from 6 to 4 warnings under the current exclusions.
- Maintainability 3.1 moves from `+0` to `+3` because 4 warnings reaches the scored `1-5` warning band; the implementation-adjusted score moves from 82/100 to 85/100.

Verification after lizard paper airplanes animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/paperAirplanes.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 4 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `4`.

### Lizard lavender fields animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/lavenderFields.ts` by splitting the butterfly frame callback into focused rest, retarget, arrival, speed, body-angle, flight, bounds, wing-angle, wing drawing, body, antenna, and shape helpers.
- Preserved the existing rest/retarget timing, same-frame movement behavior after target arrival, wing flapping, body angle easing, bounds, and drawing geometry.
- Removed the prior lizard warning for the butterfly frame callback without introducing a new warning in the file.
- Reduced the CI-style lizard count from 4 to 3 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+3` because 3 warnings stays within the scored `1-5` warning band; the next numeric movement requires reducing the baseline to zero.

Verification after lizard lavender fields animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/lavenderFields.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 3 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `3`.

### Lizard butterfly garden animation extraction pass — 2026-04-14

Implemented the next measured lizard-baseline reduction:

- Refactored `components/animations/butterflyGarden.ts` by splitting `drawButterfly` into focused target-clamp, target-shift, arrival, wander, speed, body-angle, rest/flying update, wing-pose, palette, pattern, forewing, hindwing, body, antenna, and shape helpers.
- Preserved the existing rest timing, target retargeting, same-frame acceleration after target arrival, wander oscillation, wing foreshortening, pattern drawing, antenna curl, and drawing order.
- Removed the prior lizard warning for `drawButterfly` without introducing a new warning in the file.
- Reduced the CI-style lizard count from 3 to 2 warnings under the current exclusions.
- Maintainability 3.1 remains scored `+3` because 2 warnings stays within the scored `1-5` warning band; the next numeric movement requires reducing the baseline to zero.

Verification after lizard butterfly garden animation extraction:

- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/animations/butterflyGarden.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 2 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | rg -c "warning:"` — returned `2`.

### Lizard WalletDetail filter hooks extraction pass — 2026-04-14

Implemented the final measured lizard-baseline reduction:

- Refactored `components/WalletDetail/hooks/useTransactionFilters.ts` by splitting manual transaction type, date, confirmation, label, and aggregate matching into focused helpers while preserving the existing memoized filter pipeline.
- Refactored `components/WalletDetail/hooks/useAITransactionFilter.ts` by splitting AI transaction type, label, amount, confirmation, sort, limit, and aggregation behavior into focused helpers while preserving the existing query result semantics.
- Removed the prior lizard warnings for the manual transaction filter callback and the AI transaction filter callback without introducing new warnings in either hook.
- Reduced the CI-style lizard count from 2 to 0 warnings under the current exclusions.
- Maintainability 3.1 moves from `+3` to `+5` because the warning baseline is now zero; the implementation-adjusted score moves from 85/100 to 87/100.

Verification after lizard WalletDetail filter hooks extraction:

- `npx vitest run tests/components/WalletDetail/hooks/useTransactionFilters.test.ts tests/components/WalletDetail/hooks/useAITransactionFilter.test.ts` — passed: 2 files, 63 tests.
- `npm run typecheck:app` — passed.
- `npm run lint` — passed, including `scripts/check-api-body-validation.mjs` under `lint:server`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -l typescript -C 15 -T nloc=200 components/WalletDetail/hooks/useTransactionFilters.ts components/WalletDetail/hooks/useAITransactionFilter.ts` — passed with no warnings.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 0 ... .` — passed under the same exclusions used by `.github/workflows/quality.yml`.
- `PYTHONPATH=/tmp/sanctuary-quality/python python3 -m lizard -w -i 999 ... . | awk '/warning:/{count++} END{print count + 0}'` — returned `0`.

### Gitleaks false-positive cleanup pass — 2026-04-14

Implemented the remaining local secret-scan cleanup:

- Added explicit `.gitleaks.toml` allowlists for inspected fixture/test placeholder paths, including regtest Bitcoin RPC workflow credentials, canonical BIP340 vectors, backup/encryption-key UI test placeholders, auth test placeholders, redaction test examples, and render-regression/admin-operation E2E fixtures.
- Added a narrow `.gitleaksignore` containing only four historical ignored-artifact fingerprints from `.claude/settings.local.json.old` and `extension/dist/background/service-worker.js`.
- Added `scripts/gitleaks-tracked-tree.sh`, which copies only Git-tracked files into a temporary tree before scanning so ignored local `.env`, coverage, dist, and generated SSL artifacts are not part of the current-tree quality signal.
- Updated `scripts/quality.sh` and `.github/workflows/quality.yml` so gitleaks covers both the PR/latest-commit gate and the tracked tree.
- Security 4.2 moves from the new-commit-only `+2` signal to the full `+4` measured signal; the implementation-adjusted score moves from 87/100 to 89/100.

Verification after gitleaks false-positive cleanup:

- `/tmp/gitleaks git . --config .gitleaks.toml --redact --no-banner --log-opts -1` — passed: 1 commit scanned, no leaks found.
...
- Updated `docs/plans/dependency-audit-triage.md` and this report so routine dependency maintenance is marked complete.
...
- Generated evidence produced 2026-04-15 UTC (proof artifacts removed from repo; available in git history).
- Production-like runtime evidence produced 2026-04-15 UTC (proof artifact removed from repo; available in git history).
