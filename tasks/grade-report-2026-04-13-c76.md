...
| Maintainability         | 8/15      | lizard baseline measured at 83 warnings; jscpd measured at 2.33%; OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, and render-regression e2e test splits are complete, but the largest non-generated TS/TSX file is still a 1507 LOC frontend component test; clean architecture |
...
| largest_file_lines | 1507 | `tests/components/Intelligence.tabs.test.tsx` after the transaction HTTP route split; next largest validated files include `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` at 1494 LOC and `server/tests/unit/api/bitcoin.test.ts` at 1480 LOC when generated verified-vector files are excluded. `server/tests/unit/api/transactions-http-routes.test.ts` is now a 13-line registrar, with transaction HTTP route contract modules capped at 677 LOC. | 3.3 → 0 |
...
| Largest file | Corrected after implementation: `server/tests/unit/api/openapi.test.ts` is now a 17-line suite registrar, with OpenAPI contract modules capped at 819 LOC. `server/tests/unit/api/transactions.test.ts` is now a 25-line suite registrar, with transaction API contract modules capped at 821 LOC. `server/tests/unit/api/transactions-http-routes.test.ts` is now a 13-line suite registrar, with transaction HTTP route modules capped at 677 LOC. `server/tests/unit/api/admin.test.ts` is now a 26-line suite registrar, with admin API contract modules capped at 937 LOC. `server/tests/unit/api/auth.routes.registration.test.ts` is now a 21-line suite registrar, with auth registration contract modules capped at 534 LOC. `server/tests/unit/services/bitcoin/transactionService.create.test.ts` is now a 21-line suite registrar, with transaction creation contract modules capped at 553 LOC. `server/tests/unit/services/bitcoin/electrumPool.connections.test.ts` is now a 22-line registrar, with Electrum connection contract modules capped at 652 LOC. `server/tests/unit/services/syncService.test.ts` is now a 29-line registrar, with sync service contract modules capped at 471 LOC. `server/tests/unit/services/wallet.test.ts` is now a 22-line registrar, with wallet service contract modules capped at 648 LOC. `tests/hooks/useWebSocket.test.tsx` is now a 20-line registrar, with WebSocket hook contract modules capped at 437 LOC. `server/tests/unit/api/devices.test.ts` is now a 37-line registrar, with device API contract modules capped at 657 LOC. `e2e/render-regression.spec.ts` is now a 54-line registrar, with render-regression e2e modules capped at 767 LOC. `server/tests/unit/services/policyEvaluationEngine.test.ts` is now a 30-line registrar, with policy engine contract modules capped at 601 LOC. `server/tests/unit/services/bitcoin/blockchain.test.ts` is now a 24-line registrar, with blockchain contract modules capped at 493 LOC. `server/tests/unit/services/backupService.test.ts` is now a 29-line registrar, with backup service contract modules capped at 559 LOC. `server/tests/unit/services/bitcoin/sync/phases.processTransactions.test.ts` is now a 43-line registrar, with process-transactions contract modules capped at 473 LOC. `server/tests/integration/flows/wallet.integration.test.ts` is now a 65-line registrar, with wallet integration contract modules capped at 604 LOC. The current largest non-generated TS/TSX file found in the scoped search is `tests/components/Intelligence.tabs.test.tsx` at 1507 LOC. | API, policy-engine, blockchain service, backup service, process-transactions phase, wallet integration, auth registration, transaction creation service, transaction HTTP route, Electrum connection, sync service, wallet service, WebSocket hook, device API, and render-regression e2e splits are complete; the next largest-file criterion work is frontend/service/API test decomposition or reframing the scoring criterion. |
...
2. **Remaining oversized frontend/service/API tests** — the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, and render-regression e2e god-files are split, but the current largest non-generated TS/TSX file is now `tests/components/Intelligence.tabs.test.tsx` at 1507 LOC, followed by `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` at 1494 LOC and `server/tests/unit/api/bitcoin.test.ts` at 1480 LOC when generated verified-vector files are excluded. This keeps criterion 3.3 at 0 until those files are split or the scoring threshold is reframed.
...
- Next validated targets: `tests/components/Intelligence.tabs.test.tsx` (1507 LOC), `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` (1494 LOC), and `server/tests/unit/api/bitcoin.test.ts` (1480 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + transaction HTTP route + admin API + auth registration + transaction creation + Electrum connection + sync service + wallet service + WebSocket hook + device API + render-regression e2e + policy engine + blockchain service + backup service + process-transactions + wallet integration splits) | 0 (`Intelligence.tabs.test.tsx` 1507 LOC) | 0 | +2 | +2 |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `tests/components/Intelligence.tabs.test.tsx` at 1507 LOC after the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, transaction HTTP route, admin API, auth registration API, transaction creation service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `tests/components/Intelligence.tabs.test.tsx` at 1507 LOC, followed by `server/tests/unit/services/bitcoin/transactionService.broadcast.test.ts` at 1494 LOC and `server/tests/unit/api/bitcoin.test.ts` at 1480 LOC when generated verified-vector files are excluded.
