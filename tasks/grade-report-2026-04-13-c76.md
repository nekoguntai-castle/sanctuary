...
| Input validation | Valid mixed state: `server/src/api/transactions/addresses.ts` uses Zod `validate({ body: GenerateAddressesBodySchema })`; `server/src/api/ai/models.ts`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, `server/src/api/devices/sharing.ts`, `server/src/api/wallets/sharing.ts`, `server/src/api/wallets/devices.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, `server/src/api/wallets/policies.ts`, `server/src/api/wallets/autopilot.ts`, `server/src/api/wallets/telegram.ts`, `server/src/api/wallets/xpubValidation.ts`, and `server/src/api/wallets/approvals.ts` have now been migrated during implementation. Remaining validated examples include Bitcoin mutation routes and label mutation routes; a post-slice `rg -n "req\\.body" server/src/api -g '*.ts'` sweep also shows other request-body readers that should be triaged as parser-backed exceptions or future Zod targets. | P1: continue normalizing mutation request bodies onto Zod. |
...
3. **Inconsistent input validation at mutation boundaries** — the AI/device/wallet sharing/wallet CRUD/wallet import/wallet policy/wallet settings/wallet approval slices are now on the preferred Zod `validate({ body: ... })` pattern, but other handlers still read `req.body` and validate inline, with validated remaining examples in Bitcoin mutation routes and label mutation routes. A handler missing validation is a latent CWE-20.
...
- `server/src/api/ai/models.ts` `POST /pull-model` and `DELETE /delete-model`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, device/wallet sharing, wallet-device linking, wallet CRUD/import/policy routes, wallet autopilot/Telegram settings, XPUB validation, and wallet approvals have been migrated during implementation.
- Continue with remaining inline body-check offenders such as `server/src/api/bitcoin/*.ts` and `server/src/api/labels.ts`, then sweep any other `if (!foo) return 400` patterns in mutation handlers and document parser-backed exceptions.
...
- Remaining validated sweep candidates now include Bitcoin mutation routes and label mutation routes. A post-slice `rg -n "req\\.body" server/src/api -g '*.ts'` sweep also shows other request-body readers; triage those as parser-backed exceptions or future Zod targets before claiming the full Security 4.3 sweep complete.
