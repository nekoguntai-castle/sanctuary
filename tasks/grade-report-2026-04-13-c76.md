...
| Input validation | Valid mixed state: `server/src/api/transactions/addresses.ts` uses Zod `validate({ body: GenerateAddressesBodySchema })`; `server/src/api/ai/models.ts`, `server/src/api/ai/features.ts`, `server/src/api/devices/accounts.ts`, `server/src/api/devices/sharing.ts`, `server/src/api/wallets/sharing.ts`, `server/src/api/wallets/devices.ts`, `server/src/api/wallets/crud.ts`, `server/src/api/wallets/import.ts`, and `server/src/api/wallets/policies.ts` have now been migrated during implementation. Remaining inline body-check examples include `server/src/api/wallets/autopilot.ts`, `server/src/api/wallets/telegram.ts`, `server/src/api/wallets/xpubValidation.ts`, `server/src/api/wallets/approvals.ts`, Bitcoin mutation routes, and label mutation routes. | P1: continue normalizing mutation request bodies onto Zod. |
...
3. **Inconsistent input validation at mutation boundaries** — the AI/device/wallet sharing/wallet CRUD/wallet import/wallet policy slices are now on the preferred Zod `validate({ body: ... })` pattern, but other handlers still read `req.body` and validate inline, e.g. `server/src/api/wallets/autopilot.ts`, `server/src/api/wallets/telegram.ts`, `server/src/api/wallets/xpubValidation.ts`, `server/src/api/wallets/approvals.ts`, Bitcoin mutation routes, and label mutation routes. A handler missing validation is a latent CWE-20.
...
- Continue with remaining inline body-check offenders such as `server/src/api/wallets/autopilot.ts`, `server/src/api/wallets/telegram.ts`, `server/src/api/wallets/xpubValidation.ts`, `server/src/api/wallets/approvals.ts`, `server/src/api/bitcoin/*.ts`, and `server/src/api/labels.ts`, then sweep any other `if (!foo) return 400` patterns in mutation handlers.
...
- Remaining sweep candidates still include `server/src/api/wallets/autopilot.ts`, `server/src/api/wallets/telegram.ts`, `server/src/api/wallets/xpubValidation.ts`, `server/src/api/wallets/approvals.ts`, Bitcoin mutation routes, and label mutation routes.
