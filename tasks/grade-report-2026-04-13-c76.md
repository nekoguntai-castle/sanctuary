...
| largest_file_lines | 1339 | `server/tests/integration/flows/admin.integration.test.ts` after the transfer service split; next largest validated files include `server/tests/unit/utils/docker.test.ts` at 1315 LOC and `server/tests/unit/services/intelligence/analysisService.test.ts` at 1304 LOC when generated verified-vector files are excluded. `server/tests/unit/services/transferService.test.ts` is now a 15-line registrar, with transfer service contract modules capped at 503 LOC. | 3.3 → 0 |
...
- Maintainability 3.3 score impact: **no numeric movement yet**. The OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, policy engine, blockchain service, backup service, process-transactions phase, wallet integration, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, and transfer service god-files are gone, but `server/tests/integration/flows/admin.integration.test.ts` is now the largest scoped TS/TSX file at 1339 LOC, so the repo-wide largest-file criterion remains at 0.
...
- Next validated targets: `server/tests/integration/flows/admin.integration.test.ts` (1339 LOC), `server/tests/unit/utils/docker.test.ts` (1315 LOC), and `server/tests/unit/services/intelligence/analysisService.test.ts` (1304 LOC), excluding generated verified-vector files.
...
| 3.3 Largest file (after OpenAPI + transaction + transaction HTTP route + admin API + auth registration + Bitcoin API + Draft API + Wallets API + PSBT validation + wallet import + transfer service + transaction creation + transaction broadcast + Electrum connection + sync service + wallet service + WebSocket hook + device API + render-regression e2e + Intelligence tabs + policy engine + blockchain service + backup service + process-transactions + wallet integration splits) | 0 (`admin.integration.test.ts` 1339 LOC) | 0 | +2 | +2 |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`); the recent performance/security commits (streamed exports, DoS cap, REPEATABLE READ snapshot) reinforce the existing Reliability score even though the static signals don't move. The biggest lever to reach B is now **reducing the measured lizard baseline, finishing the Zod validation sweep, and splitting the remaining oversized tests**: lint, gitleaks, lizard, and jscpd are blocking regression gates, but lizard still has 83 existing warnings and the largest test file is now `server/tests/integration/flows/admin.integration.test.ts` at 1339 LOC after the OpenAPI, transaction API, transaction HTTP route, admin API, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits.
...
- Top non-generated TS/TSX file scan — current largest after the later transaction, transaction HTTP route, admin API, auth registration API, Bitcoin API, Draft API, Wallets API, PSBT validation, wallet import, transfer service, transaction creation service, transaction broadcast service, Electrum connection, sync service, wallet service, WebSocket hook, device API, render-regression e2e, Intelligence tabs, policy engine, blockchain service, backup service, process-transactions phase, and wallet integration splits is `server/tests/integration/flows/admin.integration.test.ts` at 1339 LOC, followed by `server/tests/unit/utils/docker.test.ts` at 1315 LOC and `server/tests/unit/services/intelligence/analysisService.test.ts` at 1304 LOC when generated verified-vector files are excluded.
