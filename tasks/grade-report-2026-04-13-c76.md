...
**Overall Score**: 85/100 (implementation-adjusted; original validated baseline was 76/100)
**Grade**: B
**Confidence**: Low
**Mode**: full
**Commit**: `dd86dd2f`
...
vs 2026-04-13 (`13efff91`): original validated report was **overall +7 (69→76), grade D→C**, confidence Low→Low. The implementation-adjusted score is now **85/100** after the first-pass lint gate landed, the scoped largest-file threshold was cleared, the API body-validation sweep was guarded, and the lizard baseline reached the scored `1-5` warning band.
...
- `lizard` — not installed globally, but a temporary `/tmp` install measured 2 warnings after the continued extraction passes through the butterfly garden animation split. `.github/workflows/quality.yml` is now blocking with `-i 2`, so the warning count cannot grow without failing CI.
...
1. **CI quality signals are now blocking, but lizard is baseline-gated.** The new lint, gitleaks, lizard, and jscpd jobs are blocking in `.github/workflows/quality.yml`. `lizard` still has 2 existing warnings, so the immediate guardrail is "do not increase warning count"; reducing the remaining warnings to zero is the next maintainability score lever.
...
- Baseline the current lizard/jscpd output, document any intentional deltas, then remove `continue-on-error: true` from each job in `quality.yml` **per-job, not as a batch**. Implemented with jscpd at 2.33%, lizard now at a 2-warning no-increase baseline after the continued extraction passes through the butterfly garden animation split, and gitleaks scoped to PR/latest-commit regression scanning because full-history/current-directory scans include legacy/test/ignored-file false positives.
- Score impact: net **+3 points** in this continued pass. jscpd remains at 3.2 `+3`, while the measured lizard baseline moved 3.1 from `+0 → +3` after reaching the measured `1-5` warning band; Security 4.2 is now a measured regression gate, but not a full-history clean signal.
...
Combined low-effort ceiling from the remaining P1 items: **≈ 87–89 (B)**, depending on whether the remaining lizard baseline reaches zero and whether the legacy/current-tree gitleaks false-positive cleanup is completed. The scoped largest-file and API body-validation criteria are now cleared; additional file splits only add buffer against future growth.
...
| CI signals enforceable | Done for regression gating: `scripts/quality.sh` runs all three tools; `quality.yml`'s `gitleaks`, `lizard`, and `jscpd` jobs no longer use `continue-on-error`; gitleaks gates PR/latest commits, lizard gates no increase above 2 warnings, and jscpd gates the existing 5% threshold. |
...
The "After P1" ranges below now assume the remaining lizard warnings are removed and the gitleaks false-positive cleanup is either deferred or completed. The largest-file threshold, Zod body-validation sweep, and mid-case lizard band are already cleared; remaining score movement comes from reducing the lizard baseline from 2 warnings to zero and cleaning up full-history/current-tree gitleaks false positives.
...
| Maintainability | 13/15 | 15/15 | +2 | See per-criterion breakdown below |
...
| **TOTAL** | **85/100 (B)** | **87 → 89/100 (B)** | **+2 to +4** | Realistic next estimate: **~87 (B)** |

Arithmetic check (rounded, no handwaving):

- Current total: 20 + 12 + 13 + 13 + 4 + 13 + 10 = **85**
- Next lizard-clean total: 20 + 12 + 15 + 13 + 4 + 13 + 10 = **87**
- Best-case total: 20 + 12 + 15 + 15 + 4 + 13 + 10 = **89**
- Mid-estimate (requires remaining lizard baseline reduction to zero but defers gitleaks false-positive cleanup): 20 + 12 + 15 + 13 + 4 + 13 + 10 = **87**
...
|---|---|---|---|---|
| 3.1 Cyclomatic complexity (lizard warnings) | +3 (2 warnings) | +3 (`1–5`) | +3 (`1–5`) | +5 (`0`) |
...
The repo climbed from **D (69) → C (76)** on the back of the typecheck fix (`350f67c1`), and the implementation-adjusted score is now **85/100 (B)** after the lint gate, scoped largest-file threshold work, guarded API body-validation sweep, and the lizard baseline reduction to 2 warnings. The biggest remaining local lever is **reducing the measured lizard baseline to zero**; full-history/current-tree gitleaks false-positive cleanup can still add security headroom. The largest scoped TS/TSX file is now `server/tests/unit/services/utxoSelectionService.test.ts` at 991 LOC.
