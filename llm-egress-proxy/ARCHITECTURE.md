# LLM Egress Proxy Architecture

The LLM egress proxy is a security-isolated Express service (port 3100) that performs all external AI calls on behalf of Sanctuary. It is the only component that contacts an AI provider — the backend never calls external AI directly. Its narrow access boundary (sanitized metadata only, no database, no keys) limits blast radius if the container is compromised.
...
sequenceDiagram
    ...
    participant P as LLM Egress Proxy (3100)
...
| `POST`   | `/config`        |      —       | Update enabled/endpoint/model; requires `x-llm-egress-config-secret`           |
...
| `LLM_EGRESS_PROXY_SECRET`            | _(random at startup)_ | Shared secret for `POST /config`; must match what the backend sends                                                 |
| `LLM_EGRESS_PROXY_ALLOWED_HOSTS`      | _(empty)_             | Comma-separated host allowlist for public/cloud providers; supports exact hosts and `*.example.com` suffix patterns |
| `LLM_EGRESS_PROXY_ALLOWED_CIDRS`      | _(empty)_             | Comma-separated IPv4 CIDRs for explicit provider endpoint allowlisting                                              |
| `LLM_EGRESS_PROXY_ALLOW_PUBLIC_HTTPS` | `false`               | When `true`, any public HTTPS provider endpoint is allowed; public HTTP remains blocked unless host/CIDR-allowed    |
...
**No database access.** The proxy fetches only what each request needs from `BACKEND_URL/internal/ai/*`. There is no persistent storage, no Prisma, no Redis. This is a deliberate security boundary: compromise of the LLM egress proxy yields sanitized metadata only.

**Dedicated Compose network.** The proxy does not join the main `sanctuary-network`; it shares a separate `llm-egress-network` only with the backend. That keeps Postgres and Redis service DNS out of the proxy network while preserving provider egress.
...
**Provider endpoint policy.** The proxy allows host-local and mDNS-discovered endpoints by default (`localhost`, `host.docker.internal`, `*.local`). It also admits the admin-approved provider endpoint — the one saved through `POST /config`, or the one typed into `POST /detect-provider` for that request only — when it is a private LAN IP literal, never loopback or cloud metadata. Approval covers that exact address and port, so a provider redirect to any other LAN address or port is still refused, and saving a new endpoint revokes the previous one. Hostnames are never approved this way: a name is re-resolved on every hop, so its DNS could steer the proxy elsewhere. It only widens what the static rules refuse. Docker service names, other LAN hostnames, and other numeric LAN/public IPs require `LLM_EGRESS_PROXY_ALLOWED_HOSTS`, `LLM_EGRESS_PROXY_ALLOWED_CIDRS`, or `LLM_EGRESS_PROXY_ALLOW_PUBLIC_HTTPS=true`. URLs with embedded credentials are rejected.
