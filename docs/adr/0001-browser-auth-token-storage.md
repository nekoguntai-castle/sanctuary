...
- The frontend has **no refresh flow at all**: no `/auth/refresh` callers exist outside `gateway/`, no 401 interceptor exists in `client.ts`, and `src/contexts/UserContext.tsx` does not handle token expiry. Access tokens silently expire after their 1-hour TTL and the user gets opaque request failures until the next manual login.
...
- 2FA temporary tokens. They live in `src/contexts/UserContext.tsx` React state for ≤5 minutes and never touch `sessionStorage`. Their flow is unchanged by any option below.
...
4. **Frontend.**
   ...
   - `src/contexts/UserContext.tsx`: stop calling `apiClient.setToken`. Login state derives from a `/auth/me`-style "is the cookie still good?" check, which already exists.
...
**Carried over to Phase 6 (one release after Phase 2):** remove the JSON `token`/`refreshToken` fields from the browser-mounted `/auth/login`, `/auth/2fa/verify`, and `/auth/refresh` response bodies; remove `VITE_AUTH_TOKEN_STORAGE` from `config/tooling/vite.config.ts` and `.env.example`; audit `src/api/client.ts` for Phase-4-era dead code.
