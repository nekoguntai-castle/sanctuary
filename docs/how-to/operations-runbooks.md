...
- Grafana: `http://127.0.0.1:3000`, authenticated as `admin` with the independent `GRAFANA_PASSWORD` stored in the Sanctuary runtime environment. Upgrades migrate an existing Grafana database before startup and never reuse `ENCRYPTION_KEY` as a login credential.
  Use `./scripts/setup.sh --upgrade --enable-monitoring` or `./start.sh --with-monitoring`. These wrappers resolve the physical `grafana_data` and `grafana_quiescence` Docker volumes, require positive stopped-state proof, and run the digest-verified packaged migration image under a fixed daemon-side container name. The control volume holds single-use leases and scoped terminal outcomes, so migration safety does not depend on client host paths, PID namespaces, or `flock`. A running or indeterminate sentinel blocks startup; a terminal sentinel is removed only after its exact `success` or `rolled-back` outcome is verified. A direct Compose start intentionally refuses unmarked legacy Grafana data without that proof.
...
`csrf-csrf` uses a per-session double-submit token derived from the sanctuary JWT secret material. The CSRF cookie rotates on every credential auth response (registration, login, 2FA verify, refresh). In browsers with Web Locks, unsafe attempts construct their CSRF header while holding the shared browser-auth lock, preventing a race with exclusive refresh; without Web Locks, refresh remains single-flight within one tab but cross-tab coordination is unavailable. For a trusted registration, login, or 2FA `POST`, an access cookie plus either a missing CSRF cookie or a present CSRF cookie exactly echoed by the request header but failing its access-token binding is classified as stale. The server clears the stale cookies and returns the typed `AUTH_CSRF_SESSION_STALE` 403, after which the frontend may retry that credential request exactly once. Generic 401 refresh-and-replay remains separate and can replay eligible protected requests. In contrast, a protected unsafe request that fails CSRF and any ordinary CSRF 403 is never replayed through the stale-CSRF path. If a request persistently fails CSRF validation with `EBADCSRFTOKEN` in backend logs after Phase 4, the common causes are:
