...
| Critical security logic | Mutation gate for auth, access control, address derivation, PSBT validation, and broadcast canonicality | `cd server && npm run test:mutation:critical:gate` or the `full-critical-mutation` CI job | Required when touched; required for main/nightly |
| Broadcast canonicality | Transaction broadcast routes derive recipient, amount, fee, and input set from decoded PSBT/raw payload data before policy, audit success, persistence, or node submission | `cd server && npm run test:mutation:critical:gate`; `cd server && npx vitest run tests/unit/api/transactions-http-routes.test.ts tests/unit/services/bitcoin/transactionServiceBroadcast/broadcastContracts.test.ts tests/unit/services/bitcoin/industry/broadcastSafety.test.ts`; full backend integration `flows` matrix | Required when broadcast paths change; node `testmempoolaccept` preflight remains pending before claiming complete broadcast safety |
| Physical hardware signing | Ledger, Trezor, and BitBox signing claims require executable fixture intake plus replayable sanitized artifacts | `REQUIRE_HARDWARE_SIGNED_FIXTURES=1 npm --prefix server run test -- --run tests/unit/services/bitcoin/psbt.hardware-signed-vectors.test.ts`; fixture rows must satisfy `server/tests/helpers/hardwareSignedFixtureIntake.ts` | Intake schema required when fixture rows change; full gate pending until 11 required physical artifacts are captured or product-blocked |
