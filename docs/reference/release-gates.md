...
| Hardware signing proof                        | Tier 2 Trezor emulator conformance runs the production adapter against exact OCI platform/image/config digests, firmware, Bridge, and Connect core protocol versions/SRIs; a production Connect-Web entrypoint contract is separate and does not emulate iframe/Suite orchestration. Tier 3 claims require replayable adapter-native artifacts, a reachable tested commit, current source manifest, and a trusted signed Core receipt | `npm run test:trezor-emulator-proof`; Trezor physical-only gate: `REQUIRE_TREZOR_PHYSICAL_FIXTURES=1 npm --prefix server run test -- --run tests/unit/services/bitcoin/psbt.hardware-signed-vectors.test.ts`; full physical gate: `REQUIRE_HARDWARE_SIGNED_FIXTURES=1 npm --prefix server run test -- --run tests/unit/services/bitcoin/psbt.hardware-signed-vectors.test.ts` | Tier 2 is required in ordinary relevant CI but never enables a capability. Tier 3 and its trusted evidence key remain pending; all Trezor rows stay disabled. |
