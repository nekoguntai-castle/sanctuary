...
| Release provenance | Stable release artifacts must be covered by a manifest, signed `SHA256SUMS`, local checksums, SBOM/provenance references, offline bundle metadata, and container image digests | `npm run release:verify-artifacts -- --manifest release-manifest.json --strict-stable --public-key scripts/offline/keys/sanctuary-offline-release-public.pem`; use `--verify-image-digests` when registry access is available; `npm run test:release-artifacts` when verifier code changes | Required for stable releases; keep release objects draft/prerelease when required manifest material is missing |
