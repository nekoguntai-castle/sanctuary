...
| AI proxy, model provider, Console, and MCP | LLM sees secrets or performs wallet-changing actions | AI/MCP tools are read-only, scoped at execution time, and exclude private keys, descriptors, xpubs, PSBTs, bearer tokens, and provider API keys |
