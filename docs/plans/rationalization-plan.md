...
| Gateway runtime and deploy contracts | Phase L aligns gateway listener env handling with compose (`PORT` with `GATEWAY_PORT` compatibility), backend defaults with `backend:3001`, and push credential loading with mounted FCM/APNs files plus deterministic env overrides. | Remaining risk is operational: secrets and mounted files must still be present in the deployment environment. | Phase L implementation |
| Prebuilt compose/image inventory | Base compose builds `gateway` and `llm-egress-proxy`; `docker-compose.ghcr.yml` and the release image push script intentionally cover only frontend/backend prebuilt images. | Prebuilt deploy users get a reduced web-core service set; mobile gateway/push and LLM egress proxy isolation require source compose or the offline bundle. | Mark prebuilt as reduced/core |
...
| Stale installer helper comment | Phase L updates install diagnostics to use the live `llm-egress-proxy` service name and removes the stale Ollama mapping comment. | Residual references to historical `/ai/*` route removals remain only in planning history and negative test context. | Phase L implementation |
