...
Date: 2026-05-15
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase O; Phase P UTXO selection strategy convergence is next
...
- Current review status: no completed phase is reopened, and the physical hardware test remains the only deferred external validation item. The post-Phase-I queue is merged through Phase O: sync priority validation closed in PR #474, mempool estimator defaults closed in PR #475, gateway deploy/runtime contracts closed in PR #476, transfer route validation closed in PR #477, UTXO selection route validation closed in PR #478, websocket protocol ownership closed in PR #479, and frontend API hygiene closed in PR #480. The remaining active queue starts with lower-priority UTXO strategy values, followed by touched-code cleanup.
...
- Worth consolidating first after the already-merged J/K/L/M/M2/N work: the small frontend API hygiene pass covering Payjoin naming, central query construction, already-owned shared response types, and low-level PSBT detection helpers.
- UTXO selection strategies are still duplicated, but route validation and service comparison enumerate the same set after Phase M2. Treat this as a lower-priority coin-control follow-up rather than a top standalone phase.
...
| UTXO selection strategies | Strategy values repeat in `shared/types/domain.ts`, `server/src/api/transactions/coinSelection.ts`, `server/src/services/utxoSelectionService/types.ts`, `server/src/services/utxoSelectionService/index.ts`, `server/src/services/bitcoin/utxoSelection.ts`, transaction OpenAPI schemas, frontend API types/tests, and server tests. The legacy transaction-builder selector also still exposes `branch_and_bound`, which is not a public select/compare strategy. | Adding/removing a strategy can leave route validation, OpenAPI, service compare/recommendation results, transaction-builder defaults, and frontend types out of sync. No current public select/compare value drift found after Phase M2, but the internal `branch_and_bound` name must either be retired or explicitly documented as a separate non-public mode. | Lower-priority coin-control follow-up |
...
| P | Centralize UTXO selection strategies when coin-control code is next touched. Put the public select/compare strategy tuple/type in the shared transaction constants owner or another shared-safe transaction module; derive route validation, service compare strategy list, recommended-strategy result typing, OpenAPI, and frontend types. Classify `server/src/services/bitcoin/utxoSelection.ts` as either a consumer of the canonical public strategy subset or a separately named internal transaction-builder mode, and remove or explicitly justify its stale `branch_and_bound` value. | UTXO selection route tests for every canonical valid strategy and invalid strategies, service compare tests proving every canonical strategy is included exactly once in order, recommended-strategy tests proving only canonical public values are returned, OpenAPI contract/parity tests, frontend API/type tests including replacement of stale noncanonical mocks, focused legacy `server/src/services/bitcoin/utxoSelection.ts`/transaction-estimation tests if touched, app/server typechecks, negative production tuple search plus scoped `branch_and_bound` search. | One public strategy owner drives API, service, OpenAPI, and frontend contracts; any separate internal transaction-builder mode is named and tested as non-public rather than silently drifting. |
...
- Keep the remaining active queue as small implementation PRs. Phase P should stay focused on UTXO selection strategy ownership and should not bundle unrelated Q cleanup, generated-client work, or new amount/`scriptType` API behavior.
...
- Phase P should distinguish public API strategy values from any internal transaction-builder selection mode. Do not expose `branch_and_bound` through OpenAPI/frontend types unless the service actually implements and tests it as a supported public strategy.
...
The sync-priority, mempool-estimator, gateway, transfer, UTXO route-validation, websocket, and frontend API hygiene bullets below are retained as Phase J/K/L/M/M2/N/O closeout history; the active implementation notes now start with Phase P.
...
### Phase J-O Closeout Addendum - 2026-05-16
...
- Phase P UTXO selection strategy convergence is now the first open implementation phase. Source evidence on `origin/main` still shows strategy values repeated across shared domain types, the UTXO selection route, service types/listing, the legacy transaction-builder selector, OpenAPI, frontend API tests, and server tests.
- Source checks for this closeout review used `origin/main` at `7aa5726018102e277b822e03d6836822612e9bed`.
...
- A recursive plan-review pass on 2026-05-16 rechecked the plan against `origin/main` after Phase N merged, updated active status through Phase N, and confirmed Phase O frontend API hygiene as the next slice. Phase O later merged via PR #480, making Phase P UTXO strategy convergence the active slice.
