...
Date: 2026-05-15
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase Q2; Phase Q3 device connection-method cleanup active
...
- Current review status: no completed phase is reopened, and the physical hardware test remains the only deferred external validation item. The post-Phase-I queue is merged through Phase Q2: sync priority validation closed in PR #474, mempool estimator defaults closed in PR #475, gateway deploy/runtime contracts closed in PR #476, transfer route validation closed in PR #477, UTXO selection route validation closed in PR #478, websocket protocol ownership closed in PR #479, frontend API hygiene closed in PR #480, UTXO selection strategy ownership closed in PR #481, shared value-contract cleanup closed in PR #482, and server LLM egress config accessor cleanup closed in PR #483. The remaining Phase Q queue is low-priority touched-code cleanup rather than one coherent high-risk implementation slice; the active Q3 slice is ConnectDevice connection-method ownership.
...
| Q3 | Centralize ConnectDevice connection-method values | `utils/deviceConnection.ts`, `components/ConnectDevice/types.ts`, focused ConnectDevice tests | Device connection utility tests, ConnectDevice selector tests, app/test typechecks, lint/lizard, negative duplicate-union searches | Active; keep send-review signing methods separate |
...
- The J/K/L/M/M2/N/O/P/Q1/Q2 follow-up queue is closed. The remaining findings are lower-risk touched-code cleanup candidates rather than the kind of active divergent path that justified the earlier phases.
...
- Phase Q should be executed only as bounded sub-slices. Q1 shared value-contract ownership for admin wallet-agent statuses/severities, device roles, RBF statuses, and privacy grades is closed, and Q2 server LLM egress env accessor cleanup is closed. The next active sub-slice is ConnectDevice connection-method ownership because the utility and component prop types independently define the same `usb`/`sd_card`/`qr_code`/`manual` contract. Send-review signing methods, hardware vendor normalization, price-provider fallback, and `API_BASE_URL` export cleanup stay deferred unless fresh source evidence makes one of them the touched domain.
...
| ConnectDevice connection methods | `utils/deviceConnection.ts` and `components/ConnectDevice/types.ts` independently define the same `usb`/`sd_card`/`qr_code`/`manual` union, and `getAvailableMethods` casts model connectivity strings after checking the UI config map. | Low risk, but this is the exact value-contract drift pattern Phase Q is meant to reduce as touched. | Phase Q3 active |
...
| Q3 | Make ConnectDevice connection method values derive from one utility-owned tuple/type. Keep send-review signing methods as a separate `usb`/`airgap`/`qr` domain. | Device connection utility tests for known/unknown connectivity and manual fallback, ConnectDevice selector tests, app/test typechecks, lint/lizard, negative duplicate-union searches. | Active; component prop types and utility parsing derive from the same owner without changing UI behavior. |
| Q later | Low-priority cleanup as touched: hardware vendor normalization, price-provider offline fallback, and `API_BASE_URL` export cleanup. | Focused tests for the touched domain plus negative tuple/accessor searches. | Drift-prone literals and stale comments are reduced without bundling unrelated cleanup into Q3. |
...
- Phase Q3 should touch only ConnectDevice connection-method ownership. It must not merge send-review signing methods (`usb`/`airgap`/`qr`) with device onboarding methods (`usb`/`sd_card`/`qr_code`/`manual`), and it should leave hardware vendor alias detection for a separate review.
...
### Phase J-Q2 Closeout Addendum - 2026-05-16
...
- Phase Q is the remaining as-touched cleanup bucket. Current source evidence still shows repeatable but lower-risk candidates such as ConnectDevice connection-method duplication, hardware vendor normalization in export/import/signing helpers, and an exported frontend `API_BASE_URL` with only internal/test consumers.
- The next Phase Q sub-slice should address only ConnectDevice connection-method ownership. `utils/deviceConnection.ts` and `components/ConnectDevice/types.ts` currently define the same `usb`/`sd_card`/`qr_code`/`manual` value contract independently, while the send-review signing flow uses a distinct `usb`/`airgap`/`qr` vocabulary that should stay separate. The remaining Phase Q candidates stay deferred because they touch different behavior surfaces and need their own focused verification.
- Source checks for this closeout review used `origin/main` at `4ec0d4a4b69b508cee06a74b6468aef0ba2258b0`.
...
- Recursive plan-review passes on 2026-05-16 rechecked the plan after the Phase N, O, P, Q1, and Q2 merges. The current pass updated active status through Phase Q2 and confirmed Phase Q as a low-priority as-touched cleanup bucket rather than a required broad consolidation phase.
