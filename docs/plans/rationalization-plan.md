...
Date: 2026-05-15
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase N; Phase O frontend API hygiene is next
...
- Current review status: no completed phase is reopened, and the physical hardware test remains the only deferred external validation item. The post-Phase-I queue is merged through Phase N: sync priority validation closed in PR #474, mempool estimator defaults closed in PR #475, gateway deploy/runtime contracts closed in PR #476, transfer route validation closed in PR #477, UTXO selection route validation closed in PR #478, and websocket protocol ownership closed in PR #479. The remaining active queue starts with frontend API hygiene, followed by lower-priority UTXO strategy values and touched-code cleanup.
...
- Keep the remaining active queue as small implementation PRs. Phase O touches frontend API and QR/file/import boundaries; do not bundle P/Q cleanup or generated-client work into it.
...
The sync-priority, mempool-estimator, gateway, transfer, UTXO route-validation, and websocket bullets below are retained as Phase J/K/L/M/M2/N closeout history; the active implementation notes now start with Phase O.
...
### Phase J-N Closeout Addendum - 2026-05-16
...
- Phase O frontend API hygiene is now the first open implementation phase. Source evidence on `origin/main` still shows Payjoin API availability and send-attempt status name collision, manual frontend query construction outside `apiClient`, overlapping shared/frontend response types, and duplicate PSBT low-level detection helpers.
- Source checks for this closeout review used `origin/main` at `5566ededd900c6cd92b42223c3a29c3899f73952`.
...
- A recursive plan-review pass on 2026-05-16 rechecked the plan against `origin/main` after Phase N merged, updated active status through Phase N, and confirmed Phase O frontend API hygiene remains the next active slice.
