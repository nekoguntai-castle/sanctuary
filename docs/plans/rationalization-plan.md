...
Date: 2026-05-15
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase Q3; Phase Q4 frontend API base URL export cleanup active
...
- Current review status: no completed phase is reopened, and the physical hardware test remains the only deferred external validation item. The post-Phase-I queue is merged through Phase Q3: sync priority validation closed in PR #474, mempool estimator defaults closed in PR #475, gateway deploy/runtime contracts closed in PR #476, transfer route validation closed in PR #477, UTXO selection route validation closed in PR #478, websocket protocol ownership closed in PR #479, frontend API hygiene closed in PR #480, UTXO selection strategy ownership closed in PR #481, shared value-contract cleanup closed in PR #482, server LLM egress config accessor cleanup closed in PR #483, and ConnectDevice connection-method ownership closed in PR #484. The remaining Phase Q queue is low-priority touched-code cleanup rather than one coherent high-risk implementation slice; the active Q4 slice is frontend API base URL export cleanup.
...
| Q4 | Retire the exported frontend `API_BASE_URL` constant | `src/api/baseUrl.ts`, `src/api/client.ts`, API client tests/mocks | API base/client tests, app/test typechecks, lint/lizard, negative production export searches | Active; keep raw refresh/health helpers on `baseUrl.ts` |
...
- The J/K/L/M/M2/N/O/P/Q1/Q2/Q3 follow-up queue is closed. The remaining findings are lower-risk touched-code cleanup candidates rather than the kind of active divergent path that justified the earlier phases.
...
- Phase Q should be executed only as bounded sub-slices. Q1 shared value-contract ownership, Q2 server LLM egress env accessor cleanup, and Q3 ConnectDevice connection-method ownership are closed. The next active sub-slice is frontend API base URL export cleanup because `src/api/baseUrl.ts` owns base URL resolution while `src/api/client.ts` still exports a cached `API_BASE_URL` constant used only by the client and tests. Hardware vendor/type normalization and price-provider fallback stay deferred unless fresh source evidence makes one of them the touched domain.
...
| Admin agent statuses and severities | PR #482 added shared admin-agent value owners and migrated frontend admin types, server route schemas, OpenAPI schemas/paths, services, repositories, and dashboard UI typing to derive from them. | Closed. Wire strings are unchanged and focused tests/negative searches cover tuple, union, and cast drift. | Converged in Phase Q1 |
| Server LLM egress env bindings | PR #483 moved backend AI config sync and egress proxy auth helpers to central server config fields while preserving proxy-local env validation. | Closed. Deployment env parsing now has one backend owner, and the LLM egress proxy remains an independent security boundary. | Converged in Phase Q2 |
| ConnectDevice connection methods | PR #484 added utility-owned connection-method tuples/types/guards, made `components/ConnectDevice/types.ts` derive from that owner, and removed casts from model connectivity parsing. | Closed. Manual fallback, secure-context gating, unknown connectivity filtering, and the separate send-review signing vocabulary are preserved. | Converged in Phase Q3 |
| Frontend API base URL export | `src/api/baseUrl.ts` owns `getApiBaseUrl()` and `joinApiBaseUrl()`, while `src/api/client.ts` exports a cached `API_BASE_URL` constant with no production consumers outside the client. | Low risk, but the export is a second public accessor for base URL behavior that tests can keep alive after production callers move away. | Phase Q4 active |
...
| Device roles | PR #482 added a shared device-role owner and migrated shared domain, server device access/event types, OpenAPI, and tests to derive from it while keeping device roles separate from wallet roles. | Closed. Malformed stored direct/group roles fail closed and owner/viewer semantics are unchanged. | Converged in Phase Q1 |
| RBF statuses and privacy grades | PR #482 added shared RBF/privacy grade owners and migrated shared types, OpenAPI, services, UI helpers, and tests to derive from them. | Closed. RBF wire values, privacy-score thresholds, UI labels, and compatibility behavior are unchanged. | Converged in Phase Q1 |
...
| Q1 | Centralize the first shared value-contract slice: admin wallet-agent status, alert severity/status, funding override status, device roles, RBF status, and privacy grades. | Shared constants tests, OpenAPI parity assertions for admin/device/privacy contracts, focused server/admin/device/privacy/UI tests, app/server/shared typechecks, negative tuple and union/cast searches scoped to production. | Merged via PR #482. |
| Q2 | Make backend LLM egress URL/secret access go through central server config. Do not share backend config into the proxy process or weaken proxy-local endpoint/secret validation. | Focused AI config sync/client tests for configured URL and configured/missing secret behavior, server config tests if config schema changes, app/server typechecks as needed, server build/lint/lizard, negative direct-env searches in backend AI helpers. | Merged via PR #483. |
| Q3 | Make ConnectDevice connection method values derive from one utility-owned tuple/type. Keep send-review signing methods as a separate `usb`/`airgap`/`qr` domain. | Device connection utility tests for known/unknown connectivity and manual fallback, ConnectDevice selector tests, app/test typechecks, lint/lizard, negative duplicate-union searches. | Merged via PR #484. |
| Q4 | Retire the exported frontend `API_BASE_URL` constant and route client URL construction through `joinApiBaseUrl`. Keep `src/api/baseUrl.ts` as the public helper for raw refresh/health fetch boundaries. | API base/client tests for default and configured base URLs, query/blob/download/upload URL construction, app/test typechecks, lint/lizard, negative production `API_BASE_URL` export search. | Active; base URL resolution has one public frontend owner without changing request URLs. |
| Q later | Low-priority cleanup as touched: hardware vendor/type normalization and price-provider offline fallback. | Focused tests for the touched domain plus negative tuple/accessor searches. | Drift-prone literals and stale comments are reduced without bundling unrelated cleanup into Q4. |
...
- Phase Q1 closed as a value-contract PR only. It did not normalize hardware vendor aliases, redesign device connection methods, change price-provider bootstrap/offline behavior, alter LLM egress proxy configuration loading, or remove the exported frontend `API_BASE_URL`.
- Phase Q2 closed by touching only backend AI helpers and central server config accessors. It preserved the retained LLM egress proxy as an independently configured security boundary; proxy-local `process.env.LLM_EGRESS_PROXY_SECRET` handling remains intentional and out of scope.
- Phase Q3 closed by touching only ConnectDevice connection-method ownership. It did not merge send-review signing methods (`usb`/`airgap`/`qr`) with device onboarding methods (`usb`/`sd_card`/`qr_code`/`manual`), and it left hardware vendor alias detection for a separate review.
- Phase Q4 should touch only frontend API base URL accessor ownership. It must preserve `VITE_API_URL`, default `/api/v1`, ordinary query encoding, transfer URL construction, and the raw refresh/health fetch boundary helpers that already import `src/api/baseUrl.ts`.
...
- Q2 must preserve the current default proxy URL of `http://llm-egress-proxy:3100`, the current behavior where an absent backend secret sends empty proxy-auth/config-secret headers and logs a warning during config sync, and the existing config-sync hash/resync behavior.
- Q2 should not move proxy runtime secret generation, proxy endpoint allowlists, provider URL validation, sanitized-context routing, or proxy-local request validation into server shared config. Those are separate security-boundary responsibilities.
- Q2 tests must account for module-level URL capture in AI callers. If tests mutate env/config values, they should reset modules before import rather than depending on runtime mutation after import.
- Q3 must preserve manual entry as the always-available fallback even when the device model has no known connectivity values.
- Q3 must preserve secure-context gating for USB and QR camera scanning, and must continue to ignore unknown hardware model connectivity values rather than rendering unsupported buttons.
- Q3 must treat send-review signing method values as intentionally separate from ConnectDevice onboarding method values; `airgap` and `qr` are signing-flow labels, not aliases for `sd_card` and `qr_code` in this slice.
- Q4 must not reintroduce route-local base URL string concatenation in health or refresh. Those raw-fetch helpers intentionally call `getApiBaseUrl()`/`joinApiBaseUrl()` because they sit outside the authenticated `apiClient` interceptor path.
- Q4 should update tests to assert request URLs and `baseUrl.ts` helper behavior instead of keeping `src/api/client.ts`'s old public `API_BASE_URL` export alive through test-only imports or mocks.

### Phase J-Q3 Closeout Addendum - 2026-05-16
...
- Phase Q is the remaining as-touched cleanup bucket. Current source evidence still shows repeatable but lower-risk candidates such as an exported frontend `API_BASE_URL` with only internal/test consumers, hardware vendor/type normalization across several behavior surfaces, and price-provider offline fallback.
- The next Phase Q sub-slice should address only frontend API base URL export cleanup. `src/api/baseUrl.ts` already owns public base URL resolution/joining helpers, while `src/api/client.ts` still exports `API_BASE_URL` for tests despite having no production consumers outside the client. Hardware vendor/type normalization and price-provider fallback stay deferred because they touch broader behavior surfaces or already have a server runtime owner.
- Source checks for this closeout review used `origin/main` at `9b78adb7f93fe990250f6c07b78a8b4a942221f3`.
...
- Recursive plan-review passes on 2026-05-16 rechecked the plan after the Phase N, O, P, Q1, Q2, and Q3 merges. The current pass updated active status through Phase Q3 and confirmed Phase Q as a low-priority as-touched cleanup bucket rather than a required broad consolidation phase.
...
- The post-Q1 recursive plan-review pass accepted Q2 only as a bounded backend accessor cleanup, not a proxy/backend validation merge. Proxy-local env handling remains a justified split.
- The post-Q2 recursive plan-review pass accepted Q3 only as a bounded ConnectDevice connection-method ownership cleanup. The send-review signing-method domain, hardware vendor aliases, price-provider fallback, and frontend `API_BASE_URL` export remain deferred.
- The post-Q3 recursive plan-review pass accepted Q4 only as a bounded frontend API base URL export cleanup. Hardware vendor/type normalization remains deferred because current evidence spans onboarding, signing, icons, adapters, and import/export naming rather than one small value contract.
