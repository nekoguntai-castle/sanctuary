...
Date: 2026-05-15
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase Q1; Phase Q2 server LLM egress config accessor cleanup active
...
- Current review status: no completed phase is reopened, and the physical hardware test remains the only deferred external validation item. The post-Phase-I queue is merged through Phase Q1: sync priority validation closed in PR #474, mempool estimator defaults closed in PR #475, gateway deploy/runtime contracts closed in PR #476, transfer route validation closed in PR #477, UTXO selection route validation closed in PR #478, websocket protocol ownership closed in PR #479, frontend API hygiene closed in PR #480, UTXO selection strategy ownership closed in PR #481, and shared value-contract cleanup closed in PR #482. The remaining Phase Q queue is low-priority touched-code cleanup rather than one coherent high-risk implementation slice; the active Q2 slice is server LLM egress config accessor cleanup.
...
| Q2 | Centralize server LLM egress config accessors | `server/src/config/*`, `server/src/services/ai/config.ts`, `server/src/services/ai/llmEgressProxyClient.ts`, focused AI/proxy tests | AI config sync/client tests, server config tests, type/build/lint/lizard, negative direct-env searches | Active; preserve the proxy security boundary and proxy-local env validation |
...
- The J/K/L/M/M2/N/O/P/Q1 follow-up queue is closed. The remaining findings are lower-risk touched-code cleanup candidates rather than the kind of active divergent path that justified the earlier phases.
...
- Phase Q should be executed only as bounded sub-slices. Q1 shared value-contract ownership for admin wallet-agent statuses/severities, device roles, RBF statuses, and privacy grades is closed. The next active sub-slice is server LLM egress env accessor cleanup because central server config already owns the URL/secret values while backend AI helpers still read `process.env` directly. Device connection/vendor normalization, price-provider fallback, and `API_BASE_URL` export cleanup stay deferred unless fresh source evidence makes one of them the touched domain.
...
| Server LLM egress env bindings | Central server config exposes `llmEgressProxyUrl`/`llmEgressProxySecret`, while AI config sync and egress proxy auth helpers read `process.env` directly. | Deployment env parsing can drift; security boundary remains justified. | Phase Q2 active |
...
- Phase Q2 should touch only backend AI helpers and central server config accessors. It must preserve the retained LLM egress proxy as an independently configured security boundary; proxy-local `process.env.LLM_EGRESS_PROXY_SECRET` handling remains intentional and out of scope.
...
### Phase J-Q1 Closeout Addendum - 2026-05-16
...
- Phase Q is the remaining as-touched cleanup bucket. Current source evidence still shows repeatable but lower-risk candidates such as hardware vendor normalization in export/import/signing helpers; server LLM egress helpers reading env directly despite central config fields; and an exported frontend `API_BASE_URL` with only internal/test consumers.
- The next Phase Q sub-slice should address only backend server LLM egress env accessors. Central server config already owns `llmEgressProxyUrl` and `llmEgressProxySecret`, while `server/src/services/ai/config.ts` and `server/src/services/ai/llmEgressProxyClient.ts` still read `process.env` directly. The remaining Phase Q candidates stay deferred because they touch different behavior surfaces and need their own focused verification.
- Source checks for this closeout review used `origin/main` at `ea9bdaf0d1107b4b3326e4f67b1aea7952a73790`.
...
- Recursive plan-review passes on 2026-05-16 rechecked the plan after the Phase N, O, P, and Q1 merges. The current pass updated active status through Phase Q1 and confirmed Phase Q as a low-priority as-touched cleanup bucket rather than a required broad consolidation phase.
