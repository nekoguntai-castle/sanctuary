...
- Phase 5 is merged: Sanctuary-managed model pull/delete/install/download surfaces are removed while the LLM egress proxy security boundary remains.
- Current Phase 6 priority is delivery: centralize nested preference path reads, nested patch construction, and optimistic rollback semantics without replacing backend validation or canonical storage.
...
| Nested preference patches | `useUserPreference`, `UserContext`, list preference hooks, notification sounds, server feature settings | Dot-path reads, nested patch construction, full-snapshot sends, and optimistic rollback behavior were recreated at several call sites | Phase 6 branch `codex/phase-6-preference-patches`; `utils/preferencePaths.ts`; focused preference tests | Converge in small helper pass |
...
- Current worktree is Phase 6 on `codex/phase-6-preference-patches`; local focused tests, full frontend coverage, typechecks, lint, lizard, test hygiene, large-file classification, and `git diff --check` have passed. Architecture graph regeneration produced the intended `docs/architecture/generated/frontend.md` update for the new preference helper.
