...
Date: 2026-05-15
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase Q4; remaining Phase Q candidates deferred/watch-only
...
- Current review status: no completed phase is reopened, and the physical hardware test remains the only deferred external validation item. The post-Phase-I queue is merged through Phase Q4: sync priority validation closed in PR #474, mempool estimator defaults closed in PR #475, gateway deploy/runtime contracts closed in PR #476, transfer route validation closed in PR #477, UTXO selection route validation closed in PR #478, websocket protocol ownership closed in PR #479, frontend API hygiene closed in PR #480, UTXO selection strategy ownership closed in PR #481, shared value-contract cleanup closed in PR #482, server LLM egress config accessor cleanup closed in PR #483, ConnectDevice connection-method ownership closed in PR #484, and frontend API base URL export cleanup closed in PR #485. The remaining Phase Q findings are low-priority touched-code cleanup rather than one coherent high-risk implementation slice.
...
- The J/K/L/M/M2/N/O/P/Q1/Q2/Q3/Q4 follow-up queue is closed. The remaining findings are lower-risk touched-code cleanup candidates rather than the kind of active divergent path that justified the earlier phases.
...
- Phase Q should be executed only as bounded sub-slices. Q1 shared value-contract ownership, Q2 server LLM egress env accessor cleanup, Q3 ConnectDevice connection-method ownership, and Q4 frontend API base URL export cleanup are closed. Hardware vendor/type normalization and price-provider fallback stay deferred unless fresh source evidence makes one of them the touched domain.
...
| Frontend API base URL export | PR #485 retired `src/api/client.ts`'s exported `API_BASE_URL` constant and kept request/blob/download/upload URL construction on an internal `joinApiBaseUrl(getApiBaseUrl(), endpoint)` helper. | Closed. `src/api/baseUrl.ts` remains the public frontend base URL helper for raw refresh/health boundaries, and API client tests now assert actual request URLs instead of importing or mocking a duplicate export. | Converged in Phase Q4 |
...
- Phase Q4 closed by touching only frontend API base URL accessor ownership. It preserved `VITE_API_URL`, default `/api/v1`, ordinary query encoding, transfer URL construction, and the raw refresh/health fetch boundary helpers that already import `src/api/baseUrl.ts`.
...
The sync-priority, mempool-estimator, gateway, transfer, UTXO route-validation, websocket, frontend API hygiene, UTXO strategy, and Phase Q bullets below are retained as closeout history; no active implementation queue starts here.
...
- Q4 preserved raw health and refresh fetch helpers on `getApiBaseUrl()`/`joinApiBaseUrl()` because they sit outside the authenticated `apiClient` interceptor path.
- Q4 updated tests to assert request URLs and `baseUrl.ts` helper behavior instead of keeping `src/api/client.ts`'s old public `API_BASE_URL` export alive through test-only imports or mocks.
...
- Recursive plan-review passes on 2026-05-16 rechecked the plan after the Phase N, O, P, Q1, Q2, Q3, and Q4 merges. The latest pass updated active status through Phase Q4 and confirmed Phase Q as a low-priority as-touched cleanup bucket rather than a required broad consolidation phase.
...
- The post-Q4 recursive plan-review pass accepted only closeout documentation changes. Hardware vendor/type normalization and price-provider fallback remain deferred/watch-only for the source-evidence reasons above, and no further active rationalization phase is justified now.
