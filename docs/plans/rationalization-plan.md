...
Date: 2026-05-16
Owner: Codex
Status: Original queue complete; optional follow-up queue complete through Phase T; remaining candidates deferred/watch-only/as-touched after 2026-05-16 post-Phase-T reanalysis
...
| Wallet policy route and admin policy schemas | Wallet policy mutation routes still accept unknown/passthrough bodies while OpenAPI documents typed policy requests; admin policy routes are stricter but keep local policy type/enforcement literals. The service remains the strongest behavior guard. | Route/OpenAPI/admin enum drift can recur, but this is domain-local and lower risk than the completed route-boundary fixes. | Watch; converge with policy API work |
| Draft status route schema | Draft update route still accepts `status: z.string()`, while OpenAPI and service logic restrict draft statuses. | Service validation reduces blast radius; the route contract remains looser than documented. | Watch |
...
| --- | --- | --- | --- |
| U | Handle policy, draft, websocket-confirmation, price fallback, node config, and broader hardware normalization only when touching those domains. | Focused domain tests plus negative searches for the specific tuple/schema/helper being removed. | Drift shrinks without broad churn or cross-domain behavior merges. |
