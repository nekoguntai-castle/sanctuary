...
- Completed: stale route-to-repository exceptions fail the boundary check; current baseline is `45` exceptions after moving admin backup credential lookup, admin settings persistence, admin monitoring persistence, admin group persistence, and wallet policy persistence behind services.
