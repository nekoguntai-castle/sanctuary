...
- Completed: stale route-to-repository exceptions fail the boundary check; current baseline is `47` exceptions after moving admin backup credential lookup, admin settings persistence, admin monitoring persistence, and admin group persistence behind services.
...
- [x] 7. Reduce targeted test noise and time-based toast cleanup. Completed 2026-04-14.
- [x] 8. Triage low/moderate dependency advisory debt. Completed 2026-04-14.
- [x] 9. Run full `npm run quality` and full Docker install/upgrade e2e proof. Completed 2026-04-14.
...
- [x] 16. Add selected response-schema parity guards after OpenAPI route coverage. Completed for `AdminSettings`, `AdminMonitoringServicesResponse`, `AdminGroup`, and `AdminGroupMember` runtime response keys 2026-04-14.
