...
**Status: done.** `approval-specific-quorum-ignores-specificapprovers`: `castVote` now refuses an unlisted user at `ensureEligibleToVote` (loads `specificApprovers` via `policyRepository.findPolicyById`) before a vote row is ever created; creation validation in `vaultPolicyService.ts` rejects `requiredApprovals > specificApprovers.length`. `vault-policy-all-quorum-requiredapprovals-static-not-membership`: `'all'` quorum is derived live from `walletSharingRepository.findWalletUsersWithUsername` (roles in `WALLET_APPROVE_ROLE_VALUES`, excluding the requester unless `allowSelfApproval`) both at request creation (`getEligibleApproverIds`) and at every resolution check (`checkAllQuorumMet`), so membership growth after creation no longer resolves early and a later removal no longer deadlocks. Tests: `server/tests/unit/services/approvalService/approvalService.quorum-membership.contracts.ts` (5 new cases: unlisted voter refused, listed voter resolves, all-quorum growth stays pending, all-quorum removal resolves, creation derives live `requiredApprovals`) plus one new creation-validation case in `vaultPolicyService.create-validation.contracts.ts`; two pre-existing cast-vote-resolution cases (`all`/`specific` quorum met) were updated because they pinned the buggy behavior — they now mock `findWalletUsersWithUsername`/`findPolicyById` to match live-membership semantics. Divergence: no repository method was added — reused the existing `walletSharingRepository.findWalletUsersWithUsername` rather than adding a by-role-by-wallet query. Gates: `tsc --noEmit`, `typecheck:tests`, `lint`, `lint:server`, `check:architecture-boundaries`, `check-large-files`, lizard (86 warnings, unchanged), `git diff --check` all green.

**Contract-gap follow-up (same day, append-only commit):** the initial landing only refused ineligible votes at `castVote` time; `checkAndResolveRequest`'s `'specific'` branch still counted `approveVotes.length` outright, so a vote persisted before this fix (or cast while listed, then dropped by a later policy edit) still counted toward `requiredApprovals`. Fixed by extracting `loadSpecificApprovers(policyId)` (shared by `ensureEligibleToVote` and the new `checkSpecificQuorumMet`) and filtering `approveVotes` to users currently on `specificApprovers` before comparing to `requiredApprovals` — always re-read live, never trusted from the vote history. Failing-first test (`specific quorum: resolution does not count an approve vote from a user outside specificApprovers`, in `approvalService.quorum-membership.contracts.ts`) failed pre-fix with `expected "vi.fn()" to not be called at all, but actually been called 1 times` (`resolveApprovalRequestIfPending(requestId, 'approved')`), confirming the stale vote wrongly resolved the request; a companion test proves resolution still succeeds once enough *listed* approvers vote. Gates re-run and green: `tsc --noEmit`, `typecheck:tests`, `lint:server`, `check:architecture-boundaries`, scoped coverage (100% stmts/branch/funcs/lines on both touched files), lizard (86 warnings, unchanged), `git diff --check`. Landed as a separate append-only commit (not an amend of the earlier ones).
...
Status: done (finding `totp-code-replay-within-tolerance-window`). Diverges from the contract above: no migration: consumed steps stored in RevokedToken because the RC replay gate pins the migration tree. A first attempt added `User.twoFactorLastUsedStep` via a new migration and shipped as PR #1106; Release Candidate Validation's "Wallet Sync Live Shape Replay" failed the sealed-manifest check (`scripts/perf/wallet-sync-high-fanout-replay.mjs`), which requires `server/prisma/migrations`' SHA-256 to equal `migrationTreeSha256` in `scripts/perf/wallet-sync-persistence-manifest.json` for both the checkout and the immutable RC10 image — so any new migration can never pass that gate. PR #1106 was closed and reworked without a schema change: `sessionRepository.consumeTotpStep(userId, timeStep)` inserts a one-time marker row into the existing `revoked_tokens` table (`jti: "totp-step:<userId>:<timeStep>"`, `reason: 'totp-step-consumed'`, `expiresAt` two time steps out); the primary-key insert is the atomic one-time boundary (P2002 on replay), and the existing `cleanupExpiredRevokedTokens` 5-minute sweep purges expired markers. `twoFactorService.verifyTokenStep(secret, token)` reports the matched RFC 6238 time step (no stored watermark to pass in); all four `verifyToken` call sites (`/2fa/enable`, `/2fa/disable`, `/2fa/backup-codes/regenerate`, `/2fa/verify` login) now call `verifyTokenStep` + `consumeTotpStep` instead of the old stateless `verifyToken` boolean check.
...
**Status: done.** Added `scripts/ci/integration-db-guard.mjs` (+ `.d.mts`) as the single shared guard, called from `check-integration-db.mjs`, `prepare-integration-db.sh` (before `prisma migrate deploy`, which reads `DATABASE_URL` directly), and `database.ts`; `resolve-postgres-service.sh` now bypasses the guard only inside its own authenticated `probe()` subshell and emits `SANCTUARY_ALLOW_INTEGRATION_DB_TARGET=1` via `ci_emit_env` alongside `DATABASE_URL` once a candidate is proven. New tests: `tests/ci/integration-db-guard.test.mjs` (unit), `tests/ci/check-integration-db-guard.test.sh` (entry-point wiring, red confirmed against HEAD), and extended `tests/ci/resolve-postgres-service.test.sh` (opt-in emission, red confirmed against HEAD). `.github/workflows/**` left untouched — the resolver alone emits the variable, matching the contract.
...
Status: done — `src/contexts/send/reducerParts/outputs.ts` `REMOVE_OUTPUT` now adjusts `scanningOutputIndex`; 4 new reducer tests in `tests/contexts/send/reducer/reducer.output.contracts.ts` confirmed red then green.
...
**Status: done.** Implemented the monotonic-generation contract in `HardwareWalletService.connect()`/`disconnect()` (captured at entry, before any await, so a `disconnect()` racing adapter loading is still detected) and independently in `LedgerAdapter` (checked just before committing `this.connection`, closing the transport on mismatch). BitBox now wraps the body after `new BitBox02API(...)` in a `finishConnect()` helper so every failure — including `readRootFingerprint` and `api.connect()` rejecting — closes the handle via one outer catch. Jade rejects an overlapping `connect()` up front (`Jade connect already in progress`) rather than using per-attempt resource capture, since only one attempt now ever holds `this.port`/`reader`/`writer`. All four failing-first tests (`hardwareWallet.service.test.ts`, `hardwareWallet.ledgerAdapter.test.ts`, `hardwareWallet.bitboxAdapter.test.ts` via `bitboxAdapter.connection.contracts.ts`, `hardwareWallet.jadeAdapter.test.ts`) were confirmed red before the fix and pass after; the full `tests/services/hardwareWallet*` suite (688 tests) plus typecheck/lint/lizard(86)/large-files/`git diff --check` gates are green. `scripts/quality/safety-catch-allowlist.json` counts bumped for the two new close-on-failure/close-on-cancel catches (bitbox `connect` 1→2, ledger `connect` 3→4).
