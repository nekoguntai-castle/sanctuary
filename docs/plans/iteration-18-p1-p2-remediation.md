...
Status: done. `cpfp.ts`: "UTXO not found" now throws `NotFoundError`; "UTXO is already spent"/"UTXO is frozen"/"UTXO is locked by a pending draft" now throw `InvalidInputError(msg, 'parentVout')`. `batch.ts`: "No spendable UTXOs available" and "Insufficient funds…" now throw `InvalidInputError(msg, 'utxos')`. Message text unchanged (existing `.rejects.toThrow(...)` contracts still pass unmodified). Internal invariants (wallet identity, missing scriptPubKey/spend-evidence) left as plain `Error`, per contract. Six service-level assertions added (4 CPFP, 2 batch) in `advancedTx.cpfp.contracts.ts` / `advancedTx.batch-fees.contracts.ts` asserting `toBeInstanceOf(NotFoundError|InvalidInputError)` plus message; four new route supertests added in `bitcoin.transaction.contracts.ts` (404 for CPFP not-found, 400 for CPFP already-spent, 400 for batch no-UTXOs, 400 for batch insufficient-funds). Proven red against `origin/main`'s `cpfp.ts`/`batch.ts` first (6 service-test failures, each asserting a plain `Error` was not the expected typed subclass), then green on the fix (`advancedTx.test.ts` 76/76, `bitcoin.test.ts` 139/139).

Adding the `ApiError` import shifted every line below it by +1 in both files (only single-line throw-statement swaps otherwise, so no other line-count changes). Re-pinned in `config/wallet-safety-mutation-map.json`'s `serverFeePolicy` profile: `advanced-batch-change-and-fee-selection` 115-134 → 116-135 (canary `advanced-batch-selection-loop-cannot-be-removed` 115-134 → 116-135; canary `advanced-batch-surplus-cannot-widen-tolerance` line 131 → 132); `cpfp-package-fee-is-conserved` 53-61 → 54-62 (canary `cpfp-child-fee-subtracts-parent-fee` line 57 → 58). `rbf.ts` untouched, no shift. `server/stryker.fee-policy.config.mjs` mutate ranges realigned with the same margins as before (batch.ts: pin −25/+6 → 90-140 was already stale from a prior drift; now 91-141; cpfp.ts: pin −18/+6 → 36-68).

`cd server && npm run test:mutation:fee-policy`: `batch.ts` 94.74% (36/38 killed, 2 pre-existing survivors outside the touched throw sites), `cpfp.ts` 100.00% (8/8), `rbf.ts` 100.00% (9/9), all above their 75-point file floors; overall profile 90.51% (break threshold 75). `check-wallet-safety-mutation-map.mjs`'s `validateMutationEvidence` was run in isolation against just the `serverFeePolicy` profile and its fresh report (same approach as iteration-17 Phase 1, since the full CLI needs all six profiles' reports) — passed. Gates: server `tsc --noEmit` and `typecheck:tests` clean; root `typecheck:app`/`typecheck:tests`/`typecheck:all` clean; root `npm run lint` clean; `check:architecture-boundaries` passed (2417 files, 13 rules, 45 exceptions); `check-large-files.mjs` passed; `lizard-only.sh` exactly 86 warnings; `git diff --check` clean.
...
Status: done. Implemented in worktree `/home/nekoguntai/sanctuary-wt-i18-phase6` on branch `codex/bug-scrub-loop/i18-phase6`. `boundEntrySize` (`server/src/services/deadLetterQueue.ts`) drops the redundant `payload.data` display copy first and only replaces `payload.data`/`job.data` with a `{truncated, originalBytes, preview}` summary when still oversized after that; `validateEntrySize` stays as the last-resort throw. `eventHandlers.ts`'s DLQ write failure now logs at `log.error` with `jobId`/`queue` context instead of `log.debug`. Tests added: `server/tests/unit/services/deadLetterQueue.test.ts` (moderately oversized sync entry — display copy dropped, `job.data` intact and `isSyncWalletEnvelope` still true; a >256 KiB single-copy entry — both fields replaced with a bounded summary, no full payload leaked); `server/tests/unit/worker/workerJobQueue/workerJobQueue.internal-events.contracts.ts` (asserts `log.error` with jobId/queue/error on a DLQ write failure); new `server/tests/unit/worker/deadLetterReconcilerTruncation.test.ts` (runs the reconciler against the real `DeadLetterQueue` singleton — an oversized job is reconciled without throwing and is not reprocessed on the next cycle, proving it is no longer relost forever). Proved red by restoring `origin/main`'s `deadLetterQueue.ts` and `eventHandlers.ts` and rerunning the four new/changed tests (all failed), then restored the fix (all green). Gates green: server `tsc --noEmit`, `typecheck:tests`, root `lint`, `check:architecture-boundaries`, `check-large-files.mjs`, `lizard-only.sh` (86 warnings, unchanged), `git diff --check`, and the targeted vitest suites (deadLetterQueue, deadLetterReconciler(+Truncation), workerJobQueue, deadLetterQueueProvider, deadLetterStores, admin-infrastructure-routes, syncDeadLetterRetryAdmission, supportPackage collectors, push/sync services) all pass. No migrations; server-only change.
