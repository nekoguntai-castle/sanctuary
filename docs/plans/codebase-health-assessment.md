...
| --- | --- | --- | --- |
| P1 | Keep reducing lizard warnings, but prioritize production source before tests, E2E fixtures, benchmark scripts, and animations. | `gateway/src/services/backendEvents/notifications.ts`, `services/deviceParsers/parsers/coldcardNested.ts`, `services/deviceParsers/parsers/keystone.ts`, and `utils/urDeviceDecoder.ts` are now complete. Continue the same one-warning-per-commit cadence on `utils/urPsbt.ts`, `contexts/send/reducer.ts`, and `hooks/send/*`. | Each slice removes exactly the targeted lizard warning, lowers `.github/workflows/quality.yml` and `scripts/quality.sh` by one, updates `tasks/grade-report-2026-04-13-c76.md`, and passes the focused tests, target lizard check, `npm run typecheck:app`, and `npm run lint`. |
...
Practical next patch: refactor `utils/urPsbt.ts` by extracting PSBT decoding format detection and decoder helpers while keeping the exported utility API unchanged. Run `npx vitest run tests/utils/urPsbt.test.ts`, a target lizard check for `utils/urPsbt.ts`, `npm run typecheck:app`, and `npm run lint`.
