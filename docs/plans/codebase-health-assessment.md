...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 52 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| --- | --- | --- | --- |
| tests | pass; app 401 files/5,590 tests; backend 390 passed/22 skipped files with 9,151 passed/503 skipped tests after current remediation; gateway 20 files/513 tests | `grade.sh`; `npm run test:coverage`; `npm run test:backend:coverage`; `npm run test:coverage` in `gateway` | Correctness 1.1 |
...
| lizard_warning_count | 52 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.3 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 33; current top production JSX components are 29 CCN (`UTXOList`, `ConnectDevice`, `CoinControlPanel`, `EncryptionKeyDisplay`) | lizard warning scan | Maintainability 3.1 context |
| duplication_pct | 2.01%; 276 clones, 5,289 duplicated lines | `npx --yes jscpd@4 .` | Maintainability 3.2 |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 52 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 52 functions above CCN 15, with the top production JSX components still reaching 29 CCN.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `UTXOList`, `ConnectDevice`, `CoinControlPanel`, or `EncryptionKeyDisplay` as the next 29-CCN JSX starting points.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` completed after current remediation: 52 warnings, average CCN 1.3, max CCN 33.
- `npx --yes jscpd@4 .` completed: 2.01% duplication, 276 clones, 5,289 duplicated lines.
- `npm run test:coverage` passed after current remediation: 401 app test files, 5,590 tests, 100% statements/branches/functions/lines.
