...
Practical next patch: refactor `apiRouteHandler` in `e2e/admin-drafts-smoke.spec.ts` by moving mocked API route responses into a route table/helper while preserving the admin draft smoke behavior. Run a target lizard check for `e2e/admin-drafts-smoke.spec.ts`, the focused Playwright admin drafts smoke, `npm run typecheck:app`, and `npm run lint`.
