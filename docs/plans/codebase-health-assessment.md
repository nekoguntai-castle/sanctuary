...
Practical next patch: refactor `drawPaperAirplane` and the large frame callback in `components/animations/paperAirplanes.ts` by moving drawing and frame phases into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/paperAirplanes.ts`, `npm run typecheck:app`, and `npm run lint`.
