...
Practical next patch: refactor `createMockUtxo` in `server/tests/unit/services/bitcoin/utxoSelection.test.ts` by replacing branch-heavy fallback construction with a default fixture merge while preserving UTXO selection fixture defaults. Run the targeted Vitest file, a target lizard check for that test file, `npm run typecheck:app`, and `npm run lint`.
