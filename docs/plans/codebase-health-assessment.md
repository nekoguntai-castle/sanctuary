...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative load/scale items are carried forward after local worker queue, synthetic large-wallet history, backup restore, backend scale-out, and worker scale-out smoke proof
...
- Keep the local auto-provisioned Compose smoke as endpoint-coverage evidence; it now includes a synthetic 1,000-transaction wallet-history gate, but still does not replace a production-like largest-known-wallet dataset or load-level fanout run.
...
- Validate backend and worker scale-out capacity with representative non-production load-balanced and queue-load topologies; the local two-backend Redis WebSocket smoke and two-worker BullMQ/Electrum ownership smoke are now recorded.
...
- Use the disposable Compose smoke for repeatable two-backend Redis WebSocket and two-worker BullMQ/Electrum ownership regression proof; stand up or identify representative non-production load-balanced and worker queue-load topologies for capacity evidence.
