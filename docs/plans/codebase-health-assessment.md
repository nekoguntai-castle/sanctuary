...
Practical next patch: refactor the warning in `server/tests/unit/worker/jobs/maintenanceJobs.test.ts` by moving the branch-heavy event assertions or event fixture matching out of the anonymous test callback. Run the targeted Vitest file, a target lizard check for that test file, `npm run typecheck:app`, and `npm run lint`.
