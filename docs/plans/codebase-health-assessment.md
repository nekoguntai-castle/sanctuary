...
Practical next patch: refactor `apiRouteHandler` in `e2e/settings-persistence.spec.ts` by moving mocked API route responses into a route table/helper while preserving the settings persistence coverage. Run a target lizard check for `e2e/settings-persistence.spec.ts`, the focused Playwright settings persistence spec, `npm run typecheck:app`, and `npm run lint`.
