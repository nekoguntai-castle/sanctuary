...
Practical next patch: refactor `hooks/send/useUsbSigning.ts` by extracting PSBT selection, descriptor parsing, device-signing, signature tracking, and draft persistence helpers while keeping the hook API unchanged. Run focused USB-signing hook tests, a target lizard check for `hooks/send/useUsbSigning.ts`, `npm run typecheck:app`, and `npm run lint`.
