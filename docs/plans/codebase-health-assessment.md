...
Practical next patch: refactor `apiRouteHandler` in `e2e/admin-operations.spec.ts` by moving mocked API route responses into a route table/helper while preserving the admin operations smoke coverage. Run a target lizard check for `e2e/admin-operations.spec.ts`, the focused Playwright admin operations spec, `npm run typecheck:app`, and `npm run lint`.
