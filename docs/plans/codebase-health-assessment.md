...
Practical next patch: refactor `drawBunny` in `components/animations/bunnyMeadow.ts` by moving drawing branches into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/bunnyMeadow.ts`, `npm run typecheck:app`, and `npm run lint`.
