...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative benchmark items are carried forward
...
- Phase 3 local smoke benchmark records in `docs/plans/phase3-benchmark-2026-04-12T04-00-40-678Z.md`, `docs/plans/phase3-benchmark-2026-04-12T05-12-14-935Z.md`, `docs/plans/phase3-benchmark-2026-04-13T00-06-30-381Z.md`, and `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-04-57-821Z.md`.
...
| Performance | B | Caching, React Query discipline, Electrum pooling, database indexes, API aggregation, bounded WebSocket queues, and a Phase 3 benchmark harness are good. A disposable full-stack authenticated smoke now covers wallet list, transaction history, wallet-sync queueing, WebSocket handshake, and backup validation; representative large-wallet, WebSocket fanout with events, restore, worker queue, and scale-out benchmarks are still pending. |
...
| P2 | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end, but generated fixture data does not prove production-like performance. Load evidence would catch regressions in wallet sync, large wallet transaction history, WebSocket fanout, backup/restore, and worker queues. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-04-57-821Z.md` records the full-stack local authenticated smoke, while representative large-wallet and scale-out evidence remain pending. |
...
- Added `npm run perf:phase3:compose-smoke`, backed by `scripts/perf/phase3-compose-benchmark-smoke.mjs`, to start a disposable full-stack Compose project, wait for migration/seed and service health, run the existing Phase 3 harness in strict local-fixture mode, assert authenticated wallet list, transaction-history, wallet-sync queue, and admin backup-validation scenarios, then tear the stack down.
...
- Run and record representative large-wallet benchmark results for wallet sync, large wallet transaction history, WebSocket fanout with subscriptions/events, backup restore in non-production, and worker queue processing.
- Keep the local auto-provisioned Compose smoke as endpoint-coverage evidence; record a separate representative large-wallet run because the local generated wallet proves end-to-end behavior but not scale characteristics.
...
Phase 3 Compose benchmark smoke passed against a temporary frontend/backend/gateway/worker/Postgres/Redis stack: migration/seed, service health, frontend/API/gateway health, WebSocket handshake, local authenticated wallet list, transaction-history, wallet-sync queueing, and admin backup validation passed. Evidence: `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-04-57-821Z.md` and `docs/plans/phase3-benchmark-2026-04-13T00-06-30-381Z.md`.
...
- Dependency audits.
- Phase 3 representative large-wallet benchmark, WebSocket subscription/fanout proof, worker queue benchmark, restore-safe backup restore benchmark, and backend scale-out smoke.
