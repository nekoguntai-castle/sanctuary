...
| --- | --- | --- | --- |
| P1 | Keep reducing lizard warnings, but prioritize production source before tests, E2E fixtures, benchmark scripts, and animations. | `gateway/src/services/backendEvents/notifications.ts`, `services/deviceParsers/parsers/coldcardNested.ts`, `services/deviceParsers/parsers/keystone.ts`, `utils/urDeviceDecoder.ts`, `utils/urPsbt.ts`, `utils/utxoAge.ts`, `services/hardwareWallet/adapters/trezor/pathUtils.ts`, `services/hardwareWallet/adapters/jade.ts`, and `hooks/send/useSendTransactionActions.ts` are now complete. Continue the same one-warning-per-commit cadence on `hooks/send/useDraftManagement.ts`, `contexts/send/reducer.ts`, and the remaining `hooks/send/*` warnings. | Each slice removes exactly the targeted lizard warning, lowers `.github/workflows/quality.yml` and `scripts/quality.sh` by one, updates `tasks/grade-report-2026-04-13-c76.md`, and passes the focused tests, target lizard check, `npm run typecheck:app`, and `npm run lint`. |
...
Practical next patch: refactor `hooks/send/useDraftManagement.ts` by extracting draft payload construction and persistence helpers while keeping the hook API unchanged. Run focused draft-management hook tests, a target lizard check for `hooks/send/useDraftManagement.ts`, `npm run typecheck:app`, and `npm run lint`.
