...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 54 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| lizard_warning_count | 54 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.3 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 33; current top production JSX component is `ConfigurableTable` at 30 | lizard warning scan | Maintainability 3.1 context |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 54 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 54 functions above CCN 15, with the top JSX component still reaching 30 CCN.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `ConfigurableTable` as the clear next starting point among 30-CCN JSX components.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` completed after current remediation: 54 warnings, average CCN 1.3, max CCN 33.
