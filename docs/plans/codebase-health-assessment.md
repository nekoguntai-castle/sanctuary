...
Practical next patch: refactor the `build` helper in `server/tests/integration/repositories/setup.ts` by moving repeated optional fixture defaults into focused helpers while preserving integration repository fixture behavior. Run a target lizard check for `server/tests/integration/repositories/setup.ts`, the focused repository integration tests, `npm run typecheck:app`, and `npm run lint`.
