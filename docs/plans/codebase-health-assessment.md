...
Practical next patch: refactor `contexts/send/reducer.ts` by extracting action-specific transaction state reducers while keeping the reducer contract unchanged. Run focused send reducer tests, a target lizard check for `contexts/send/reducer.ts`, `npm run typecheck:app`, and `npm run lint`.
