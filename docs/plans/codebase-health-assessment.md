...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 57 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| --- | --- | --- | --- |
| tests | pass; app 401 files/5,587 tests; backend 390 passed/22 skipped files with 9,151 passed/503 skipped tests after current remediation; gateway 20 files/513 tests | `grade.sh`; `npm run test:coverage`; `npm run test:backend:coverage`; `npm run test:coverage` in `gateway` | Correctness 1.1 |
...
| lizard_warning_count | 57 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.3 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 33; current top production JSX component is `LogTab` at 31 | lizard warning scan | Maintainability 3.1 context |
| duplication_pct | 2.02%; 277 clones, 5,300 duplicated lines | `npx --yes jscpd@4 .` | Maintainability 3.2 |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 57 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 57 functions above CCN 15, with the top JSX component still reaching 31 CCN.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `LogTab` as the clear next starting point among 31-CCN JSX components.
...
| 1 | Reduce complexity concentration | Extract top JSX decision/render helpers from the highest CCN components. | lizard warnings trend below 15. | `+1` to `+3`; `WalletStats`, `TransactionRow`, `TransactionList`, `NetworkConnectionCard`, `AddressesTab`, `DeviceDetail`, `SharingSection`, `OwnershipSection`, `TransferOwnershipModal`, `ManualAccountForm`, `AddAccountFlow`, `QrImport`, `QRSigningModal`, `DeviceDetailsForm`, `NetworkSyncActions`, `Monitoring`, `WalletDetail`, `WalletTelegramSettings`, `AppRoutes`, `LoginForm`, `AILabelSuggestion`, `ChatTab`, `UsersGroups`, `AIQueryInput`, `Layout`, `SidebarContent`, `PriceChart`, `RestorePanel`, `Variables`, `UTXOGarden`, `UTXORow`, `AgentWalletDashboard`, `QrScannerPanel`, `DraftList`, `LabelSelector`, `SendTransactionPage`, `LabelManager`, `NotificationToast`, `NotificationPanel`, `BlockVisualizer`, `ThemeSection`, `Account`, and `AgentManagement` extractions have started this trend. |
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` completed after current remediation: 57 warnings, average CCN 1.3, max CCN 33.
- `npx --yes jscpd@4 .` completed: 2.02% duplication, 277 clones, 5,300 duplicated lines.
- `npm run test:coverage` passed after current remediation: 401 app test files, 5,587 tests, 100% statements/branches/functions/lines.
