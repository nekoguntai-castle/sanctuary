...
Practical next patch: refactor the large frame callback in `components/animations/lavenderFields.ts` by moving field, bee, butterfly, firefly, particle, and frame phases into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/lavenderFields.ts`, `npm run typecheck:app`, and `npm run lint`.
