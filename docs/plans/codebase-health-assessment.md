...
Practical next patch: refactor `validateDraftResponse` in `server/tests/helpers/contractValidation.ts` using the same rule-table and item-validator pattern while preserving contract validation behavior. Run a target lizard check for `server/tests/helpers/contractValidation.ts`, `npx vitest run --config server/vitest.config.ts server/tests/contract/api.contract.test.ts`, `npm run typecheck:app`, and `npm run lint`.
