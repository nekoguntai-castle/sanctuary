...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 86 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
- Current remediation: `WalletStats`, `TransactionRow`, `TransactionList`, `LabelEditor`, `FlowPreview`, `NetworkConnectionCard`, `ServerForm`, `DraftList`, `LabelSelector`, `SendTransactionPage`, `LabelManager`, `NotificationToast`, `NotificationPanel`/`NotificationItem`, `BlockVisualizer`, `Block`, `QueuedSummaryBlock`, `PendingTxDot`, `AppearanceTab`, `BackgroundsPanel`, `Account`, and `AgentManagement` are no longer top lizard findings; warning-band file pressure dropped by splitting dashboard repository tests, AgentManagement views, TransactionList helpers, and NetworkConnectionCard controller/form helpers, and gitleaks now has a pinned project-tooling path.
...
| --- | --- | --- | --- |
| tests | pass; app 397 files/5,561 tests; backend 390 passed/22 skipped files with 9,151 passed/503 skipped tests after current remediation; gateway 20 files/513 tests | `grade.sh`; `npm run test:coverage`; `npm run test:backend:coverage`; `npm run test:coverage` in `gateway` | Correctness 1.1 |
...
| lizard_warning_count | 86 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 53; current top JSX components are `AddressesTab`, `DeviceDetail`, `QRSigningModal`, and `DeviceDetailsForm` | lizard CSV sort | Maintainability 3.1 context |
...
| test_file_count | 1,197 | `grade.sh` heuristic | Test Quality 6.2 |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 86 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 86 functions above CCN 15, led by `components/WalletDetail/tabs/AddressesTab.tsx`, `components/DeviceDetail/DeviceDetail.tsx`, `components/qr/QRSigningModal.tsx`, and `components/ConnectDevice/DeviceDetailsForm.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `AddressesTab`, `DeviceDetail`, `QRSigningModal`, and `DeviceDetailsForm` as clear next starting points.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 86 warnings, average CCN 1.4, max CCN 53.
- `npx --yes jscpd@4 --silent --reporters json --output /tmp/sanctuary-jscpd-grade-20260419 .` completed: 2.1% duplication.
- `npm run test:coverage` passed after current remediation: 397 app test files, 5,561 tests, 100% statements/branches/functions/lines.
