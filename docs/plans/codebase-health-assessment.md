...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 96 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
- Current remediation: `WalletStats`, `TransactionRow`, `DraftList`, `LabelSelector`, `SendTransactionPage`, `LabelManager`, `NotificationToast`, `NotificationPanel`/`NotificationItem`, `BlockVisualizer`, `Block`, `QueuedSummaryBlock`, and `PendingTxDot` are no longer top lizard findings; warning-band file pressure dropped by splitting dashboard repository tests, and gitleaks now has a pinned project-tooling path.
...
| lizard_warning_count | 96 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 59; current top JSX components are `AppearanceTab`, `Account`, `AgentManagement` callbacks, and `TransactionList` | lizard CSV sort | Maintainability 3.1 context |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 96 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 96 functions above CCN 15, led by `components/Settings/sections/ThemeSection/AppearanceTab.tsx`, `components/Account/Account.tsx`, `components/AgentManagement/index.tsx`, and `components/TransactionList/TransactionList.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `AppearanceTab`, `Account`, `AgentManagement`, and `TransactionList` as clear next starting points.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 96 warnings, average CCN 1.4, max CCN 59.
