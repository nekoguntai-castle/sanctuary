...
Owner: TBD
Status: Refreshed assessment; the `/grade` implementation-adjusted score is 92/100 (A-) after the privacy-safe generated capacity proof, Phase 3 repository-controlled generated-data capacity proof is complete, production-like runtime review is recorded, Phase 4 test typecheck and dependency-audit triage gates are current, the lizard baseline is clean at zero warnings, the secret-scan false-positive cleanup is complete for latest-commit, tracked-tree, and full-history scans, and the safe root dependency-audit maintenance item is complete
...
- Latest Phase 3 generated capacity profile in `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md`, with underlying benchmark output in `docs/plans/phase3-benchmark-2026-04-15T00-12-49-889Z.md`.
...
- Production-like runtime review in `docs/plans/phase2-production-like-runtime-review-2026-04-15T00-12-26-208Z.md`.
...
- Real production telemetry review beyond the recorded production-like Compose runtime window.

## Executive Summary

Overall grade: **B+** overall, with the implementation-adjusted `/grade` score now at **92/100 (A-)**. The gap between those labels is deliberate: the repository-controlled performance/runtime proof is now strong, but durable production alert delivery still depends on a real external notification channel.
...
The main risk is no longer broad architecture quality or known P0 correctness defects. The remaining path to durable A grades is centered on proof and drift prevention: keep OpenAPI/shared schema coverage current, keep route-backed gateway whitelist checks green, repeat the generated capacity profile when target topology changes, wire durable production alert receivers, attach real deployment telemetry when available, and continue browser token/CSP hardening.
...
| --- | --- | --- | --- |
| Done | Reduce the lizard warning baseline to zero. | `gateway/src/services/backendEvents/notifications.ts`, `services/deviceParsers/parsers/coldcardNested.ts`, `services/deviceParsers/parsers/keystone.ts`, `utils/urDeviceDecoder.ts`, `utils/urPsbt.ts`, `utils/utxoAge.ts`, `services/hardwareWallet/adapters/trezor/pathUtils.ts`, `services/hardwareWallet/adapters/jade.ts`, `hooks/send/useSendTransactionActions.ts`, `hooks/send/useDraftManagement.ts`, `hooks/send/useQrSigning.ts`, `hooks/send/useUsbSigning.ts`, `contexts/send/reducer.ts`, `scripts/verify-addresses/generate-vectors.ts`, the E2E API harnesses, `components/animations/paperBoats.ts`, `components/animations/bunnyMeadow.ts`, `components/animations/trainStation.ts`, `components/animations/fireworks.ts`, `components/animations/ducklingParade.ts`, `components/animations/paperAirplanes.ts`, `components/animations/lavenderFields.ts`, `components/animations/butterflyGarden.ts`, `components/WalletDetail/hooks/useTransactionFilters.ts`, and `components/WalletDetail/hooks/useAITransactionFilter.ts` are now complete. | Repo-wide lizard passes with `-i 0`; `.github/workflows/quality.yml` and `scripts/quality.sh` now enforce a zero-warning baseline. |
| Done | Clean up the secret-scan signal without weakening real secret detection. | `.gitleaks.toml` now has explicit fixture/test placeholder allowlists, `.gitleaksignore` only suppresses four historical ignored-artifact fingerprints, and `scripts/gitleaks-tracked-tree.sh` scans tracked files without local ignored artifacts such as `.env`, coverage, dist, or generated SSL keys. | `/tmp/gitleaks git . --config .gitleaks.toml --redact --no-banner --log-opts -1`, `GITLEAKS_BIN=/tmp/gitleaks bash scripts/gitleaks-tracked-tree.sh`, and full-history `/tmp/gitleaks git . --config .gitleaks.toml --redact --no-banner` all pass. |
| P2 | Keep the zero-warning architecture gate simple unless noise returns. | The repo-wide lizard gate is now clean at zero warnings, so a separate production-source complexity gate is no longer needed for the current backlog. Revisit only if future E2E fixtures, benchmark scripts, or generated-output-adjacent artifacts start obscuring product architecture work again. | `scripts/quality.sh` and `.github/workflows/quality.yml` keep the zero-warning lizard gate blocking; any future split should preserve that no-regression signal. |
| Done | Record privacy-safe generated performance and scale evidence. | The strongest valid generated profile now covers 25,000 synthetic transactions, 100 authenticated history requests at concurrency 10, a generated restore, queue proof, worker scale-out, backend scale-out, and 100 WebSocket clients. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md` and `docs/plans/phase3-benchmark-2026-04-15T00-12-49-889Z.md` record p95/p99, failure counts, dataset/topology notes, and sensitive-data exclusion. |
| Done | Record production-like runtime evidence. | The full-stack generated capacity run provides a clean runtime window with healthy containers, 0 failed benchmark scenarios, 0 failed proof jobs, and recorded capacity snapshots. | `docs/plans/phase2-production-like-runtime-review-2026-04-15T00-12-26-208Z.md` records the runtime review; real production telemetry should still be attached when an operator-controlled deployment exists. |
| P2 | Keep the remaining A-grade operations evidence moving outside the lizard loop. | Durable production alert receiver proof still requires a real notification channel and credentials; mock/local webhook delivery is already covered but does not prove external delivery. | Evidence should be recorded in `docs/plans/` with alert delivery proof, channel owner, secret-free config notes, and sensitive data excluded. |
| Done | Run routine dependency-audit maintenance separately from architecture refactors. | The non-forced `npm audit fix` path updated `package-lock.json` to `follow-redirects@1.16.0`; root production audit now has 14 low upstream `elliptic` findings and 0 moderate/high/critical findings. Accepted Prisma/polyfill chains stay documented separately. | `npm audit --omit=dev --audit-level=moderate` exits clean; `docs/DEPENDENCY_AUDIT_TRIAGE.md` documents the remaining accepted findings and breaking/no-fix paths. |

Practical next patch: choose the durable production alert receiver channel and record a production-safe delivery proof without committing secrets.
...
| Scalability | A- | Dedicated worker, BullMQ/Redis, distributed locks, WebSocket limits, Redis bridge broadcasts, Prisma indexes, cache invalidation, and generated scale-out evidence are now strong. The current profile proves Redis-backed WebSocket delivery to 100/100 clients across two backend replicas, BullMQ diagnostic processing on two worker replicas, recurring job deduplication, one Electrum subscription owner after retry recovery, a shared-lock skip, 30 worker proof jobs, and Postgres/Redis capacity snapshots before and after 25,000 generated transactions. Production scale-out still needs a target-environment rerun when topology or hardware differs from local Compose. |
| Performance | A- | Caching, React Query discipline, Electrum pooling, database indexes, API aggregation, bounded WebSocket queues, and the Phase 3 benchmark harness are backed by current generated-data evidence. The disposable full-stack authenticated capacity profile covers wallet list, transaction history, a synthetic 25,000-transaction wallet-history gate with 100 requests at concurrency 10 and p95 70 ms, wallet-specific WebSocket sync fanout, wallet-sync queueing, WebSocket handshake, generated backup validation and 25,076-record restore, worker queue proof, two-worker diagnostic scale-out, and 100-client two-backend Redis WebSocket fanout. Target-environment thresholds still need recalibration when deployment hardware or supported backup size changes. |
| Perpetual operations and supportability | B+ | `/health`, `/api/v1/health`, `/metrics`, Prometheus alerts, Grafana/Loki/Jaeger, support-package collectors, Docker healthchecks, resource limits, monitoring exposure docs, and operations runbooks are strong. A disposable restore drill, full Compose gateway audit persistence proof, local monitoring stack smoke, disposable Alertmanager receiver delivery smoke, and production-like runtime review now exist. Durable production external alert receiver delivery is still missing. |
...
2. Finish Phase 2 production alert receiver selection and proof before claiming durable A-grade operations.
3. Treat the Phase 3 generated capacity proof as complete for repository-controlled evidence, and rerun it only when release scope or target topology changes.
...
## Outstanding And Recently Closed Items

These are the remaining known blockers and the items closed in this follow-up. Repository-controlled proof is complete for Phase 2/3/4 where noted; the remaining item requires a real production-channel choice.

| Status | Area | Item | Required evidence / exit criteria |
| --- | --- | --- | --- |
| Pending | Phase 2 operations | Choose durable external production alert receiver channels and credentials. | Alertmanager/receiver configuration is committed without secrets, credential ownership is documented, and a production-safe test alert is delivered to the chosen channel. The disposable local webhook receiver remains smoke evidence only. |
| Done | Phase 2 operations | Capture production-like runtime evidence. | `docs/plans/phase2-production-like-runtime-review-2026-04-15T00-12-26-208Z.md` records a clean full-stack runtime window with healthy final containers, 0 failed benchmark scenarios, 0 failed proof jobs, capacity snapshots, and sensitive data excluded. |
| Done | Phase 3 scalability/performance | Run privacy-safe generated load benchmarks. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md` records wallet sync, transaction history, WebSocket fanout, worker queue processing, backend scale-out, worker scale-out, and backup validation/restore using generated fixtures only. It does not use third-party wallets, public wallet histories, or private user wallet activity. |
| Done | Phase 3 topology | Validate generated two-backend/two-worker topology under expected local capacity. | The latest profile records 25,000 generated transactions, 100 history requests at concurrency 10, p95 70 ms, p99 77.02 ms, 100/100 WebSocket fanout across two backend replicas, two-worker proof, p95 16.2 ms worker jobs, 16.3 MiB restore, and Postgres/Redis capacity snapshots. |
| Ongoing | Security/dependencies | Monitor accepted dependency audit findings. | No unaccepted high/critical production advisories remain; revisit root hardware-wallet/polyfill chains, server Prisma tooling, and gateway optional dependency findings when same-major, non-downgrade fixes are available. |
| Ongoing | Phase 4 hygiene | Keep contract, validation, redaction, and release-gate guardrails current. | New or touched routes keep OpenAPI/shared-schema/gateway whitelist coverage; gateway logs continue to use shared redaction; large-file cleanup stays tied to files already being changed. |
| Ongoing | Release verification | Run remaining release-scope checks when cutting a release or touching covered areas. | Full backend/gateway coverage, backend integration, Playwright e2e, install/container workflows, and the critical mutation gate are rerun according to `docs/RELEASE_GATES.md`. |
...
| P1 | Complete operations proof, not just operations docs. | Non-production restore, full Compose gateway audit persistence, local monitoring-stack proof, disposable Alertmanager webhook receiver delivery proof, and a production-like runtime review now exist. Durable production external alert receiver delivery still needs a real channel and credentials. | `docs/OPERATIONS_RUNBOOKS.md` maps alerts and failure modes to triage; `docs/plans/phase2-operations-proof-2026-04-12T08-44-39-1000.md` records backup/restore plus in-process gateway audit proof; `docs/plans/phase2-monitoring-smoke-2026-04-12T22-42-59-008Z.md` records monitoring stack proof with loopback bindings; `docs/plans/phase2-gateway-audit-compose-smoke-2026-04-12T23-18-24-249Z.md` records the multi-container backend/gateway HMAC audit proof; `docs/plans/phase2-alert-receiver-smoke-2026-04-12T23-33-46-561Z.md` records local Alertmanager receiver delivery proof; `docs/plans/phase2-production-like-runtime-review-2026-04-15T00-12-26-208Z.md` records the clean runtime window. |
| Done | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end with a stronger generated profile: a synthetic 25,000-transaction wallet-history gate, 100 history requests at concurrency 10, a generated 25,076-record backup restore, wallet-specific WebSocket sync fanout, 30 worker proof jobs, two-worker diagnostic scale-out, 100-client two-backend Redis WebSocket fanout, and Postgres/Redis capacity snapshots. Generated fixture data proves repository-controlled regressions; target-environment reruns are required only when the release claims deployment-specific capacity. Third-party real-world wallet usage and public wallet-history profiling are out of scope. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md` records the latest full-stack generated capacity profile, with benchmark output in `docs/plans/phase3-benchmark-2026-04-15T00-12-49-889Z.md`. |
| Done | Validate the supported generated scale-out topology. | Redis bridge, distributed locks, worker queues, health checks, and scale-out docs now have generated proof at the supported local ceiling. Production worker replica support should still be gated by a current target-environment run. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md` proves a wallet-specific sync event reached 100/100 WebSocket clients across two backend replicas through Redis with p95 23 ms, and proves two worker replicas process diagnostic BullMQ jobs while only one owns Electrum subscriptions. `docker-compose.yml` forwards `MAX_WEBSOCKET_PER_USER` and `MAX_WEBSOCKET_CONNECTIONS`; `docs/SCALABILITY_AND_PERFORMANCE.md` records the backend `MAX_WEBSOCKET_PER_USER <= 100` schema ceiling. |
...
Status: **Restore, gateway audit, monitoring stack, local alert receiver proof, and production-like runtime review complete; production external alert receiver proof pending**
...
Completed in the 2026-04-14 production-like runtime review slice:

- Recorded `docs/plans/phase2-production-like-runtime-review-2026-04-15T00-12-26-208Z.md` from the strongest valid generated-data full-stack Compose benchmark.
- The runtime review records that the final topology had two backend containers, two worker containers, frontend, gateway, PostgreSQL, and Redis healthy; authenticated benchmark scenarios recorded 0 errors; queue proof counts recorded 0 failed jobs; and the disposable stack was removed after the proof.
- This closes the "no production/prod-like runtime evidence" gap for repository-controlled proof. A real production telemetry or incident review should still be attached when an operator-controlled deployment exists.
...
Status: **Repository-controlled generated capacity proof complete as of 2026-04-14 (Pacific/Honolulu); target-environment rerun gates production scale-out claims**
...
- Fixed the generated backend scale-out proof script after the extracted helper set missed `extractAccessTokenFromSetCookie(...)`; the proof now parses the HttpOnly login cookie inside the inline backend-scale script the same way the top-level harness does.
- Added upfront benchmark-profile validation for the backend `MAX_WEBSOCKET_PER_USER <= 100` schema ceiling so oversized generated fanout runs fail clearly before container startup.
- Regenerated the root lockfile with npm 10 in the Node 22 Alpine Docker image so optional peer entries under `@bitcoinerlab/descriptors` are present and container `npm ci` succeeds.
- Recorded the stronger repository-controlled generated capacity profile in `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md`, with underlying benchmark evidence in `docs/plans/phase3-benchmark-2026-04-15T00-12-49-889Z.md`. The run inserted 25,000 synthetic transactions, passed 100 authenticated transaction-history requests at concurrency 10 with p95 70 ms and p99 77.02 ms, created/validated/restored a 16.3 MiB generated backup with 25,076 records in 12,533 ms, completed 30 worker proof jobs with p95 16.2 ms, proved two-worker diagnostic processing and one Electrum owner, delivered the Redis-bridged sync event to 100/100 WebSocket clients across two backend replicas with p95 23 ms, and captured capacity snapshots from 10.01 MiB Postgres/2 MiB Redis at baseline to 46.14 MiB Postgres/2.98 MiB Redis after the generated load profile.

Target-environment evidence still required before broadening production scale-out support:

- Repeat and record the generated-data profile, or an operator-owned testnet/regtest equivalent, on the target non-production topology when hardware, load balancer, Postgres, Redis, or worker sizing differs from local Compose.
- Keep the local auto-provisioned Compose smoke as endpoint-coverage and generated capacity evidence; it now includes a synthetic 25,000-transaction wallet-history gate, 100-client two-backend Redis WebSocket fanout, 30 worker proof jobs, generated 25,076-record backup restore, two-worker ownership proof, and Postgres/Redis capacity snapshots.
- Calibrate strict release thresholds after the first repeatable target-topology run.
- Validate backend and worker scale-out capacity with privacy-safe target topology runs before broadening production replica support beyond the generated proof boundary.
...
- Obtain a privacy-safe non-production `SANCTUARY_TOKEN` and `SANCTUARY_WALLET_ID` for target-environment transaction history and wallet sync queue benchmarks, using generated/regtest data or an operator-owned testnet wallet created for this purpose.
- Obtain a non-production `SANCTUARY_ADMIN_TOKEN` and approved `SANCTUARY_BACKUP_FILE` for backup validation and restore timing when the intended support-size backup is larger than the generated 16.3 MiB proof, with `SANCTUARY_ALLOW_RESTORE=true` only in a restore-safe environment.
- Use the disposable Compose smoke for repeatable generated capacity and regression proof, including two-backend Redis WebSocket fanout, two-worker BullMQ/Electrum ownership, synthetic transaction-history, generated backup restore, worker queue repeats, and capacity snapshots; stand up or identify privacy-safe non-production load-balanced and worker queue-load topologies when a release claims deployment-specific capacity.
...
- Phase 4 should reference the Phase 3 generated capacity proof as complete, while keeping target-environment reruns required when release scope claims deployment-specific capacity.
- The final durable A-grade operations claim remains blocked by the production external alert receiver item above.
...
- Marked the Phase 3 performance and scale gate as pending privacy-safe calibrated evidence instead of treating skipped authenticated benchmarks as proof. This was superseded by the 2026-04-14 generated capacity profile; target-environment reruns remain required for deployment-specific capacity claims.
...
docker build --target deps -t sanctuary-frontend-deps-check .
...
Phase 3 Compose benchmark smoke passed against a temporary frontend/backend/gateway/worker/Postgres/Redis stack: migration/seed, service health, frontend/API/gateway health, WebSocket handshake, local authenticated wallet list, wallet-specific WebSocket sync fanout, transaction-history, synthetic 25,000-transaction wallet-history gate, wallet-sync queueing, admin backup validation and restore, generated 25,076-record backup restore, 30 worker proof jobs, two-worker BullMQ/Electrum ownership proof, 100-client two-backend Redis WebSocket fanout, and Postgres/Redis capacity snapshots passed. Latest evidence: `docs/plans/phase3-compose-benchmark-smoke-2026-04-15T00-12-26-208Z.md` and `docs/plans/phase3-benchmark-2026-04-15T00-12-49-889Z.md`.
Docker frontend dependency install compatibility passed after the lockfile npm 10 peer-entry refresh: `docker build --target deps -t sanctuary-frontend-deps-check .`.
Production-like runtime review was recorded from the same generated-data capacity run: `docs/plans/phase2-production-like-runtime-review-2026-04-15T00-12-26-208Z.md`.
