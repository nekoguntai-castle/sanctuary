...
Practical next patch: refactor `apiRouteHandler` in `e2e/dashboard-price-blocks.spec.ts` by moving mocked API route responses into a route table/helper while preserving the dashboard price-block smoke coverage. Run a target lizard check for `e2e/dashboard-price-blocks.spec.ts`, the focused Playwright dashboard price-block spec, `npm run typecheck:app`, and `npm run lint`.
