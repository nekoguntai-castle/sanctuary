...
Practical next patch: refactor the route handlers in `e2e/error-recovery.spec.ts` by moving mocked responses and failure scenarios into route tables/helpers while preserving the error recovery flows. Run a target lizard check for `e2e/error-recovery.spec.ts`, the focused Playwright error recovery spec, `npm run typecheck:app`, and `npm run lint`.
