...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 49 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| lizard_warning_count | 49 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.3 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 33; current top production JSX components are 29 CCN (`UTXOList`, `CoinControlPanel`, `EncryptionKeyDisplay`) | lizard warning scan | Maintainability 3.1 context |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 49 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 49 functions above CCN 15, with the top production JSX components still reaching 29 CCN.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `UTXOList`, `CoinControlPanel`, or `EncryptionKeyDisplay` as the next 29-CCN JSX starting points.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` completed after current remediation: 49 warnings, average CCN 1.3, max CCN 33.
