...
Practical next patch: split the remaining embedded backend scale-out proof-script builder in `scripts/perf/phase3-compose-benchmark-smoke.mjs` while preserving the generated Compose proof script. Run `node --check scripts/perf/phase3-compose-benchmark-smoke.mjs`, a target lizard check for `scripts/perf/phase3-compose-benchmark-smoke.mjs`, `npm run typecheck:app`, and `npm run lint`.
