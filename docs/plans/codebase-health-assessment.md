...
Practical next patch: refactor one measured helper in `scripts/perf/phase3-compose-benchmark-smoke.mjs`, starting with `assertBenchmarkProof`, while keeping the benchmark proof output unchanged. Run the most relevant script smoke available locally, a target lizard check for `scripts/perf/phase3-compose-benchmark-smoke.mjs`, `npm run typecheck:app`, and `npm run lint`.
