...
Practical next patch: refactor `scripts/verify-addresses/generate-vectors.ts` by extracting xpub derivation helpers or splitting script responsibilities while keeping generated vector output unchanged. Run the relevant verify-address vector script/test, a target lizard check for `scripts/verify-addresses/generate-vectors.ts`, `npm run typecheck:app`, and `npm run lint`.
