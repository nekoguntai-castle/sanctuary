...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 94 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
- Current remediation: `WalletStats`, `TransactionRow`, `DraftList`, `LabelSelector`, `SendTransactionPage`, `LabelManager`, `NotificationToast`, `NotificationPanel`/`NotificationItem`, `BlockVisualizer`, `Block`, `QueuedSummaryBlock`, `PendingTxDot`, `AppearanceTab`, and `BackgroundsPanel` are no longer top lizard findings; warning-band file pressure dropped by splitting dashboard repository tests, and gitleaks now has a pinned project-tooling path.
...
| lizard_warning_count | 94 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 59; current top JSX components are `Account`, `AgentManagement` callbacks, `TransactionList`, and `NetworkConnectionCard` | lizard CSV sort | Maintainability 3.1 context |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 94 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 94 functions above CCN 15, led by `components/Account/Account.tsx`, `components/AgentManagement/index.tsx`, `components/TransactionList/TransactionList.tsx`, and `components/NetworkConnectionCard/NetworkConnectionCard.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `Account`, `AgentManagement`, `TransactionList`, and `NetworkConnectionCard` as clear next starting points.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 94 warnings, average CCN 1.4, max CCN 59.
