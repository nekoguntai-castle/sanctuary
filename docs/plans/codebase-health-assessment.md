...
Practical next patch: refactor `decodeSegwitAddress` in `server/tests/unit/services/bitcoin/bip173-bip350.verified.test.ts` by moving BIP173/BIP350 decode validation branches into named helpers while preserving the verified-vector coverage. Run the targeted Vitest file, a target lizard check for that test file, `npm run typecheck:app`, and `npm run lint`.
