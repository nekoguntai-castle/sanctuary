...
Practical next patch: refactor `apiRouteHandler` in `e2e/import-wallet-flow.spec.ts` by moving mocked API route responses into a route table/helper while preserving the import wallet flow coverage. Run a target lizard check for `e2e/import-wallet-flow.spec.ts`, the focused Playwright import wallet flow spec, `npm run typecheck:app`, and `npm run lint`.
