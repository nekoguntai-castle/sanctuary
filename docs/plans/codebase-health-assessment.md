...
Practical next patch: refactor `apiRouteHandler` in `e2e/accessibility.spec.ts` by moving mocked API route responses into a route table/helper while preserving the accessibility smoke coverage. Run a target lizard check for `e2e/accessibility.spec.ts`, the focused Playwright accessibility spec, `npm run typecheck:app`, and `npm run lint`.
