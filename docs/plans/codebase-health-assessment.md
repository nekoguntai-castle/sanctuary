...
Owner: TBD
Status: Refreshed assessment; the `/grade` implementation-adjusted score is 85/100 (B), Phase 3 repository-controlled local capacity proof is complete, Phase 4 test typecheck and dependency-audit triage gates are current, and the next score-moving work is finishing lizard baseline reduction plus secret-scan false-positive cleanup
...
- Remaining score-moving local work: finish reducing the measured lizard baseline to zero and clean up current-tree/full-history gitleaks false positives. Performance and operations A-grade movement still requires environment evidence, not broad local refactoring.

Recommended next sequence:

| Priority | Work | Start With | Exit Criteria |
| --- | --- | --- | --- |
| P1 | Keep reducing lizard warnings, but prioritize production source before tests, E2E fixtures, benchmark scripts, and animations. | `gateway/src/services/backendEvents/notifications.ts`, `services/deviceParsers/parsers/coldcardNested.ts`, `services/deviceParsers/parsers/keystone.ts`, `utils/urDeviceDecoder.ts`, `utils/urPsbt.ts`, `utils/utxoAge.ts`, `services/hardwareWallet/adapters/trezor/pathUtils.ts`, `services/hardwareWallet/adapters/jade.ts`, `hooks/send/useSendTransactionActions.ts`, `hooks/send/useDraftManagement.ts`, `hooks/send/useQrSigning.ts`, `hooks/send/useUsbSigning.ts`, `contexts/send/reducer.ts`, `scripts/verify-addresses/generate-vectors.ts`, the E2E API harnesses, `components/animations/paperBoats.ts`, `components/animations/bunnyMeadow.ts`, `components/animations/trainStation.ts`, `components/animations/fireworks.ts`, `components/animations/ducklingParade.ts`, `components/animations/paperAirplanes.ts`, `components/animations/lavenderFields.ts`, and `components/animations/butterflyGarden.ts` are now complete. Continue the same focused cadence on the remaining WalletDetail filter hooks. | Each slice removes the targeted lizard warning or warnings, lowers `.github/workflows/quality.yml` and `scripts/quality.sh` by the verified delta, updates `tasks/grade-report-2026-04-13-c76.md`, and passes the focused tests, target lizard check, `npm run typecheck:app`, and `npm run lint`. |
...
Practical next patch: refactor the filter predicate in `components/WalletDetail/hooks/useTransactionFilters.ts` by moving search, direction, status, label, address, and amount matching into focused helpers while preserving filtering behavior. Run a target lizard check for `components/WalletDetail/hooks/useTransactionFilters.ts`, `npm run typecheck:app`, and `npm run lint`.
