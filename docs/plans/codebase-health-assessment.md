...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 93 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| --- | --- | --- | --- |
| tests | pass; app 396 files/5,557 tests; backend 390 passed/22 skipped files with 9,151 passed/503 skipped tests after current remediation; gateway 20 files/513 tests | `grade.sh`; `npm run test:coverage`; `npm run test:backend:coverage`; `npm run test:coverage` in `gateway` | Correctness 1.1 |
...
| lizard_warning_count | 93 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 55; current top JSX components are `AgentManagement` callbacks, `TransactionList`, `NetworkConnectionCard`, and `DeviceDetail` | lizard CSV sort | Maintainability 3.1 context |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 93 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 93 functions above CCN 15, led by `components/AgentManagement/index.tsx`, `components/TransactionList/TransactionList.tsx`, `components/NetworkConnectionCard/NetworkConnectionCard.tsx`, and `components/DeviceDetail/DeviceDetail.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `AgentManagement`, `TransactionList`, `NetworkConnectionCard`, and `DeviceDetail` as clear next starting points.
...
| 1 | Reduce complexity concentration | Extract top JSX decision/render helpers from the highest CCN components. | lizard warnings trend below 15. | `+1` to `+3`; `WalletStats`, `TransactionRow`, `DraftList`, `LabelSelector`, `SendTransactionPage`, `LabelManager`, `NotificationToast`, `NotificationPanel`, `BlockVisualizer`, `ThemeSection`, and `Account` extractions have started this trend. |
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 93 warnings, average CCN 1.4, max CCN 55.
- `npx --yes jscpd@4 --silent --reporters json --output /tmp/sanctuary-jscpd-grade-20260419 .` completed: 2.1% duplication.
- `npm run test:coverage` passed after current remediation: 396 app test files, 5,557 tests, 100% statements/branches/functions/lines.
