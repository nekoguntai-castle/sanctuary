...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative load/scale items are carried forward
...
| P2 | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end, including wallet-specific WebSocket sync fanout, but generated fixture data does not prove production-like performance. Load evidence would catch regressions in wallet sync, large wallet transaction history, WebSocket fanout, backup/restore, and worker queues. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-23-45-258Z.md` records the latest full-stack local authenticated smoke, while representative large-wallet/load and scale-out evidence remain pending. |
...
- Run and record representative large-wallet/load benchmark results for wallet sync, large wallet transaction history, WebSocket fanout with larger client counts and production-like update traffic, backup restore in non-production, and worker queue processing.
...
Phase 3 Compose benchmark smoke passed against a temporary frontend/backend/gateway/worker/Postgres/Redis stack: migration/seed, service health, frontend/API/gateway health, WebSocket handshake, local authenticated wallet list, wallet-specific WebSocket sync fanout, transaction-history, wallet-sync queueing, and admin backup validation passed. Latest evidence: `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-23-45-258Z.md` and `docs/plans/phase3-benchmark-2026-04-13T00-24-08-028Z.md`.
...
- Dependency audits.
- Phase 3 representative large-wallet/load benchmark, representative WebSocket fanout under larger client counts and production-like events, worker queue benchmark, restore-safe backup restore benchmark, and backend scale-out smoke.
