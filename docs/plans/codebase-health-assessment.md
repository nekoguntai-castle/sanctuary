...
Practical next patch: refactor `explodeRocket` in `components/animations/fireworks.ts` by moving particle burst creation into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/fireworks.ts`, `npm run typecheck:app`, and `npm run lint`.
