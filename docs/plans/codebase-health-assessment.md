...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative load/scale items are carried forward after local worker queue, backend scale-out, and worker scale-out smoke proof
...
| Performance | B | Caching, React Query discipline, Electrum pooling, database indexes, API aggregation, bounded WebSocket queues, and a Phase 3 benchmark harness are good. A disposable full-stack authenticated smoke now covers wallet list, transaction history, wallet-specific WebSocket sync fanout, wallet-sync queueing, WebSocket handshake, backup validation, local worker queue handler execution, two-worker diagnostic scale-out, and two-backend Redis WebSocket delivery; representative large-wallet/load, restore, representative worker queue load, and scale-out capacity benchmarks are still pending. |
...
| P2 | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end, including wallet-specific WebSocket sync fanout, worker queue handler execution, two-worker diagnostic scale-out, and two-backend Redis WebSocket delivery, but generated fixture data does not prove production-like performance. Load evidence would catch regressions in wallet sync, large wallet transaction history, WebSocket fanout, backup/restore, and representative worker queues. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T01-28-46-631Z.md` records the latest full-stack local authenticated smoke with worker queue, worker scale-out, and backend scale-out proof, while representative large-wallet/load and scale-out capacity evidence remain pending. |
| P2 | Validate the supported scale-out topology. | Redis bridge, distributed locks, worker queues, health checks, and scale-out docs show intent. Local two-backend and two-worker smokes now exist; operators still need representative backend/load-balancer capacity evidence and representative worker queue load before production worker replica support is broadened beyond the proven smoke boundary. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T01-28-46-631Z.md` proves a wallet-specific sync event triggered on one backend reaches a WebSocket subscribed on another backend through Redis, and proves two worker replicas process diagnostic BullMQ jobs while only one owns Electrum subscriptions; `docker-compose.yml` still runs one backend service and one worker service by default. |
...
- Run and record representative large-wallet/load benchmark results for wallet sync, large wallet transaction history, WebSocket fanout with larger client counts and production-like update traffic, backup restore in non-production, and representative worker queue processing.
- Keep the local auto-provisioned Compose smoke as endpoint-coverage evidence; record separate representative large-wallet and fanout runs because the local generated wallet proves end-to-end behavior but not scale characteristics.
...
- Dependency audits.
- Phase 3 representative large-wallet/load benchmark, representative WebSocket fanout under larger client counts and production-like events, representative worker queue load benchmark, restore-safe backup restore benchmark, and representative backend scale-out load/capacity benchmark.
