...
Practical next patch: refactor the render-regression E2E API mock helpers in `e2e/render-regression/renderRegressionHarness.ts` by moving route matching and response construction out of the large route handler while preserving the mocked API behavior. Run a target lizard check for `e2e/render-regression/renderRegressionHarness.ts`, the focused Playwright render-regression smoke if available, `npm run typecheck:app`, and `npm run lint`.
