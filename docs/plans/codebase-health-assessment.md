...
Practical next patch: refactor `seedSessions` in `server/tests/mocks/repositories.ts` by moving optional session field defaults into a small normalizer/helper while preserving repository mock behavior. Run a target lizard check for `server/tests/mocks/repositories.ts`, the focused backend tests that cover the repository mocks, `npm run typecheck:app`, and `npm run lint`.
