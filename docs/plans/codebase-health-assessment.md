...
| P1 | Harden browser token handling and CSP. | Partially completed in Phase 4 for CSP: Swagger UI inline/CDN allowances are now scoped to the `/api/v1/docs` route instead of the default API CSP. Browser token storage still needs a separate auth architecture decision because moving access tokens out of `localStorage` affects client refresh/session behavior. | `server/src/index.ts` now uses default API `script-src` and `style-src` values of `'self'`; `server/src/api/openapi/index.ts` sets the Swagger UI-specific CSP only for the docs HTML route; `server/tests/unit/api/openapi.test.ts` covers the docs CSP. `src/api/client.ts` still stores `sanctuary_token` in `localStorage`. |
