...
Practical next patch: refactor `createMockUTXO` in `server/tests/unit/services/bitcoin/industry/utxoEdgeCases.test.ts` by moving branch-specific UTXO fixture shaping into named helpers while preserving the existing edge-case fixtures. Run the targeted Vitest file, a target lizard check for that test file, `npm run typecheck:app`, and `npm run lint`.
