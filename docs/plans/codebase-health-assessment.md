...
Practical next patch: refactor `drawButterfly` in `components/animations/butterflyGarden.ts` by moving wing, body, antenna, sparkle, and detail drawing into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/butterflyGarden.ts`, `npm run typecheck:app`, and `npm run lint`.
