...
Practical next patch: refactor `validateWalletResponse` in `server/tests/helpers/contractValidation.ts` by moving optional-field checks into named assertion helpers while preserving contract validation behavior. Run a target lizard check for `server/tests/helpers/contractValidation.ts`, the tests that import the helper if practical, `npm run typecheck:app`, and `npm run lint`.
