...
- Production-like operator load testing beyond the Phase 3 local capacity proof.
...
3. Treat the Phase 3 local proof as complete for repository-controlled evidence, and collect representative operator load/capacity evidence before claiming A-grade scalability or performance.
...
Status: **Repository-controlled local proof complete as of 2026-04-12 (Pacific/Honolulu); representative operator evidence still gates A-grade scalability/performance**
...
Operator evidence still required before an A-grade scalability/performance claim:
...
- Keep the local auto-provisioned Compose smoke as endpoint-coverage and local capacity evidence; it now includes a synthetic 10,000-transaction wallet-history gate, 64-client two-backend Redis WebSocket fanout, 60 worker proof jobs, generated 10,076-record backup restore, and Postgres/Redis capacity snapshots, but still does not replace a production-like largest-known-wallet dataset or load-level fanout run.
...
- Validate backend and worker scale-out capacity with representative non-production load-balanced and queue-load topologies; the local two-backend Redis WebSocket fanout smoke and two-worker BullMQ/Electrum ownership smoke are now recorded.
...
- Use the disposable Compose smoke for repeatable local capacity and regression proof, including two-backend Redis WebSocket fanout, two-worker BullMQ/Electrum ownership, synthetic transaction-history, generated backup restore, worker queue repeats, and capacity snapshots; stand up or identify representative non-production load-balanced and worker queue-load topologies for operator capacity evidence.
...
- Phase 4 should reference the Phase 3 local capacity proof as complete, while keeping representative operator load, restore, and scale-out capacity runs pending.
...
- Dependency audits.
- Phase 3 production-like largest-known-wallet/load benchmark, representative WebSocket fanout under operator client counts and production-like events, representative worker queue load benchmark, representative backup restore-size benchmark, and representative backend scale-out load/capacity benchmark.
