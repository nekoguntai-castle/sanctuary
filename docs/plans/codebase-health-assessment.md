...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative load/scale items are carried forward after local worker queue, synthetic large-wallet history, backup restore, multi-client backend fanout, and worker scale-out smoke proof
...
| Scalability | B | Dedicated worker, BullMQ/Redis, distributed locks, WebSocket limits, Redis bridge broadcasts, Prisma indexes, cache invalidation, and a scale-out baseline are solid. Disposable two-backend and two-worker smokes now prove multi-client Redis-backed WebSocket delivery across backend replicas, BullMQ diagnostic processing on both worker replicas, recurring job deduplication, one Electrum subscription owner after retry recovery, and a shared-lock skip. The grade stays at B until Redis/Postgres capacity, backend scale-out under representative load, representative worker queue processing, and larger WebSocket fanout are validated under non-production load. |
| Performance | B | Caching, React Query discipline, Electrum pooling, database indexes, API aggregation, bounded WebSocket queues, and a Phase 3 benchmark harness are good. A disposable full-stack authenticated smoke now covers wallet list, transaction history, a synthetic 1,000-transaction wallet-history gate, wallet-specific WebSocket sync fanout, wallet-sync queueing, WebSocket handshake, backup validation and restore, local worker queue handler execution, two-worker diagnostic scale-out, and multi-client two-backend Redis WebSocket fanout; production-like largest-known-wallet/load, representative backup restore size, representative worker queue load, and scale-out capacity benchmarks are still pending. |
...
| P2 | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end, including a synthetic 1,000-transaction wallet-history gate, backup restore against a generated disposable backup, wallet-specific WebSocket sync fanout, worker queue handler execution, two-worker diagnostic scale-out, and multi-client two-backend Redis WebSocket fanout, but generated fixture data still does not prove production-like performance. Load evidence would catch regressions in wallet sync, largest-known-wallet transaction history, WebSocket fanout, representative backup restore size, and representative worker queues. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-21-10-302Z.md` records the latest full-stack local authenticated smoke with backup restore, synthetic large-wallet history, worker queue, worker scale-out, and backend fanout proof, while production-like large-wallet/load and scale-out capacity evidence remain pending. |
| P2 | Validate the supported scale-out topology. | Redis bridge, distributed locks, worker queues, health checks, and scale-out docs show intent. Local two-backend and two-worker smokes now exist; operators still need representative backend/load-balancer capacity evidence and representative worker queue load before production worker replica support is broadened beyond the proven smoke boundary. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-21-10-302Z.md` proves a wallet-specific sync event triggered on one backend reaches 8/8 WebSocket clients spread across both backend replicas through Redis, and proves two worker replicas process diagnostic BullMQ jobs while only one owns Electrum subscriptions; `docker-compose.yml` still runs one backend service and one worker service by default. |
...
- Recorded a passing full-stack smoke with worker queue proof in `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-40-08-363Z.md`, with underlying benchmark evidence in `docs/plans/phase3-benchmark-2026-04-13T00-40-31-291Z.md`. The worker proof completed 6 jobs with p95 17.75 ms and zero failed jobs; it remains handler smoke evidence rather than representative queue load proof.
...
- Recorded a passing full-stack smoke with backup restore, synthetic large-wallet, worker queue, worker scale-out, and multi-client backend fanout proof in `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-21-10-302Z.md`, with underlying benchmark evidence in `docs/plans/phase3-benchmark-2026-04-13T02-21-33-723Z.md`. The generated backup validate and restore scenarios passed with restore p95 55.25 ms; the synthetic large-wallet proof inserted 1,000 transactions and passed 20 authenticated transaction-history requests with p95 31.05 ms and p99 31.81 ms. The worker scale-out proof observed two healthy workers, diagnostic processors `0c0bb91dbaa2` and `8d73f3bfbe68`, one Electrum owner, one locked diagnostic execution, one `lock_held` skip, and one repeatable definition for each core recurring job. The backend fanout proof delivered the wallet sync event to 8/8 WebSocket clients across 2 backend replicas via Redis with p95 14 ms; this remains local smoke evidence rather than production-like largest-known-wallet/load-balanced capacity proof.

Remaining Phase 3 work:

- Run and record production-like largest-known-wallet/load benchmark results for wallet sync, transaction history, WebSocket fanout with larger client counts and production-like update traffic, representative backup restore size in non-production, and representative worker queue processing.
- Keep the local auto-provisioned Compose smoke as endpoint-coverage evidence; it now includes a synthetic 1,000-transaction wallet-history gate and multi-client two-backend Redis WebSocket fanout, but still does not replace a production-like largest-known-wallet dataset or load-level fanout run.
- Calibrate the scripted benchmark harness with realistic dataset size, request counts, concurrency, and strict release thresholds after the first representative dataset run.
- Validate backend and worker scale-out capacity with representative non-production load-balanced and queue-load topologies; the local multi-client two-backend Redis WebSocket fanout smoke and two-worker BullMQ/Electrum ownership smoke are now recorded.
...
- Obtain a representative non-production `SANCTUARY_TOKEN` and `SANCTUARY_WALLET_ID` for production-like largest-known-wallet transaction history and wallet sync queue benchmarks.
- Obtain a non-production `SANCTUARY_ADMIN_TOKEN` and representative `SANCTUARY_BACKUP_FILE` for representative backup validation and restore timing, with `SANCTUARY_ALLOW_RESTORE=true` only in a restore-safe environment.
- Use the disposable Compose smoke for repeatable multi-client two-backend Redis WebSocket fanout and two-worker BullMQ/Electrum ownership regression proof; stand up or identify representative non-production load-balanced and worker queue-load topologies for capacity evidence.
...
- Phase 4 should reference Phase 3 benchmark artifacts as optional or pending until the representative load, restore, and scale-out capacity runs are recorded.
...
Phase 3 Compose benchmark smoke passed against a temporary frontend/backend/gateway/worker/Postgres/Redis stack: migration/seed, service health, frontend/API/gateway health, WebSocket handshake, local authenticated wallet list, wallet-specific WebSocket sync fanout, transaction-history, synthetic 1,000-transaction wallet-history gate, wallet-sync queueing, admin backup validation and restore, local worker queue handler proof, two-worker BullMQ/Electrum ownership proof, and multi-client two-backend Redis WebSocket fanout passed. Latest evidence: `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-21-10-302Z.md` and `docs/plans/phase3-benchmark-2026-04-13T02-21-33-723Z.md`.
...
- Dependency audits.
- Phase 3 production-like largest-known-wallet/load benchmark, representative WebSocket fanout under larger client counts and production-like events, representative worker queue load benchmark, representative backup restore-size benchmark, and representative backend scale-out load/capacity benchmark.
