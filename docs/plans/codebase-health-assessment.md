...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 63 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| --- | --- | --- | --- |
| tests | pass; app 400 files/5,580 tests; backend 390 passed/22 skipped files with 9,151 passed/503 skipped tests after current remediation; gateway 20 files/513 tests | `grade.sh`; `npm run test:coverage`; `npm run test:backend:coverage`; `npm run test:coverage` in `gateway` | Correctness 1.1 |
...
| lizard_warning_count | 63 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.3 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 33; current top JSX component is `SharingSection` | lizard warning scan | Maintainability 3.1 context |
| duplication_pct | 2.02%; 276 clones, 5,288 duplicated lines | `npx --yes jscpd@4 .` | Maintainability 3.2 |
...
| test_file_count | 1,200 | `grade.sh` heuristic | Test Quality 6.2 |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 63 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 63 functions above CCN 15, with top JSX components still reaching 33 CCN.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `SharingSection` as the clear next starting point among tied 33-CCN JSX components.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` completed after current remediation: 63 warnings, average CCN 1.3, max CCN 33.
- `npx --yes jscpd@4 .` completed: 2.02% duplication, 276 clones, 5,288 duplicated lines.
- `npm run test:coverage` passed after current remediation: 400 app test files, 5,580 tests, 100% statements/branches/functions/lines.
