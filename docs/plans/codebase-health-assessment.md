...
Practical next patch: refactor `runSizedBackupRestoreProof` in `scripts/perf/phase3-compose-benchmark-smoke.mjs` while keeping the generated-backup create/validate/restore proof output unchanged. Run `node --check scripts/perf/phase3-compose-benchmark-smoke.mjs`, a target lizard check for `scripts/perf/phase3-compose-benchmark-smoke.mjs`, `npm run typecheck:app`, and `npm run lint`.
