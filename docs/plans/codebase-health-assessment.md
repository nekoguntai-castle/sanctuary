...
Practical next patch: refactor `apiRouteHandler` in `e2e/user-journeys.spec.ts` by moving the large mocked API response surface into route tables/helpers while preserving the user journey flows. Run a target lizard check for `e2e/user-journeys.spec.ts`, the focused Playwright user journeys spec, `npm run typecheck:app`, and `npm run lint`.
