...
Practical next patch: refactor `drawDuck` in `components/animations/ducklingParade.ts` by moving drawing branches into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/ducklingParade.ts`, `npm run typecheck:app`, and `npm run lint`.
