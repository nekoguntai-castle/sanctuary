...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard still reports 105 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
- Current remediation: `WalletStats`, `TransactionRow`, `DraftList`, and `LabelSelector` are no longer top lizard findings; warning-band file pressure dropped by splitting dashboard repository tests, and gitleaks now has a pinned project-tooling path.
...
| --- | --- | --- | --- |
| tests | pass; app 396 files/5,556 tests; backend 390 passed/22 skipped files with 9,151 passed/503 skipped tests after current remediation; gateway 20 files/513 tests | `grade.sh`; `npm run test:coverage`; `npm run test:backend:coverage`; `npm run test:coverage` in `gateway` | Correctness 1.1 |
...
| lizard_warning_count | 105 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 65; current top JSX components are `SendTransactionPage`, `LabelManager`, `NotificationToast`, and `NotificationItem` | lizard CSV sort | Maintainability 3.1 context |
...
| test_file_count | 1,196 | `grade.sh` heuristic | Test Quality 6.2 |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 105 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 105 functions above CCN 15, led by `components/send/SendTransactionPage.tsx`, `components/LabelManager.tsx`, `components/NotificationToast.tsx`, and `components/NotificationPanel.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `SendTransactionPage`, `LabelManager`, `NotificationToast`, and `NotificationPanel` as clear next starting points.
...
| 1 | Reduce complexity concentration | Extract top JSX decision/render helpers from the highest CCN components. | lizard warnings trend below 15. | `+1` to `+3`; `WalletStats`, `TransactionRow`, `DraftList`, and `LabelSelector` extractions have started this trend. |
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 105 warnings, average CCN 1.4, max CCN 65.
- `npx --yes jscpd@4 --silent --reporters json --output /tmp/sanctuary-jscpd-grade-20260419 .` completed: 2.1% duplication.
- `npm run test:coverage` passed after current remediation: 396 app test files, 5,556 tests, 100% statements/branches/functions/lines.
...
- Focused remediation checks passed: `npx vitest run tests/unit/repositories/agentRepository.test.ts tests/unit/api/admin-agents-routes.test.ts tests/unit/agent/dto.test.ts`, `npx vitest run tests/components/WalletStats.test.tsx`, `npx vitest run tests/components/TransactionList/TransactionRow.branches.test.tsx tests/components/TransactionList.test.tsx`, `npx vitest run tests/components/DraftList.test.tsx tests/components/DraftList/DraftList.branches.test.tsx tests/components/DraftList/DraftRow.branches.test.tsx tests/components/DraftList/utils.branches.test.ts`, `npx vitest run tests/components/LabelSelector.test.tsx`, `npx vitest run tests/unit/repositories/agentRepository.test.ts tests/unit/repositories/agentDashboardRepository.test.ts`, `.tmp/quality-tools/lizard-1.21.2/bin/lizard -C 15 -w components/WalletStats.tsx components/WalletStats server/tests/unit/repositories/agentRepository.test.ts server/tests/unit/repositories/agentDashboardRepository.test.ts scripts/quality.sh`, `.tmp/quality-tools/lizard-1.21.2/bin/lizard -C 15 -w components/TransactionList/TransactionRow.tsx components/TransactionList/TransactionRow`, `.tmp/quality-tools/lizard-1.21.2/bin/lizard -C 15 -w components/DraftList/DraftList.tsx components/DraftList/draftListHelpers.ts components/DraftList/useDraftListController.ts`, `.tmp/quality-tools/lizard-1.21.2/bin/lizard -C 15 -w components/LabelSelector.tsx components/LabelSelector`, `npm run lint:app`, `npm run lint:server`, `npm run typecheck:app`, and `npm run typecheck:server:tests`.
