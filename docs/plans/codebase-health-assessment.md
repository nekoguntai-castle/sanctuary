...
Practical next patch: refactor `animate` in `components/animations/paperBoats.ts` by moving the per-frame drawing/update branches into focused helpers while preserving the animation behavior. Run a target lizard check for `components/animations/paperBoats.ts`, `npm run typecheck:app`, and `npm run lint`.
