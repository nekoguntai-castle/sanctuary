...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative load/scale items are carried forward after local worker queue and backend scale-out smoke proof
...
| Scalability | B | Dedicated worker, BullMQ/Redis, distributed locks, WebSocket limits, Redis bridge broadcasts, Prisma indexes, cache invalidation, and a scale-out baseline are solid. A disposable two-backend smoke now proves Redis-backed WebSocket delivery across backend replicas; the grade stays at B until worker ownership, Redis/Postgres capacity, backend scale-out under representative load, and larger WebSocket fanout are validated under non-production load. |
| Performance | B | Caching, React Query discipline, Electrum pooling, database indexes, API aggregation, bounded WebSocket queues, and a Phase 3 benchmark harness are good. A disposable full-stack authenticated smoke now covers wallet list, transaction history, wallet-specific WebSocket sync fanout, wallet-sync queueing, WebSocket handshake, backup validation, local worker queue handler execution, and two-backend Redis WebSocket delivery; representative large-wallet/load, restore, representative worker queue load, and scale-out capacity benchmarks are still pending. |
...
| P2 | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end, including wallet-specific WebSocket sync fanout, worker queue handler execution, and two-backend Redis WebSocket delivery, but generated fixture data does not prove production-like performance. Load evidence would catch regressions in wallet sync, large wallet transaction history, WebSocket fanout, backup/restore, and representative worker queues. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-54-44-886Z.md` records the latest full-stack local authenticated smoke with worker queue and backend scale-out proof, while representative large-wallet/load and scale-out capacity evidence remain pending. |
| P2 | Validate the supported scale-out topology. | Redis bridge, distributed locks, worker queues, health checks, and scale-out docs show intent. A local two-backend WebSocket proof now exists; operators still need worker replica safety, or an explicit production singleton worker policy, and representative backend/load-balancer capacity evidence. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-54-44-886Z.md` proves a wallet-specific sync event triggered on one backend reaches a WebSocket subscribed on another backend through Redis; `docker-compose.yml` still runs one backend service and one worker service by default, and `docs/SCALABILITY_AND_PERFORMANCE.md` keeps worker scale-out non-production only until validated. |
...
- Validate backend scale-out capacity with a representative non-production load-balanced topology; the local two-backend Redis WebSocket smoke is now recorded.
...
- Use the disposable Compose smoke for repeatable two-backend Redis WebSocket regression proof; stand up or identify a representative non-production load-balanced topology for backend scale-out capacity evidence.
...
Phase 3 Compose benchmark smoke passed against a temporary frontend/backend/gateway/worker/Postgres/Redis stack: migration/seed, service health, frontend/API/gateway health, WebSocket handshake, local authenticated wallet list, wallet-specific WebSocket sync fanout, transaction-history, wallet-sync queueing, admin backup validation, local worker queue handler proof, and two-backend Redis WebSocket delivery passed. Latest evidence: `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T00-54-44-886Z.md` and `docs/plans/phase3-benchmark-2026-04-13T00-55-56-530Z.md`.
