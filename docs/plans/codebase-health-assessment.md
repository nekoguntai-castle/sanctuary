...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard still reports 104 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
- Current remediation: `WalletStats`, `TransactionRow`, `DraftList`, `LabelSelector`, and `SendTransactionPage` are no longer top lizard findings; warning-band file pressure dropped by splitting dashboard repository tests, and gitleaks now has a pinned project-tooling path.
...
| lizard_warning_count | 104 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 65; current top JSX components are `LabelManager`, `NotificationToast`, `NotificationItem`, and `BlockVisualizer` | lizard CSV sort | Maintainability 3.1 context |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 104 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 104 functions above CCN 15, led by `components/LabelManager.tsx`, `components/NotificationToast.tsx`, `components/NotificationPanel.tsx`, and `components/BlockVisualizer/BlockVisualizer.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `LabelManager`, `NotificationToast`, `NotificationPanel`, and `BlockVisualizer` as clear next starting points.
...
| 1 | Reduce complexity concentration | Extract top JSX decision/render helpers from the highest CCN components. | lizard warnings trend below 15. | `+1` to `+3`; `WalletStats`, `TransactionRow`, `DraftList`, `LabelSelector`, and `SendTransactionPage` extractions have started this trend. |
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 104 warnings, average CCN 1.4, max CCN 65.
