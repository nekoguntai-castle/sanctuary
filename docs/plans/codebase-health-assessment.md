...
| Maintainability       |      14/15 | Lizard is green, and the previous PSBT, price-route, Trezor, Ledger, address repository, agent route, admin agent route, agent repository, transaction HTTP export, and remaining admin API warning-sized test files were split along clean boundaries. Large-file policy passes, but there is still 1 warning-sized test file below the hard limit. |
...
| large-file policy                            | pass; largest source 795 lines; largest test 984 lines; 1 warning-sized test file; 0 over-limit files                                                           | `node scripts/quality/check-large-files.mjs --json`                                                            | Maintainability 3.3                      |
...
- **[3.3] Large-file hygiene - High but not perfect**: no file exceeds the 1000-line gate, but 1 test file remains in the warning band and should be split only when a real ownership boundary appears.
...
3. One test file remains near the large-file warning threshold. It passes policy, but future additions should avoid pushing it over 1000 lines.
...
3. Split the remaining warning-sized test file only when touching it for real behavior changes.
...
- `npm --prefix server run test -- tests/unit/api/transactions-http-routes.test.ts` - passed, 74 tests after splitting transaction export streaming/pagination coverage into its own contract file.
- `npm run test:run -- tests/api/remainingApiModules.test.ts tests/api/remainingAdminApiModules.test.ts` - passed, 21 tests after splitting admin-only API wrapper coverage into its own file.
