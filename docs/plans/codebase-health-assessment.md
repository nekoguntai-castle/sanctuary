...
Owner: TBD
Status: Refreshed assessment; production alert receiver and Phase 3 representative load/scale items are carried forward after local worker queue, synthetic large-wallet history, backend scale-out, and worker scale-out smoke proof
...
| P2 | Run authenticated performance and scale gates for high-risk workflows. | The Phase 3 harness and disposable Compose smoke now prove local authenticated paths end to end, including a synthetic 1,000-transaction wallet-history gate, wallet-specific WebSocket sync fanout, worker queue handler execution, two-worker diagnostic scale-out, and two-backend Redis WebSocket delivery, but generated fixture data still does not prove production-like performance. Load evidence would catch regressions in wallet sync, largest-known-wallet transaction history, WebSocket fanout, backup/restore, and representative worker queues. | `docs/SCALABILITY_AND_PERFORMANCE.md`, `npm run perf:phase3`, and `npm run perf:phase3:compose-smoke` exist; `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T01-49-46-785Z.md` records the latest full-stack local authenticated smoke with synthetic large-wallet history, worker queue, worker scale-out, and backend scale-out proof, while production-like large-wallet/load and scale-out capacity evidence remain pending. |
| P2 | Validate the supported scale-out topology. | Redis bridge, distributed locks, worker queues, health checks, and scale-out docs show intent. Local two-backend and two-worker smokes now exist; operators still need representative backend/load-balancer capacity evidence and representative worker queue load before production worker replica support is broadened beyond the proven smoke boundary. | `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T01-49-46-785Z.md` proves a wallet-specific sync event triggered on one backend reaches a WebSocket subscribed on another backend through Redis, and proves two worker replicas process diagnostic BullMQ jobs while only one owns Electrum subscriptions; `docker-compose.yml` still runs one backend service and one worker service by default. |
...
- Run and record production-like largest-known-wallet/load benchmark results for wallet sync, transaction history, WebSocket fanout with larger client counts and production-like update traffic, backup restore in non-production, and representative worker queue processing.
...
- Obtain a non-production `SANCTUARY_ADMIN_TOKEN` and representative `SANCTUARY_BACKUP_FILE` for backup validation and restore timing, with `SANCTUARY_ALLOW_RESTORE=true` only in a restore-safe environment.
...
- Dependency audits.
- Phase 3 production-like largest-known-wallet/load benchmark, representative WebSocket fanout under larger client counts and production-like events, representative worker queue load benchmark, restore-safe backup restore benchmark, and representative backend scale-out load/capacity benchmark.
