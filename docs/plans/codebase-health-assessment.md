...
| gateway coverage | 21 files, 528 tests | `npm --prefix gateway run test:coverage` | Test Quality context |
...
- `npm --prefix gateway run test:coverage` - passed with 100% coverage after adding the malformed optional-claim regression test.
...
- `npm --prefix gateway run test:run -- tests/unit/middleware/auth.test.ts` - 21 focused gateway auth tests passed after adding malformed optional-claim coverage.
