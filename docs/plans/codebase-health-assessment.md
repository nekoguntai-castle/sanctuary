...
| Maintainability | 7/15 | Architecture and duplication are strong, and the large-file policy is green again; lizard now reports 71 CCN warnings and the largest file is a classified 2,637-line proof harness. |
...
| lizard_warning_count | 71 functions with CCN > 15 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard -w .` | Maintainability 3.1 |
| lizard_avg_ccn | 1.4 | `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` | Maintainability 3.1 context |
| lizard_max_ccn | 39; current top JSX component is `UTXOGarden` | lizard CSV sort | Maintainability 3.1 context |
| duplication_pct | 2.04%; 276 clones, 5,287 duplicated lines | `npx --yes jscpd@4 .` | Maintainability 3.2 |
...
- **[3.5] Readability/naming - Medium -> +1**: ISO Analyzability is mixed because naming is generally clear, but 71 lizard warnings and oversized files make review/change analysis harder.
...
1. Complexity remains the largest score drag - lizard reports 71 functions above CCN 15, led by `components/UTXOList/UTXOGarden.tsx`, `components/WalletDetail/WalletTelegramSettings.tsx`, and `App.tsx`.
...
1. Reduce top lizard components - expected gain: up to +5 Maintainability if warnings reach 0; effort: high, with `UTXOGarden`, `WalletTelegramSettings`, and `AppRoutes` as clear next starting points.
...
- `.tmp/quality-tools/lizard-1.21.2/bin/lizard .` and `-w .` completed after current remediation: 71 warnings, average CCN 1.4, max CCN 39.
- `npx --yes jscpd@4 .` completed: 2.04% duplication.
