...
Status: Phase 3 local capacity proof baseline
...
| Worker | One container | Non-production smoke proven; production still gated by representative load | BullMQ/Redis queues, per-job distributed locks, retryable Electrum subscription ownership | The disposable two-worker smoke proves diagnostic BullMQ processing on both replicas, recurring job deduplication, one Electrum owner after retry recovery, a shared-lock skip, and a repeated local queue-handler proof. Production support remains one worker until representative worker queue load and Electrum subscription volume are tested. |
...
- Keep exactly one worker in default deployments until the release has a current worker scale-out smoke and representative worker queue-load evidence for the target environment.
- Treat any worker replica count above one as non-production unless the worker scale-out smoke test for this release passes; for production, also require representative queue-load and Electrum subscription-volume evidence.
...
The auto-provisioned local fixture is repository-controlled capacity evidence, not production-like operator evidence. It is useful for proving the authenticated paths, generated backup validation and restore, a synthetic large-wallet transaction-history query, worker queue handlers, two-worker BullMQ/Electrum ownership behavior, multi-client two-backend Redis WebSocket fanout, and Postgres/Redis capacity snapshots end to end without requiring a pre-existing local stack. It does not replace the production-like largest-known-wallet dataset, load-level WebSocket fanout, representative backup size, representative worker queue depth, or backend load-balanced capacity evidence needed for the final scalability/performance A-grade claim. The current passing disposable evidence is `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-50-46-877Z.md` with benchmark output in `docs/plans/phase3-benchmark-2026-04-13T02-51-11-475Z.md`.
...
Repository-controlled Phase 3 local proof is complete as of the 2026-04-13 UTC evidence run:

- Current local evidence is `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-50-46-877Z.md`.
- The run recorded wallet list, transaction history, WebSocket subscription fanout, wallet sync queueing, backup validation and restore, a synthetic 10,000-transaction wallet-history gate, a 6.53 MiB generated backup with 10,076 restored records, 60 worker proof jobs, two-worker diagnostic processing/ownership, 64-client two-backend Redis WebSocket fanout, and Postgres/Redis capacity snapshots.
...
The following remain required before claiming A-grade scalability/performance or broadening production scale-out support:

- Record production-like largest-known-wallet/load benchmark results for wallet sync, transaction history, WebSocket fanout with operator client counts and production-like update traffic, representative backup restore size in non-production, and representative worker queue processing.
- Validate backend and worker scale-out capacity with representative non-production load-balanced and queue-load topologies.
- Calibrate strict release thresholds from representative dataset runs rather than local generated fixtures.
