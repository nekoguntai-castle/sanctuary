...
The auto-provisioned local fixture is smoke evidence only. It is useful for proving the authenticated paths, generated backup validation and restore, a synthetic large-wallet transaction-history query, worker queue handlers, two-worker BullMQ/Electrum ownership behavior, and multi-client two-backend Redis WebSocket fanout run end to end, and the Compose wrapper records this proof without requiring a pre-existing local stack, but it does not replace the production-like largest-known-wallet dataset, load-level WebSocket fanout, representative backup size, representative worker queue depth, or backend load-balanced capacity evidence needed for the final scalability/performance gate. The current passing disposable evidence is `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-21-10-302Z.md` with benchmark output in `docs/plans/phase3-benchmark-2026-04-13T02-21-33-723Z.md`.
...
- At least one representative benchmark run is recorded for wallet sync, transaction history, WebSocket fanout, backup/restore, and worker queue processing. The disposable Compose smoke proves the WebSocket subscription path, generated backup validation and restore, a synthetic 1,000-transaction wallet-history gate, local worker queue handlers, two-worker diagnostic processing/ownership, and multi-client two-backend Redis WebSocket fanout, but not production-like largest-known-wallet behavior, fanout volume, representative backup size, representative queue load, or load-balanced backend capacity.
- The backend scale-out smoke test proves Redis-backed WebSocket broadcast delivery across at least two backend instances. Current local evidence is `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T02-21-10-302Z.md`.
- Worker scale-out has current local smoke evidence, but production worker scale-out remains gated by representative queue-load and Electrum subscription-volume evidence.
