...
- At least one representative benchmark run is recorded for wallet sync, transaction history, WebSocket fanout, backup/restore, and worker queue processing. The disposable Compose smoke proves the WebSocket subscription path, local worker queue handlers, two-worker diagnostic processing/ownership, and two-backend Redis WebSocket delivery, but not production-like fanout volume, representative queue load, or load-balanced backend capacity.
- The backend scale-out smoke test proves Redis-backed WebSocket broadcast delivery across at least two backend instances. Current local evidence is `docs/plans/phase3-compose-benchmark-smoke-2026-04-13T01-28-46-631Z.md`.
