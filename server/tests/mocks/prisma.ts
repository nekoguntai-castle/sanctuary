...
const createModelMock = () => ({
  ...
  createManyAndReturn: vi.fn().mockImplementation((data) => Promise.resolve(data.data)),
...
export const mockPrismaClient = {
  ...
  addressSubscriptionCheckpoint: createModelMock(),
...
const defaultModelImplementations = {
  ...
  findUniqueOrThrow: () => Promise.reject(new Error('Record not found')),
  create: (data: any) => Promise.resolve({ id: 'mock-id', ...data.data }),
  createMany: () => Promise.resolve({ count: 0 }),
  createManyAndReturn: (data: any) => Promise.resolve(data.data),
