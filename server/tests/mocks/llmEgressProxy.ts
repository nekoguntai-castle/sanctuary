...
/**
 * LLM Egress Proxy Mock
 *
 * Mock responses for the LLM egress proxy service used in testing.
 * Simulates the isolated LLM egress proxy that handles external AI calls.
 */
...
interface LlmEgressProxyMockState {
  healthy: boolean;
  aiAvailable: boolean;
  ollamaFound: boolean;
}
...
function respondProviderCheck(ollamaFound: boolean) {
  return mockJsonResponse(
    ollamaFound
      ? { compatible: true, endpointType: 'host', providerType: 'ollama' }
      : { compatible: false, reason: 'provider_unreachable' },
  );
}
...
function dispatchLlmEgressProxyRequest(
  url: string,
  request: ParsedMockRequest,
  state: LlmEgressProxyMockState,
) {
  ...
  if (isPostEndpoint(url, method, '/check-provider')) {
    return respondProviderCheck(state.ollamaFound);
  }
...
/**
 * Create a mock fetch function that simulates LLM egress proxy responses
 */
export function createLlmEgressProxyMock(options?: {
  healthy?: boolean;
  aiAvailable?: boolean;
  ollamaFound?: boolean;
}): Mock {
  ...
  return vi.fn().mockImplementation((url: string, init?: RequestInit) => {
    return dispatchLlmEgressProxyRequest(url, parseMockRequest(init), {
      healthy,
      aiAvailable,
      ollamaFound,
    });
...
/**
 * Reset all LLM egress proxy mocks to default state
 */
export function resetLlmEgressProxyMocks(...mocks: Mock[]): void {
  mocks.forEach((mock) => {
