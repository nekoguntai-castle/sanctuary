/**
 * BIP-340 Official Test Vectors (Schnorr Signatures for secp256k1)
 *
 * These are the official test vectors from the BIP-340 specification:
...
export interface Bip340SignatureVector {
  index: number;
  secretKey: string;       // 32-byte hex (empty string if public key only)
  publicKey: string;        // 32-byte x-only public key hex
  auxRand: string;          // 32-byte auxiliary randomness hex (empty for verification-only)
  message: string;          // Hex-encoded message (variable length)
  signature: string;        // 64-byte signature hex
  verificationResult: boolean;
  comment: string;
}

/**
 * BIP-340 test vectors from the official CSV
 *
 * Vectors 0-3: Valid signatures (can test both signing and verification)
 * Vectors 4: Verification-only valid signature
 * Vectors 5-14: Invalid signatures (must fail verification)
 * Vectors 15-18: Variable-length messages (added 2022-12)
 */
export const BIP340_TEST_VECTORS: Bip340SignatureVector[] = [
  {
    index: 0,
  ...
  // Variable-length message vectors (added 2022-12)
  {
    index: 15,
