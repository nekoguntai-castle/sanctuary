...
describe('consoleRepository', () => {
  ...
  it('creates, finds, lists, and updates sessions', async () => {
    ...
    await expect(consoleRepository.createSession({
      userId,
      title: undefined,
      scope: { kind: 'general' },
    ...
    await expect(consoleRepository.findSessionForUser(sessionId, userId)).resolves.toBe(session);
    await expect(consoleRepository.listSessions(userId, 10, 5)).resolves.toEqual([session]);
    await expect(consoleRepository.updateSessionScope(sessionId, { kind: 'admin' }, 'admin')).resolves.toBe(session);
  ...
  it('creates and completes turns, attaches prompt history, and lists traces', async () => {
    ...
    (prisma.consoleTurn.findMany as Mock).mockResolvedValue([{ ...turn, toolTraces: [] }]);

    await expect(consoleRepository.createTurn({
      sessionId,
      prompt: 'hello',
      scope: { kind: 'general' },
      maxSensitivity: 'wallet',
    ...
    await expect(consoleRepository.updateTurnState(turnId, 'planning')).resolves.toBe(turn);
    await expect(consoleRepository.completeTurn({
      id: turnId,
      response: 'answer',
    ...
    await expect(consoleRepository.failTurn(turnId, { message: 'failed' })).resolves.toBe(turn);
    await expect(consoleRepository.attachPromptToTurn(turnId, promptId)).resolves.toBe(turn);
    await expect(consoleRepository.listTurns(sessionId, 20)).resolves.toEqual([{ ...turn, toolTraces: [] }]);
  ...
  it('creates, filters, updates, deletes, and replays prompt history', async () => {
    ...
    await expect(consoleRepository.createPrompt({
      userId,
      sessionId: undefined,
      turnId: undefined,
      prompt: 'hello',
    ...
    await expect(consoleRepository.listPrompts(userId, {
      search: 'hello',
      saved: false,
      includeExpired: false,
      now,
    }, 25, 2)).resolves.toEqual([prompt]);
    await expect(consoleRepository.listPrompts(userId, {
      includeExpired: true,
    }, 5, 0)).resolves.toEqual([prompt]);
    await expect(consoleRepository.findPromptForUser(promptId, userId, now)).resolves.toBe(prompt);
    await expect(consoleRepository.updatePrompt({
      id: promptId,
      saved: true,
      title: null,
      expiresAt: null,
    ...
    await expect(consoleRepository.updatePromptMetadata({
      id: promptId,
      tools: ['get_fee_estimates'],
      providerProfileId: undefined,
      model: undefined,
    })).resolves.toBe(prompt);
    await expect(consoleRepository.softDeletePrompt(promptId)).resolves.toBe(prompt);
    await expect(consoleRepository.markPromptReplayed(promptId)).resolves.toBe(prompt);
  ...
  it('stores tool traces, purges expired prompts, and reports support stats', async () => {
    ...
    (prisma.consolePromptHistory.updateMany as Mock).mockResolvedValue({ count: 2 });
    ...
    await expect(consoleRepository.createToolTrace({
      turnId,
    ...
    await expect(consoleRepository.purgeExpiredPromptHistory(now)).resolves.toBe(2);
    ...
    expect(prisma.consoleToolTrace.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ toolName: 'get_fee_estimates', facts: { summary: 'fees' } }),
    });
