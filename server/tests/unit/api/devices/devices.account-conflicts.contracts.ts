...
export function registerNormalizeIncomingAccountsTests(): void {
  describe('normalizeIncomingAccounts', () => {
    ...
    it('returns valid multi-account input unchanged', async () => {
      ...
      expect(normalizeIncomingAccounts(accounts, undefined, undefined)).toEqual({ accounts });
    });

    it.each([
      [
        { purpose: 'single_sig', scriptType: 'native_segwit', derivationPath: "m/84'/0'/0'" },
        'Each account must have purpose, scriptType, derivationPath, and xpub',
      ],
      [
        { purpose: 'shared', scriptType: 'native_segwit', derivationPath: "m/84'/0'/0'", xpub: 'xpub-native' },
        'Account purpose must be "single_sig" or "multisig"',
      ],
      [
        { purpose: 'single_sig', scriptType: 'wrapped', derivationPath: "m/84'/0'/0'", xpub: 'xpub-native' },
        'Account scriptType must be one of: native_segwit, nested_segwit, taproot, legacy',
      ],
    ])('returns validation error for invalid account %#', async (account, error) => {
      const { normalizeIncomingAccounts } = await import('../../../../src/api/devices/accountConflicts');

      expect(normalizeIncomingAccounts([account as DeviceAccountInput], undefined, undefined)).toEqual({ error });
    });
    ...
    ])('builds a legacy account for %s', async (derivationPath, purpose, scriptType) => {
      const { normalizeIncomingAccounts } = await import('../../../../src/api/devices/accountConflicts');

      expect(normalizeIncomingAccounts(undefined, 'xpub-legacy', derivationPath)).toEqual({
        accounts: [{ purpose, scriptType, derivationPath, xpub: 'xpub-legacy' }],
      });
    });
    ...
    it('returns empty accounts for xpub-only legacy input', async () => {
      ...
      expect(normalizeIncomingAccounts(undefined, 'xpub-only', undefined)).toEqual({ accounts: [] });
    });
  });
}
