...
export function registerTransactionHttpReadTests(): void {
  ...
  it("passes Console-compatible date and type filters to wallet transaction listing", async () => {
    mockPrismaClient.wallet.findUnique.mockResolvedValue({
      network: "testnet",
    });
    mockPrismaClient.transaction.findMany.mockResolvedValue([]);
  ...
  it("handles array, blank, invalid, and single-sided Console transaction filters", async () => {
    mockPrismaClient.wallet.findUnique.mockResolvedValue({
      network: "testnet",
    });
    mockPrismaClient.transaction.findMany.mockResolvedValue([]);
    ...
    let response = await request(app).get(
      `/api/v1/wallets/${walletId}/transactions?type=unknown&dateFrom=not-a-date&dateFrom=2020-02-01T00%3A00%3A00.000Z&dateTo=2020-06-30T23%3A59%3A59.999Z`,
    );
  ...
  it("returns 500 when pending transaction query fails", async () => {
    ...
    mockPrismaClient.transaction.findMany.mockRejectedValue(
      new Error("pending query failed"),
    );
