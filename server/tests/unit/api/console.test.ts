...
describe('console API routes', () => {
  ...
  beforeEach(() => {
    ...
    mocks.updatePromptHistory.mockResolvedValue({ id: 'prompt-1', saved: true });
  ...
  it('lists tools and sessions for the authenticated actor', async () => {
    ...
    const sessions = await request(app).get('/api/v1/console/sessions?limit=5&offset=2');

    expect(tools.status).toBe(200);
    expect(tools.body.tools).toEqual([{ name: 'get_fee_estimates' }]);
    expect(sessions.status).toBe(200);
    expect(mocks.listConsoleTools).toHaveBeenCalledWith({ userId: 'user-1', username: 'alice', isAdmin: true });
  ...
  it('creates sessions and rejects malformed session requests', async () => {
    ...
    expect(mocks.createConsoleSession).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('lists turns and runs turns with audit context', async () => {
    const turns = await request(app).get('/api/v1/console/sessions/session-1/turns');
    const run = await request(app)
      .post('/api/v1/console/turns')
      .send({ prompt: 'how long ago was block 840000?' });
    ...
    expect(mocks.runConsoleTurn).toHaveBeenCalledWith(
      { userId: 'user-1', username: 'alice', isAdmin: true },
      expect.objectContaining({ prompt: 'how long ago was block 840000?' }),
      { ipAddress: '127.0.0.1', userAgent: 'test-agent' }
    );
  });

  it('lists prompt history with strict boolean parsing and rejects invalid prompt queries', async () => {
    const valid = await request(app).get('/api/v1/console/prompts?saved=false&includeExpired=1&search=fee');
    const invalid = await request(app).get('/api/v1/console/prompts?saved=definitely');

    expect(valid.status).toBe(200);
    expect(mocks.listPromptHistory).toHaveBeenCalledWith(
      { userId: 'user-1', username: 'alice', isAdmin: true },
      expect.objectContaining({ saved: false, includeExpired: true, search: 'fee' })
    );
    expect(invalid.status).toBe(400);
  });
  ...
  it('updates, deletes, and replays prompt history', async () => {
    ...
    const invalidPatch = await request(app)
      .patch('/api/v1/console/prompts/prompt-1')
      .send({ title: '' });
    const deleted = await request(app).delete('/api/v1/console/prompts/prompt-1');
