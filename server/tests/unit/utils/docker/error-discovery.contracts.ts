...
import { listAllContainers } from '../../../../src/utils/docker/common';
import {
  createTorContainer,
  discoverProjectName,
  mockFetch,
...
export function registerDockerErrorDiscoveryContracts(): void {
  ...
  describe('discoverProjectName', () => {
    it('returns no containers when the Docker inventory request is rejected', async () => {
      mockFetch.mockResolvedValueOnce({ ok: false, status: 503 });

      await expect(listAllContainers()).resolves.toEqual([]);
    });
    ...
    it('should prefer Docker Compose project labels over legacy container names', async () => {
      mockContainerList([
        container(
          '/legacy-backend-1',
          composeLabels('labeledsanctuary', 'backend'),
        ),
      ]);
    ...
    it('should use frontend Compose service labels for project discovery', async () => {
      mockContainerList([
        container(
          '/ignored-frontend-1',
          composeLabels('frontendproject', 'frontend'),
        ),
      ]);
    ...
    it('should fall back to frontend container names when Compose labels are missing', async () => {
      mockContainerList([container('/nameproject-frontend-1')]);
    ...
    it('uses the default when a discovery-looking name has no project prefix', async () => {
      mockContainerList([container('/-backend-1')]);

      await expect(discoverProjectName()).resolves.toBe('sanctuary');
    });
