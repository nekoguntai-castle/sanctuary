...
describe("assistant read-tool DTO redaction helpers", () => {
  ...
  it("summarizes policy configs without leaking user id lists", () => {
    ...
    ).toMatchObject({ perTransaction: 50, exemptRoleCount: 0 });
    ...
    expect(
      toPolicySummaryDto({
        ...
        config: {
          ...
          specificVetoers: ["user-1"],
        },
      }).config,
    ).toMatchObject({ delayHours: 2, specificVetoerCount: 1 });
    ...
    ).toMatchObject({ trigger: null, delayHours: 4, specificVetoerCount: 0 });
    ...
    ).toMatchObject({ mode: "allowlist", allowSelfSend: true });
    ...
    expect(
      toPolicySummaryDto({
        ...
        config: {
          scope: "per_user",
          maxPerDay: 3,
          exemptRoles: ["viewer", "signer"],
        },
      }).config,
    ).toMatchObject({ maxPerDay: 3, exemptRoleCount: 2 });
    ...
    ).toMatchObject({ maxPerHour: 1, maxPerWeek: 7, exemptRoleCount: 0 });

    expect(
      toPolicySummaryDto({ type: "unknown", config: null }).config,
    ).toEqual({ redacted: true });
    expect(
      toPolicySummaryDto({ type: "unknown", config: ["raw"] }).config,
    ).toEqual({ redacted: true });
    expect(
      toPolicySummaryDto({ type: "unknown", config: "raw" }).config,
    ).toEqual({ redacted: true });
  ...
  it("maps draft status without PSBT material", () => {
    ...
    expect(
      toDraftStatusDto({ amount: 3000n, fee: null, recipient: "bc1qone" }),
    ).toMatchObject({
      totalAmount: "3000",
      feeAmount: null,
      recipientCount: 1,
  ...
  it("maps batch two DTO detail helpers with sanitized defaults", () => {
    ...
    expect(toLabelSummaryDto({ _count: null })).toMatchObject({
      transactionCount: 0,
      addressCount: 0,
    });
    expect(
