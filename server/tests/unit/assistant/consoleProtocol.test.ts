...
describe('console protocol helpers', () => {
  it('normalizes prompts and optional dates', () => {
    ...
    expect(parseOptionalDate('2026-04-26T00:00:00.000Z')).toEqual(new Date('2026-04-26T00:00:00.000Z'));
  });

  it('parses prompt history booleans without treating "false" as truthy', () => {
    expect(ConsolePromptListQuerySchema.parse({ saved: 'false', includeExpired: '0' })).toMatchObject({
      saved: false,
      includeExpired: false,
    });
    expect(ConsolePromptListQuerySchema.parse({ saved: '1', includeExpired: 'true' })).toMatchObject({
      saved: true,
      includeExpired: true,
    });
    expect(ConsolePromptListQuerySchema.parse({ saved: ['true'], includeExpired: false })).toMatchObject({
      saved: true,
      includeExpired: false,
    });
    expect(ConsolePromptListQuerySchema.parse({ saved: '', limit: '500', offset: '-10' })).toMatchObject({
      limit: 100,
      offset: 0,
      includeExpired: false,
    });
    expect(ConsolePromptListQuerySchema.safeParse({ saved: ['true', 'false'] }).success).toBe(false);
    expect(ConsolePromptListQuerySchema.safeParse({ saved: 1 }).success).toBe(false);
    expect(ConsolePromptListQuerySchema.safeParse({ includeExpired: 'definitely' }).success).toBe(false);
  });
  ...
  it('parses console request bodies with defaults and validation', () => {
    ...
    expect(ConsoleCreateSessionBodySchema.parse({
      title: ' Wallet Q&A ',
    ...
    expect(ConsoleRunTurnBodySchema.parse({
      prompt: ' block height? ',
  ...
  it('extracts and checks wallet scopes', () => {
    ...
    expect(scopeWalletIds({ kind: 'wallet_set', walletIds: [walletId, secondWalletId] })).toEqual([walletId, secondWalletId]);
    expect(scopeWalletIds({
      kind: 'object',
      walletId,
    ...
    expect(scopeIncludesWallet({ kind: 'wallet', walletId }, walletId)).toBe(true);
    expect(scopeIncludesWallet({ kind: 'wallet_set', walletIds: [walletId] }, secondWalletId)).toBe(false);
    expect(scopeIncludesWallet({
      kind: 'object',
      walletId,
      objectType: 'address',
      objectId: 'bc1q',
  ...
  it('validates stored scopes and falls back to general on malformed JSON', () => {
    expect(parseStoredConsoleScope({ kind: 'wallet', walletId })).toEqual({ kind: 'wallet', walletId });
    expect(parseStoredConsoleScope({ kind: 'wallet_set', walletIds: [] })).toEqual({ kind: 'general' });
    expect(parseStoredConsoleScope(null)).toEqual({ kind: 'general' });
  });

  it('describes planning tools and compacts tool envelopes', () => {
    expect(describeToolForPlanning({
