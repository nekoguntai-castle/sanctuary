...
const mocks = vi.hoisted(() => {
  class MockAssistantToolError extends Error {
    ...
    constructor(statusCode: number, message: string, code = 'assistant_tool_error') {
      super(message);
...
function resetHappyPath() {
  ...
  mocks.consoleRepository.completeTurn.mockResolvedValue({ ...turn(), state: 'completed', response: 'answer' });
  mocks.consoleRepository.updatePromptMetadata.mockResolvedValue(promptHistory());
  mocks.consoleRepository.createToolTrace.mockImplementation(async (input: Record<string, unknown>) => completedTrace(input));
  ...
  mocks.planConsoleTools.mockResolvedValue({
    toolCalls: [{ name: 'get_wallet_overview', input: { walletId }, reason: 'Need wallet state' }],
    warnings: [],
...
describe('console service', () => {
  ...
  it('lists tools with admin availability applied', () => {
    ...
    expect(nonAdminTools).toEqual([
      ...
      expect.objectContaining({ name: 'get_admin_operational_summary', available: false }),
    ]);
    expect(adminTools[1]).toMatchObject({ name: 'get_admin_operational_summary', available: true });
  });
  ...
  it('runs a turn through planning, scoped tool execution, synthesis, prompt history, and audit', async () => {
    ...
    expect(result.turn).toMatchObject({ state: 'completed', response: 'answer' });
    expect(mocks.consoleRepository.createPrompt).toHaveBeenCalledWith(expect.objectContaining({
      prompt: 'What is this wallet doing?',
      normalizedPrompt: 'what is this wallet doing?',
    }));
    ...
    expect(mocks.synthesizeConsoleAnswer).toHaveBeenCalledWith(expect.objectContaining({
    ...
    expect(mocks.auditLog).toHaveBeenCalledWith(expect.objectContaining({
      ...
      details: expect.objectContaining({
        toolCount: 1,
        tools: [expect.objectContaining({ toolName: 'get_wallet_overview', status: 'completed' })],
      }),
      ipAddress: '127.0.0.1',
      userAgent: 'test-agent',
    }));
    expect(JSON.stringify(mocks.auditLog.mock.calls[0][0].details)).not.toContain('What is this wallet doing?');
  });
  ...
  it('stores a denied trace when a wallet-sensitive tool is requested without explicit wallet scope', async () => {
    ...
    mocks.consoleRepository.createToolTrace.mockImplementation(async (input: Record<string, unknown>) =>
      completedTrace({ ...input, status: input.status, errorCode: input.errorCode })
    );
  ...
  it('creates default-scope sessions when no wallet context is supplied', async () => {
    await expect(createConsoleSession({
      actor: actor(),
      maxSensitivity: 'wallet',
      expiresAt: '2026-04-27T00:00:00.000Z',
    })).resolves.toMatchObject({ id: sessionId });

    expect(mocks.consoleRepository.createSession).toHaveBeenCalledWith(expect.objectContaining({
      userId: 'user-1',
      scope: { kind: 'general' },
      maxSensitivity: 'wallet',
  ...
  it('returns not found when a supplied turn session is missing', async () => {
    ...
    await expect(runConsoleTurn(actor(), {
      sessionId,
      prompt: 'hello',
      scope: { kind: 'general' },
      maxSensitivity: 'wallet',
  ...
  it('accepts manual tool calls and applies the per-turn limit', async () => {
    ...
    expect(mocks.consoleRepository.updatePromptMetadata).toHaveBeenCalledWith(expect.objectContaining({
      tools: ['get_wallet_overview', 'get_wallet_overview', 'get_wallet_overview', 'get_wallet_overview', 'get_wallet_overview'],
    }));
    expect(mocks.consoleRepository.completeTurn).toHaveBeenCalledWith(expect.objectContaining({
      plannedTools: expect.objectContaining({ warnings: ['tool_call_limit_applied'] }),
    }));
  });

  it('accepts manual tool calls within the limit without warnings', async () => {
    await runConsoleTurn({ userId: 'user-1', isAdmin: false }, {
      prompt: 'manual',
      scope: { kind: 'wallet', walletId },
      maxSensitivity: 'wallet',
      toolCalls: [{ name: 'get_wallet_overview', input: { walletId } }],
  ...
  it('runs turns with default scope when no scope is supplied', async () => {
    ...
    expect(mocks.consoleRepository.createTurn).toHaveBeenCalledWith(expect.objectContaining({
      scope: { kind: 'general' },
    }));
  });

  it('replays stored prompts with validated stored scope and sensitivity fallbacks', async () => {
    mocks.consoleRepository.findPromptForUser.mockResolvedValue(promptHistory({
    ...
    expect(mocks.consoleRepository.markPromptReplayed).toHaveBeenCalledWith(promptId);
  ...
  it('updates, deletes, and purges prompt history through ownership checks', async () => {
    mocks.consoleRepository.findPromptForUser.mockResolvedValue(promptHistory());
    mocks.consoleRepository.updatePrompt.mockResolvedValue(promptHistory({ saved: true }));
    mocks.consoleRepository.softDeletePrompt.mockResolvedValue(promptHistory({ deletedAt: new Date() }));
    mocks.consoleRepository.purgeExpiredPromptHistory.mockResolvedValue(3);

    await expect(updatePromptHistory(actor(), promptId, { saved: true })).resolves.toMatchObject({ saved: true });
    await expect(deletePromptHistory(actor(), promptId)).resolves.toMatchObject({ id: promptId });
    await expect(purgeExpiredPromptHistory(new Date('2026-04-26T00:00:00.000Z'))).resolves.toBe(3);
  });
  ...
  it('lists sessions, turns, and prompt history through repository filters', async () => {
    ...
    await expect(listConsoleSessions(actor(), 10, 1)).resolves.toBe(sessionRows);
    await expect(listConsoleTurns(actor(), sessionId, 50)).resolves.toBe(turnRows);
    await expect(listPromptHistory(actor(), {
      limit: 20,
      offset: 5,
      search: '  Fee   Rate ',
      saved: true,
    ...
    expect(mocks.consoleRepository.listSessions).toHaveBeenCalledWith('user-1', 10, 1);
    expect(mocks.consoleRepository.listTurns).toHaveBeenCalledWith(sessionId, 50);
  ...
  it('returns not found for missing sessions and prompts', async () => {
    mocks.consoleRepository.findSessionForUser.mockResolvedValue(null);
    await expect(listConsoleTurns(actor(), sessionId)).rejects.toMatchObject({ statusCode: 404 });

    mocks.consoleRepository.findPromptForUser.mockResolvedValue(null);
    await expect(updatePromptHistory(actor(), promptId, { saved: true })).rejects.toMatchObject({ statusCode: 404 });
    await expect(deletePromptHistory(actor(), promptId)).rejects.toMatchObject({ statusCode: 404 });
    await expect(replayPromptHistory(actor(), promptId, {})).rejects.toMatchObject({ statusCode: 404 });
  });
  ...
  it('marks failed turns and audits when planning or synthesis fails', async () => {
    ...
    mocks.consoleRepository.failTurn.mockResolvedValue({ ...turn(), state: 'failed' });
    ...
    expect(mocks.consoleRepository.failTurn).toHaveBeenCalledWith(turnId, { message: 'model refused' });
    expect(mocks.auditLog).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it('normalizes non-error turn failures', async () => {
    ...
    expect(mocks.consoleRepository.failTurn).toHaveBeenCalledWith(turnId, { message: 'Console turn failed' });
  });

  it('rethrows service unavailable failures after recording turn failure', async () => {
    mocks.synthesizeConsoleAnswer.mockRejectedValue(new ServiceUnavailableError('proxy unavailable'));
    ...
    expect(mocks.consoleRepository.failTurn).toHaveBeenCalledWith(turnId, { message: 'proxy unavailable' });
  });
  ...
  it('replays prompt history without a stored session when no override is supplied', async () => {
    mocks.consoleRepository.findPromptForUser.mockResolvedValue(promptHistory({ sessionId: null }));
    mocks.planConsoleTools.mockResolvedValue({ toolCalls: [], warnings: [] });
