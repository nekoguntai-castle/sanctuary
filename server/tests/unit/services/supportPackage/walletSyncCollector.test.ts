...
function networkRow(overrides: Record<string, unknown> = {}) {
  return {
    ...
    incrementalPending: 0,
    unclaimedIncrementalPending: 0,
    claimedIncrementalPending: 0,
    trailingIncrementalRequest: 0,
    readyIncrementalPending: 0,
    maxIncrementalDrift: 0,
    unpreparedFullResyncPending: 0,
    preparedFullResyncPending: 0,
    actionRequired: 0,
    actionRequiredPending: 0,
    orphanedActionRequired: 0,
    deferredRetryPending: 0,
    dueRetryPending: 0,
    activeLease: 0,
    expiredLease: 0,
    inProgressWithoutClaim: 0,
    claimWithoutInProgress: 0,
    incoherentLease: 0,
    withSyncError: 0,
    ...overrides,
  };
  ...
  it('emits aggregate-only counts across every network axis', async () => {
    mockAggregates.mockResolvedValue({
      networks: [
        networkRow({
          ...
          incrementalPending: 3,
          unclaimedIncrementalPending: 1,
          claimedIncrementalPending: 2,
          trailingIncrementalRequest: 1,
          readyIncrementalPending: 1,
          maxIncrementalDrift: 2,
          unpreparedFullResyncPending: 1,
          preparedFullResyncPending: 1,
          actionRequired: 1,
          actionRequiredPending: 1,
          deferredRetryPending: 1,
          dueRetryPending: 1,
          activeLease: 1,
          expiredLease: 1,
          inProgressWithoutClaim: 1,
          claimWithoutInProgress: 1,
          incoherentLease: 1,
          withSyncError: 2,
        }),
        networkRow({
    ...
    expect(result).toMatchObject({
      ...
      incremental: {
        ...
        claimedPendingCount: 2,
        trailingRequestCount: 1,
        readyUnclaimedCount: 1,
        maxDrift: 'two_to_five',
      },
      actionRequired: { totalCount: 1, pendingIntentCount: 1, orphanedCount: 0 },
      retry: { deferredPendingCount: 1, duePendingCount: 1 },
      leaseAuthority: {
        activeCount: 1,
        expiredCount: 1,
        inProgressWithoutClaimCount: 1,
        claimWithoutInProgressCount: 1,
        incoherentCount: 1,
      },
      errorClasses: {
