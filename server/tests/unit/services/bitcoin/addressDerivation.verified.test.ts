 ...
 * by four exact implementations (Bitcoin Core, bitcoinjs-lib, Python, Go).
 *
...
function canonicalDescriptorsForMultisigVector(
  vector: VerifiedMultisigVector,
  xpubs: readonly string[] = vector.xpubs,
): {
  ...
  const descriptor = (branch: 0 | 1): string => {
    const keys = xpubs.map((xpub, signerIndex) =>
      `[${fingerprints[signerIndex]}/${accountPath}]${xpub}/${branch}/*`
    );
...
describe('Address Derivation - Cross-Implementation Verification', () => {
  ...
  describe('Descriptor derivation routing', () => {
    ...
    it('derives consecutive descriptor ranges from the requested start index', () => {
      const firstVector = getRequiredSingleSigVector('native_segwit', 'testnet3', false, 1);
    ...
    describe('Key ordering consistency', () => {
      it('should produce identical addresses regardless of input key order', () => {
        const vector = VERIFIED_MULTISIG_VECTORS.find(candidate => (
          candidate.network === 'testnet3' && candidate.scriptType === 'p2wsh'
        ))!;
        const canonical = canonicalDescriptorsForMultisigVector(vector).receiveDescriptor;
        const reversed = canonicalDescriptorsForMultisigVector(
          vector,
          [...vector.xpubs].reverse(),
        ).receiveDescriptor;
        expect(deriveAddressFromDescriptor(canonical, vector.index, { network: vector.network }).address)
          .toBe(deriveAddressFromDescriptor(reversed, vector.index, { network: vector.network }).address);
      });
    });
  });
...
describe('Verification coverage summary', () => {
  it('has the exact single-sig policy and chain coverage', () => {
    ...
    expect(VERIFIED_SINGLESIG_VECTORS).toHaveLength(240);
    expect(scriptTypes).toEqual(new Set(['legacy', 'nested_segwit', 'native_segwit', 'taproot']));
    expect(networks).toEqual(new Set(['mainnet', 'testnet3', 'testnet4', 'signet', 'regtest']));
  });

  it('has the exact multisig policy and chain coverage', () => {
    const scriptTypes = new Set(VERIFIED_MULTISIG_VECTORS.map(v => v.scriptType));
    const networks = new Set(VERIFIED_MULTISIG_VECTORS.map(v => v.network));
    expect(VERIFIED_MULTISIG_VECTORS).toHaveLength(240);
    expect(scriptTypes).toEqual(new Set(['p2sh_p2wsh', 'p2wsh']));
    expect(networks).toEqual(new Set(['mainnet', 'testnet3', 'testnet4', 'signet', 'regtest']));
  });
});
