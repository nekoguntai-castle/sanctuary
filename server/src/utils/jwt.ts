...
export interface RefreshSessionLineage {
  sessionFamilyId: string;
  expiresAt: Date;
}
...
const parseRefreshTokenPayload = (value: unknown): RefreshTokenPayload => {
  ...
  if (
    ...
    payload.sessionFamilyId.length === 0 ||
...
export function generateRefreshToken(
  userId: string,
  sessionVersion = 0,
  sessionFamilyId: string = randomUUID()
): string {
  const jti = generateJti();
  return jwt.sign(
    {
...
/** Extract required revocation lineage from a token generated by this service. */
export function getTokenLineage(token: string): TokenLineage {
  ...
  return { jti: decoded.jti, expiresAt: new Date((expiresAtSeconds as number) * 1000) };
}

/** Extract stable family lineage from an already-verified refresh token. */
export function getRefreshSessionLineage(token: string): RefreshSessionLineage {
  const decoded = decodeToken(token);
  const expiresAtSeconds = decoded?.exp;
  if (!isNonEmptyString(decoded?.sessionFamilyId)
    || !Number.isSafeInteger(expiresAtSeconds)
    || expiresAtSeconds! <= 0) {
    throw new Error('Refresh token is missing session-family lineage');
  }
  return {
    sessionFamilyId: decoded.sessionFamilyId,
    expiresAt: new Date((expiresAtSeconds as number) * 1000),
  };
}
