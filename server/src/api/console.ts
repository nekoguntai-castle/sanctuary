...
router.get('/sessions', asyncHandler(async (req, res) => {
  ...
  const sessions = await listConsoleSessions(actorFromRequest(req), limit, offset);
  res.json({ sessions });
}));
...
router.get('/sessions/:id/turns', asyncHandler(async (req, res) => {
...
router.patch('/prompts/:id', asyncHandler(async (req, res) => {
  ...
  const prompt = await updatePromptHistory(actorFromRequest(req), req.params.id, body.data);
  res.json({ prompt });
}));
