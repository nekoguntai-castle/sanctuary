...
export const CONSOLE_SENSITIVITY_VALUES = [
  "public",
  "wallet",
  "high",
  "admin",
...
export const ConsoleScopeSchema = z.discriminatedUnion("kind", [
  ...
  z
    .object({
      ...
      walletIds: z.array(UuidSchema).min(1).max(MAX_WALLET_SET_SCOPE_WALLETS),
    })
    .strict(),
  z
    .object({
...
export type ConsoleClientContext = z.infer<typeof ConsoleClientContextSchema>;

const DEFAULT_CONSOLE_SCOPE: ConsoleScope = { kind: "general" };

export const ConsoleToolCallSchema = z
  .object({
...
export const ConsoleRunTurnBodySchema = z
  .object({
    sessionId: UuidSchema.optional(),
    prompt: z.string().trim().min(1).max(12000),
    scope: ConsoleScopeSchema.optional(),
    clientContext: ConsoleClientContextSchema.optional(),
    maxSensitivity: ConsoleSensitivitySchema.default("wallet"),
    expiresAt: OptionalIsoDateSchema,
...
export const ConsolePromptUpdateBodySchema = z
  .object({
...
export function parseStoredConsoleSensitivity(
  value: unknown,
): ConsoleSensitivity {
...
export function scopeIncludesWallet(
  scope: ConsoleScope,
  walletId: string,
): boolean {
...
export function sensitivityAllowed(
  ...
  const rank: Record<ConsoleSensitivity, number> = {
    public: 0,
    wallet: 1,
    high: 2,
    admin: 3,
...
export function describeToolForPlanning(
