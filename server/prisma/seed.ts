...
async function main() {
  ...
  if (existingNodeConfig) {
    ...
    console.log(
      "Created default node configuration (Blockstream public Electrum server)",
    );
  }
  ...
  if (existingAdmin) {
    console.log(
      `Default admin user '${DEFAULT_ADMIN.username}' already exists, skipping...`,
    );
  } else {
    ...
    console.log(
      "║  IMPORTANT: You will be required to change this password on        ║",
    );
    console.log(
