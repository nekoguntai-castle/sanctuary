...
if [ "${1:-}" = "-e" ]; then
  ...
  count=$((count + 1))
  printf '%s\n' "$count" > "$counter_file"
  if [ "$count" -eq 1 ]; then printf '%064d' 0 | tr '0' a; else printf '%064d' 0 | tr '0' b; fi
  exit 0
fi
exit 2
NODEEOF
...
if [ "$1" = "buildx" ] && [ "${2:-}" = "build" ]; then
  ...
  while [ "$#" -gt 0 ]; do
    if [ "$1" = "--iidfile" ]; then
      printf '%s\n' 'sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc' > "$2"
      break
    fi
    shift
  done
...
main() {
  ...
  grep -F -- '--tag sanctuary/verify-addresses-python:3.13.5-bip-utils-2.12.1-v1' \
    "$TEST_TEMP_DIR/docker-build-log" >/dev/null ||
    fail 'expected the Python verifier build to use its exact image identity'
