...
set -euo pipefail
if [ "${1:-}" = "--version" ]; then
  echo "${SANCTUARY_STUB_NPM_VERSION:-11.19.1}"
  exit 0
fi
exit 2
EOF
...
main() {
  ...
  PATH="$TEST_TEMP_DIR/bin:$PATH" NODE_VERSION=24.14.1 NPM_VERSION=11.19.1 bash "$NODE_SCRIPT" >/dev/null
  ...
  assert_fails_with 'expected Node.js 24.14.2, got 24.14.1' \
    env PATH="$TEST_TEMP_DIR/bin:$PATH" NODE_VERSION=24.14.2 NPM_VERSION=11.19.1 bash "$NODE_SCRIPT"
  assert_fails_with 'NODE_VERSION is required' \
    env -u NODE_VERSION -u SANCTUARY_NODE_VERSION PATH="$TEST_TEMP_DIR/bin:$PATH" NPM_VERSION=11.19.1 bash "$NODE_SCRIPT"
  assert_fails_with 'expected npm 11.19.2, got 11.19.1' \
    env PATH="$TEST_TEMP_DIR/bin:$PATH" NODE_VERSION=24 NPM_VERSION=11.19.2 bash "$NODE_SCRIPT"
  ...
  assert_fails_with 'unexpected arguments' \
    env PATH="$TEST_TEMP_DIR/bin:$PATH" NODE_VERSION=24 NPM_VERSION=11.19.1 bash "$NODE_SCRIPT" extra
