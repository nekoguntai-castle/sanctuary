...
function runReporterFixture() {
  ...
  reporter.onTestEnd(
    fakeTest('tests/e2e/wallet-flow.spec.ts', 'chromium', ['chromium', 'wallet flow', 'creates wallet']),
    { status: 'passed', retry: 0, duration: 1250 },
  );
  reporter.onTestEnd(
    fakeTest('tests/e2e/wallet-flow.spec.ts', 'chromium', ['chromium', 'wallet flow', 'sends payment']),
    { status: 'failed', retry: 1, duration: 2750 },
  );
  reporter.onTestEnd(
    fakeTest('tests/e2e/settings.spec.ts', 'chromium', ['chromium', 'settings', 'updates profile']),
...
function assertReporterOutputs() {
  ...
  try {
    ...
    assert.equal(report.specs[0].file, 'tests/e2e/settings.spec.ts');
    ...
    assert.match(markdown, /\| tests\/e2e\/settings\.spec\.ts \| 5s \| 1 \| chromium \| passed: 1 \|/);
...
function assertPureReportHelpers() {
  ...
  assert.equal(report.slowestTests[0].file, 'tests/e2e/a.spec.ts');
}
