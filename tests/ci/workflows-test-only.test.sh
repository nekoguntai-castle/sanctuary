...
[ "$(grep -c -- '-- scripts/ci/build-runtime-image.sh' "$docker_workflow")" -eq 6 ] ||
  fail "Docker validation must locally build, smoke, and attest all six shipped images"
[ "$(grep -c 'scripts/ci/cleanup-ci-callsite.sh run' "$docker_workflow")" -eq 6 ] ||
  fail "Every Docker validation image build must run under the cleanup coordinator"
[ "$(grep -c 'name: Upload .* cleanup evidence' "$docker_workflow")" -eq 6 ] ||
  fail "Every Docker validation image build must upload final cleanup evidence"
[ "$(grep -c 'name: Require .* final cleanup evidence' "$docker_workflow")" -eq 6 ] ||
