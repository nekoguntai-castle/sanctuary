...
vi.mock("../../llm-egress-proxy/src/aiClient", () => ({
  callExternalAIWithMessagesResult: mocks.callExternalAIWithMessagesResult,
  parseStructuredResponse: (raw: string) => JSON.parse(raw),
}));

import { registerConsoleRoutes } from "../../llm-egress-proxy/src/consoleRoutes";
import { AI_ANALYSIS_TIMEOUT_MS } from "../../llm-egress-proxy/src/constants";
...
describe("LLM egress proxy console routes", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
