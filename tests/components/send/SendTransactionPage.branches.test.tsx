...
const wizardMockHelpers = vi.hoisted(() => ({
  getLoadedDataValues(props: any) {
    ...
    const mempoolBlocks = props.mempoolBlocks || [];
    const draftTxData = props.draftTxData || {};

    return [
      ["wallet-type", wallet.type || ""],
      ["device-count", devices.length],
      ["address-count", walletAddresses.length],
      ["min-fee", fees.minimumFee || ""],
      ["mempool-count", mempoolBlocks.length],
      ["draft-fee", draftTxData.fee || ""],
...
describe("SendTransactionPage branch coverage", () => {
  ...
  const renderPage = (state?: Record<string, unknown>) =>
    render(
      <MemoryRouter
        initialEntries={[{ pathname: "/wallets/wallet-1/send", state }]}
      >
        <Routes>
  ...
  it("covers multisig descriptor fallback matching, address mapping, and fee fallback branches", async () => {
    vi.mocked(walletsApi.getWallet).mockResolvedValue({
      ...
      descriptor:
        "wsh(sortedmulti(2,[abcd1234/48'/0'/0'/2']xpubA,[eeee1111/48'/0'/0'/2']xpubB))",
    } as any);
    ...
    vi.mocked(devicesApi.getDevices).mockResolvedValue([
      {
        ...
        label: "No Match",
        type: "ledger",
        wallets: [{ wallet: { id: "other-wallet" } }],
      },
      {
        id: "fp-match",
        label: "FP Match",
  ...
  it("covers draft UTXO filtering with locked-draft fallback and unavailable-info warning", async () => {
    ...
    expect(showInfoMock).toHaveBeenCalledWith(
      "1 UTXOs are no longer available",
    );
  });
  ...
  it("covers preselected UTXO frozen-removal branches (singular and plural)", async () => {
    ...
    expect(screen.getByTestId("show-coin-control")).toHaveTextContent("true");
    expect(showInfoMock).toHaveBeenCalledWith(
      "1 frozen UTXO removed from selection",
    );
    first.unmount();
    ...
    expect(showInfoMock).toHaveBeenCalledWith(
      "2 frozen UTXOs removed from selection",
    );
  });
