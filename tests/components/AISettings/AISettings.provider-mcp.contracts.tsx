...
function getTabButton(name: string) {
  const tabSpan = screen.getAllByText(name).find((element) => element.classList.contains('hidden'));
  return tabSpan?.closest('button');
}

async function navigateToTab(user: ReturnType<typeof userEvent.setup>, name: string, readyText: string) {
  await waitFor(() => {
...
export function registerAISettingsProviderMcpContracts() {
  describe('Provider and MCP settings', () => {
    ...
    it('saves typed provider profile changes and credential updates', async () => {
      ...
      await user.selectOptions(screen.getByLabelText('Provider Profile'), 'default-ollama');
      ...
      await user.type(screen.getByLabelText('AI Endpoint URL'), 'http://llm.local:11434/v1');
      await user.type(screen.getByLabelText('Provider Credential'), 'secret-key');
      ...
      await waitFor(() => {
        expect(mockUpdateSystemSettings).toHaveBeenCalledWith(expect.objectContaining({
          ...
          aiProviderProfiles: expect.arrayContaining([expect.objectContaining({
            id: 'default-ollama',
            name: 'LAN OpenAI',
    ...
    it('creates and revokes MCP keys from the MCP Access tab', async () => {
      mockGetUsers.mockResolvedValue([
        ...
        { id: 'user-2', username: 'bob', email: null, emailVerified: true, isAdmin: false, createdAt: '2026-04-26T00:00:00.000Z' },
      ]);
      ...
      await user.type(screen.getByLabelText('Wallet Scope'), 'wallet-1 wallet-1 wallet-2');
      ...
      expect(screen.getByText(/mcp_test_token_visible_once/)).toBeInTheDocument();
