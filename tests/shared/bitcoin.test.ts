...
describe("Shared Bitcoin Utilities", () => {
  ...
  describe("isValidAddressFormat", () => {
    describe("mainnet addresses", () => {
      it("should validate legacy addresses (P2PKH)", () => {
        expect(isValidAddressFormat("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa")).toBe(
          true,
        );
        expect(isValidAddressFormat("1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2")).toBe(
          true,
        );
      });

      it("should validate P2SH addresses", () => {
        expect(isValidAddressFormat("3J98t1WpEZ73CNmYviecrnyiWrnqRhWNLy")).toBe(
          true,
        );
      });
    ...
    describe("testnet addresses", () => {
      it("should validate testnet legacy addresses (m/n)", () => {
        expect(isValidAddressFormat("mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn")).toBe(
          true,
        );
        expect(isValidAddressFormat("n1ZCYg9YXtB5XCZazLxSmPDa8iwJRZHhGx")).toBe(
          true,
        );
      });
      ...
      it("should validate testnet P2SH addresses (2)", () => {
        expect(
          isValidAddressFormat("2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc"),
        ).toBe(true);
      });
  ...
  describe("detectAddressType", () => {
    ...
    it("should detect P2SH addresses", () => {
      expect(detectAddressType("3J98t1WpEZ73CNmYviecrnyiWrnqRhWNLy")).toBe(
        "p2sh",
      );
    });
    ...
    it("should detect testnet legacy addresses", () => {
      ...
      expect(detectAddressType("n1ZCYg9YXtB5XCZazLxSmPDa8iwJRZHhGx")).toBe(
        "testnet_legacy",
      );
    });

    it("should detect testnet P2SH addresses", () => {
      expect(detectAddressType("2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc")).toBe(
        "testnet_p2sh",
      );
    });
    ...
    it("should detect testnet SegWit addresses", () => {
      expect(
        detectAddressType("tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx"),
      ).toBe("testnet_segwit");
    });
  ...
  describe("isMainnetAddress", () => {
    it("should return true for mainnet addresses", () => {
      ...
      expect(
        isMainnetAddress("bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4"),
      ).toBe(true);
      expect(
        isMainnetAddress(
          "bc1p5d7rjq7g6rdk2yhzks9smlaqtedr4dekq08ge8ztwac72sfr9rusxg3297",
    ...
    it("should return false for testnet addresses", () => {
      expect(isMainnetAddress("mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn")).toBe(
        false,
      );
      expect(isMainnetAddress("2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc")).toBe(
        false,
      );
      expect(
  ...
  describe("isTestnetAddress", () => {
    it("should return true for testnet addresses", () => {
      ...
      expect(isTestnetAddress("2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc")).toBe(
        true,
      );
      expect(
    ...
    it("should return false for mainnet addresses", () => {
      ...
      expect(isTestnetAddress("3J98t1WpEZ73CNmYviecrnyiWrnqRhWNLy")).toBe(
        false,
      );
      expect(
  ...
  describe("derivation path helpers", () => {
    it("normalizes derivation path prefixes and hardening notation", () => {
      ...
      expect(normalizeDerivationPath(null as any)).toBe("m/");
    });
    ...
    it("parses single-sig account paths into structured fields", () => {
      expect(parseDerivationPath("84h/1h/2h/0/15")).toMatchObject({
        normalizedPath: "m/84'/1'/2'/0/15",
        purpose: 84,
        coinType: 1,
        accountIndex: 2,
        scriptPath: null,
    ...
    it("parses BIP-48 script path metadata", () => {
      expect(parseDerivationPath("m/48'/0'/3'/1'/1/9")).toMatchObject({
        purpose: 48,
        coinType: 0,
        accountIndex: 3,
        scriptPath: 1,
        changeIndex: 1,
        addressIndex: 9,
        accountPath: "m/48'/0'/3'/1'",
        accountPurpose: "multisig",
        scriptType: "nested_segwit",
      });
    });

    it("maps standard single-sig purposes to script types", () => {
      expect(parseDerivationPath("m/44'/0'/0'")).toMatchObject({
        accountPurpose: "single_sig",
        scriptType: "legacy",
      });
      expect(parseDerivationPath("m/49'/0'/0'")).toMatchObject({
        accountPurpose: "single_sig",
        scriptType: "nested_segwit",
      });
      expect(parseDerivationPath("m/86'/0'/0'")).toMatchObject({
        accountPurpose: "single_sig",
        scriptType: "taproot",
      });
    });

    it("parses one-component and malformed address suffixes explicitly", () => {
      expect(parseDerivationPath("m/84'/0'/0'/7")).toMatchObject({
        changeIndex: null,
        addressIndex: 7,
        valid: true,
      });
      expect(parseDerivationPath("m/84'/0'/0'/bad/4")).toMatchObject({
        changeIndex: null,
        addressIndex: 4,
        valid: false,
      });
      expect(parseDerivationPath("m/84'/0'/0'/0/bad")).toMatchObject({
        changeIndex: 0,
        addressIndex: null,
        valid: false,
      });
    ...
    it("marks unknown derivation purposes without guessing script type", () => {
      expect(parseDerivationPath("m/99'/0'/0'")).toMatchObject({
        purpose: 99,
    ...
    it("does not build an account path from incomplete account components", () => {
      expect(parseDerivationPath("m/84'")).toMatchObject({
        purpose: 84,
        accountPath: null,
        accountPurpose: "single_sig",
        scriptType: "native_segwit",
    ...
    it("marks malformed derivation components as invalid", () => {
      expect(parseDerivationPath("m/84'/bad/0'")).toMatchObject({
        purpose: 84,
        coinType: null,
        accountIndex: 0,
        accountPath: null,
    ...
    it("marks unsafe numeric derivation components as invalid", () => {
      expect(parseDerivationPath("m/9007199254740992'/0'/0'")).toMatchObject({
        purpose: null,
        coinType: 0,
        accountIndex: 0,
        accountPath: null,
    ...
    it("handles empty or null derivation paths without throwing", () => {
      expect(parseDerivationPath("")).toMatchObject({
        normalizedPath: "m/",
        purpose: null,
        accountPurpose: "unknown",
        scriptType: "unknown",
      ...
      expect(parseDerivationPath(null as any)).toMatchObject({
        normalizedPath: "m/",
        purpose: null,
        accountPurpose: "unknown",
        scriptType: "unknown",
