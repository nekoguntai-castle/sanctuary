...
# #1057: optional-owned-source is the non-blocking canary that
...
test_optional_owned_source_canary_fixture_contract() {
  ...
  assert_not_contains "$(upgrade_active_extended_fixture_records)" "optional-owned-source" \
    "canary fixture must not join the required/active fixture registry" || failures=1
  assert_not_contains "$(upgrade_active_extended_fixtures_csv)" "optional-owned-source" \
    "canary fixture must not join the required/active fixture CSV" || failures=1
  assert_contains "$(upgrade_canary_extended_fixture_records)" "optional-owned-source 36" \
    "canary fixture registry should carry the dedicated fixture" || failures=1
  assert_equals "latest-stable" "$(upgrade_extended_fixture_source_ref optional-owned-source latest-stable)" \
    "canary fixture should track the default source ref, not the optional-profiles pin" || failures=1
  upgrade_validate_extended_fixture_selection "optional-owned-source" || {
    echo -e "${RED}ASSERTION FAILED:${NC} canary fixture selection should validate"
    failures=1
  }
  validate_upgrade_fixture "optional-owned-source" || {
  ...
  validate_upgrade_fixture "optional-owned-source"
  apply_upgrade_fixture_defaults "optional-owned-source"
...
# #1057 canary incident (run 17568, job 220695): every extended-fixture name
# the selection registry can produce becomes a "extended-<fixture>" cleanup
# lane in run-extended-upgrade-fixtures.sh, and
# scripts/ownership/ci-cleanup-authority.mjs rejects any lane that does not
# match /^[a-z0-9][a-z0-9-]{0,31}$/ (max 32 chars). The
# optional-profiles-owned-source canary's first real run produced a
# 39-character lane and died in seconds with "cleanup lane has an invalid
# format", before any install work ran. This reads the real regex out of the
# .mjs (rather than duplicating the literal) so a future registry addition or
# a regex change there is both caught here.
test_extended_fixture_lane_names_fit_cleanup_lane_limit() {
  local failures=0
  local authority_mjs="$PROJECT_ROOT/scripts/ownership/ci-cleanup-authority.mjs"
  local fixture lane result

  while read -r fixture _offset; do
    [ -n "$fixture" ] || continue
    lane="extended-${fixture}"
    if ! result="$(node -e '
      const fs = require("fs");
      const src = fs.readFileSync(process.argv[1], "utf8");
      const m = src.match(/const LANE = (\/.*\/);/);
      if (!m) {
        console.error("LANE regex not found in ci-cleanup-authority.mjs");
        process.exit(2);
      }
      const LANE = eval(m[1]);
      const lane = process.argv[2];
      if (!LANE.test(lane)) {
        console.error(`cleanup lane "${lane}" (length ${lane.length}) does not match ci-cleanup-authority LANE ${m[1]}`);
        process.exit(1);
      }
    ' "$authority_mjs" "$lane" 2>&1)"; then
      echo -e "${RED}ASSERTION FAILED:${NC} extended fixture cleanup lane must fit ci-cleanup-authority.mjs LANE format"
      echo "  $result"
      failures=1
    fi
