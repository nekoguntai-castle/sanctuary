...
main() {
  ...
  head_sha="$(git -C "$repo_dir" rev-parse HEAD)"
  run_classifier "$repo_dir" "$base_sha" "$head_sha" "$output_file"
  assert_scope "$output_file" "true" "true" "true" "false" "true" "true" "false" "false" "false" "true"
  assert_exact_output "$output_file" "scope" "runtime-entrypoint"

  base_sha="$head_sha"
  commit_file "$repo_dir" "scripts/ops/migrate-grafana-password.sh" "#!/usr/bin/env bash" "Grafana migration payload"
  head_sha="$(git -C "$repo_dir" rev-parse HEAD)"
  run_classifier "$repo_dir" "$base_sha" "$head_sha" "$output_file"
  assert_scope "$output_file" "true" "true" "false" "false" "false" "false" "false" "false" "false" "false"
  assert_upgrade_selection "$output_file" "" ""
  assert_exact_output "$output_file" "scope" "grafana-migration"

  base_sha="$head_sha"
  commit_file "$repo_dir" "scripts/ops/run-grafana-password-migration.sh" "#!/usr/bin/env bash" "Grafana migration coordinator"
  head_sha="$(git -C "$repo_dir" rev-parse HEAD)"
  run_classifier "$repo_dir" "$base_sha" "$head_sha" "$output_file"
  assert_scope "$output_file" "true" "true" "false" "false" "false" "false" "false" "false" "false" "false"
  assert_upgrade_selection "$output_file" "" ""
  ...
  base_sha="$head_sha"
  commit_file "$repo_dir" "scripts/ops/grafana-quiescence-records.sh" "#!/usr/bin/env bash" "Grafana control records"
  head_sha="$(git -C "$repo_dir" rev-parse HEAD)"
  run_classifier "$repo_dir" "$base_sha" "$head_sha" "$output_file"
  assert_scope "$output_file" "true" "true" "false" "false" "false" "false" "false" "false" "false" "false"
  assert_upgrade_selection "$output_file" "" ""
  ...
  base_sha="$head_sha"
  commit_file "$repo_dir" "scripts/ci/subject-budget.mjs" "// Shared job deadline" "subject budget helper"
  head_sha="$(git -C "$repo_dir" rev-parse HEAD)"
  run_classifier "$repo_dir" "$base_sha" "$head_sha" "$output_file"
  assert_release_smoke_scope "$output_file"
  ...
  base_sha="$head_sha"
  commit_file "$repo_dir" "docker/grafana-migration/Dockerfile" "FROM scratch" "Grafana migration image"
  head_sha="$(git -C "$repo_dir" rev-parse HEAD)"
  run_classifier "$repo_dir" "$base_sha" "$head_sha" "$output_file"
  assert_scope "$output_file" "true" "true" "true" "false" "true" "false" "false" "false" "false" "true"
