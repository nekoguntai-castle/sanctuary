...
run_unit_suites() {
    ...
    if ! run_test_suite "Grafana Password Migration Tests" "$SCRIPT_DIR/unit/grafana-password-migration.test.sh"; then
        suite_failed=true
    fi
    if ! run_test_suite "Grafana Quiescence Tests" "$SCRIPT_DIR/unit/grafana-quiescence.test.sh"; then
        suite_failed=true
    fi
    if ! run_test_suite "Reset 2FA Script Unit Tests" "$SCRIPT_DIR/unit/reset-user-2fa-script.test.sh"; then
        suite_failed=true
    fi
    if ! run_test_suite "Upgrade Backup Script Unit Tests" "$SCRIPT_DIR/unit/upgrade-backup-script.test.sh"; then
        suite_failed=true
    fi
    if ! run_test_suite "Offline Bundle Script Unit Tests" "$SCRIPT_DIR/unit/offline-bundle-script.test.sh"; then
        suite_failed=true
    fi
    if ! run_test_suite "Offline Compose Image Contract Tests" "$SCRIPT_DIR/unit/offline-compose-images.test.sh"; then
        suite_failed=true
    fi
