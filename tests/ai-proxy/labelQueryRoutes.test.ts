...
import { registerLabelQueryRoutes } from '../../ai-proxy/src/labelQueryRoutes';
import { RATE_LIMIT_MAX_REQUESTS } from '../../ai-proxy/src/constants';
...
describe('AI proxy label/query routes', () => {
  ...
  it('returns the AI proxy 429 payload after the configured request limit', async () => {
    const { app, routes } = makeApp();
    registerLabelQueryRoutes(app as any, makeDeps() as any);
